    ...
    P("全量-01", "全量確認処理開始（定期）", "全量確認処理開始（定期）", "全量（日次）",
      ...
      d=dict(
        ...
        inputs=[
          ...
          ["FlexibleTimeWindow", "OFF / FLEXIBLE", "要", "同上",
           "**Mode は指定必須**。全量確認は時刻の厳密性を要さないため FLEXIBLE（例 15 分）を推奨。起動を分散でき、"
           "同時刻に集中する他ジョブとの競合を避けられる（M-Q-PD-15）"],
        ...
        exceptions=[
          ...
          ["リトライ枯渇", "InvocationDroppedCount / InvocationsSentToDeadLetterCount",
           "DLQ へ退避（運用-02 で再処理）", "MM-2 → P2 Platform", "18 §18.5.1"],
          ["全量確認が丸ごと動かない（スケジュール無効化等）", "—",
           "**巡回（MM-1）のような専用のハートビートを持たない**ため、気づくのが遅れる",
           "—", "M-Q-PD-17"],
        ],
        idem=(
        ...
        opens=[
          ...
          ["M-Q-PD-15", "FlexibleTimeWindow を FLEXIBLE にするか OFF にするか。"
           "全量確認は時刻厳密性を要さないため FLEXIBLE を推奨するが、"
           "「毎日 X 時に走る」ことを運用が前提にするなら OFF の方が説明しやすい", "運用 / 設計担当", ""],
          ["M-Q-PD-17", "全量確認そのものが止まったことをどう検知するか。"
           "巡回には MM-1（DiscoveryLastSuccess の欠損）があるが、全量確認には対応するハートビートが無い。"
           "案: 全量確認の完了時に FullScanLastSuccess を emit し、**36 時間欠損で発報**（MM-1 と同じく検知の遅れは許容）",
           "設計担当（18 §18.5.1 に MM-6 として追加するか要判断）", ""],
        ],
      )),
    ...
    P("全量-04", "アプリ単位確認依頼", "アプリ単位確認依頼（fan-out）", "全量（日次・手動）",
      ...
      d=dict(
        ...
        perf=(
          ...
          "**同時実行数（既定 1,000）を対象件数分消費する**。アプリ単位の並列は別ドメインへのアクセスのため"
          "1 アプリに負荷が集中することはないが、件数が増えたら並列度の上限設定を検討する（M-Q-PD-18）。"
          "Lambda のスケーリングは 1 関数あたり 10 秒で 1,000 実行環境まで",
        ),
        opens=[
          ["M-Q-PD-18", "fan-out の並列度に上限を設けるか。"
           "アプリ単位の並列は別ドメインへのアクセスなので負荷は分散するが、"
           "アプリ数が増えると同時実行数を一気に消費し、巡回起点の検査（対象検索-11）を圧迫する可能性がある。"
           "案: 予約同時実行の割り当て、またはバッチ分割して時間をずらす", "設計担当", ""],
