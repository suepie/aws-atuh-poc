...
PROCESSES = [
    ...
    P("対象検索-01", "認証実装確認処理開始", "認証実装確認処理開始", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-1", "【解決・2026-09-18】**リトライ 2 回 / 最大 600 秒**で確定。"
           "AWS 公式は 185 回・24 時間を「最大値」としか記載しておらず既定値が不明なため、明示設定する。"
           "巡回は 1 時間ごとに再実行されるため、長時間リトライするより早く諦めて DLQ + MM-2 で気づく方がよい",
           "—（解決済み）", ""],
          ["M-Q-PD-2", "【解決・2026-09-14】MM-1（巡回停止検知）の閾値は**緩和する**。"
           ...
           "設計担当", ""],
          ["M-Q-PD-3", "【解決・2026-09-18】**予約同時実行 1 を設定する**。多重起動を抑止し 412 競合を減らす。スロットルされた起動は Scheduler 側のリトライ（PD-1）で吸収し、枯渇すれば DLQ + MM-2 で気づける", "—（解決済み）", ""],
        ],
      )),
    P("対象検索-02", "対象アカウント一覧取得", "対象アカウント一覧取得", "対象検索（1 時間毎）",
      "対象検索 Lambda", "SSM Parameter Store（監視対象アカウント一覧）", "同期", "対象検索 Lambda", "巡回ごとに 1 回",
      "巡回対象となる App アカウントの一覧を、中央が管理する登録リストから取得する。",
      "ssm:GetParameter（対象パラメータに限定）/ ssm.{region}.amazonaws.com",
      "対象検索 Lambda -> SSM Parameter Store : GetParameter（監視対象アカウント一覧）\nSSM --> 対象検索 Lambda : アカウント ID の配列\n【シーケンス図の所在】17 §17.2.1 巡回シーケンス図（対象検索-01〜14 を通した図）/ 10 §10.1.7 F1",
      "登録リストの置き場と、アカウント追加時のオンボーディング手順を明記する",
      d=dict(
        position="対象検索-01 の直後。巡回 1 回につき 1 度だけ実行し、以降の 対象検索-03〜13 をアカウント単位でループする。",
        pre=[
          "監視対象アカウントの登録リストが SSM Parameter Store に存在すること",
          "リストへの登録は**申請ベース**（案件側が連携バケットと読み取りロールを作成し、中央へアカウント ID の登録を依頼する）。"
          "オンボーディング手順は vendor-guide に記載",
          "Lambda ロールに ssm:GetParameter（対象パラメータに限定）があること",
        ],
        inputs=[
          ["パラメータ名", "string", "要", "Lambda 環境変数", "例 /auth-monitoring/target-accounts"],
          ["登録リスト", "JSON 配列", "要", "SSM Parameter Store",
           "[{accountId, appOwner, registeredAt}] 形式。**中央が管理する**（アプリ側からは書き換えられない）"],
        ],
        outputs=[
          ["対象アカウント一覧", "[{accountId, …}]", "対象検索-03 以降（メモリ内）", "登録されているアカウントのみ"],
          ["列挙結果ログ", "JSON 1 行", "CloudWatch Logs", "取得件数。アカウント ID は出すが担当者情報は出さない"],
        ],
        steps=[
          ["SSM Parameter Store から監視対象アカウントの登録リストを取得する",
           "**Organizations の ListAccounts は使わない**（2026-09-16 確定）。"
           "連携バケットと読み取りロールを**案件側が作成する**方式にしたため、"
           "組織全体を列挙しても『どのアカウントが監視対象か』を判別できず、"
           "判別のために全アカウントへ AssumeRole を試すと対象外アカウントで必ず失敗して"
           "DiscoveryAccountErrors（MM-3）が常時発報し、本物の障害が埋もれるため"],
          ["JSON をパースして accountId の配列を得る", "形式不正は巡回を中断（黙って 0 件で進めない）"],
          ["リストが空なら異常として扱い、巡回を中断してアラートする",
           "「対象が消えた」のか「パラメータを壊した」のかを区別できないため、0 件で正常終了しない"
           "（誤って全アプリが消滅検知されるのを防ぐ、対象検索-13）"],
        ],
        exceptions=[
          ["パラメータが存在しない", "ParameterNotFound",
           "巡回を中断", "MM-2 → P2 Platform", "18 §18.5.1"],
          ["JSON の形式不正", "パース例外", "巡回を中断", "MM-2 → P2 Platform", "—"],
          ["リストが空", "件数", "巡回を中断し、消滅検知（対象検索-13）を実行しない", "MM-2 → P2 Platform", "対象検索-13 の前提"],
          ["AccessDenied", "API 例外", "巡回を中断", "MM-2 → P2 Platform", "16 §16.3"],
          ["**アカウントの登録漏れ**", "—",
           "**そのアカウントのアプリは一切監視されない**（巡回対象に入らないため、アプリ単位の自動発見も働かない）",
           "月次棚卸しで検出（M-Q-17-3）", "本方式の受容したリスク。§11 未決参照"],
        ],
        idem=("読み取りのみで副作用なし。", "状態を更新しない。"),
        authnote=[
          "ssm:GetParameter（対象パラメータ 1 本に限定）。共通基盤アカウント内のため AssumeRole は不要",
          "**Organizations の委任ポリシーは不要になった**（2026-09-16）。"
          "組織の管理アカウントへの依頼（旧 W1-3 / M-Q-17-2）が外部調整から落ちる",
          "登録リストは**中央管理**。アプリ側が書き換えられる場所に置かない（勝手に監視対象から外れないようにするため）",
        ],
        logs=[
          ["アプリログ", "アカウント列挙結果", "取得件数 / 所要 ms", "担当者情報（連絡先等）は出力しない"],
        ],
        perf=(
          "巡回ごとに 1 回の GetParameter。Phase 1 は約 3 アカウント",
          "SSM Parameter Store の Standard パラメータは 4KB 上限。"
          "アカウント数が増えて超える場合は Advanced（8KB）または S3 上の台帳へ移す（M-Q-PD-38）",
        ),
        opens=[
          ["M-Q-PD-4", "【解決・2026-09-16】対象アカウントは **SSM Parameter Store の登録リスト（申請ベース）** とする。"
           "連携バケットと読み取りロールを**案件側が作成する**方針に変更したため、Organizations での全アカウント列挙では"
           "監視対象を判別できず、判別のための AssumeRole 試行は MM-3 の常時発報を招くため。"
           "**代償: アカウント単位では登録漏れが起きうる**（アプリ単位の自動発見は従来どおり）。月次棚卸しで補完する",
           "—（解決済み）", ""],
          ["M-Q-PD-38", "【解決・2026-09-18】**SSM Standard（4KB）で開始**する。Phase 1 の規模（数アカウント）では十分。数十アカウントに増えて上限に近づいたら S3 上の台帳へ移す", "—（解決済み）", ""],
          ["M-Q-17-3", "アカウント単位の登録漏れを検出する棚卸しの方法（API 提供契約 / Service Catalog の払い出し実績との突合）。"
           "**本方式では重要度が上がる**（アプリ単位の自動発見が効かない層のため）", "設計担当 + 運用", ""],
        ],
      )),
    P("対象検索-03", "対象アカウント接続権限取得", "対象アカウント接続権限取得", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        pre=[
          "各 App アカウントに DiscoveryReadRole が作成済み（**案件側が中央提供のテンプレートで作成する**。2026-09-16 変更。16 §16.2 / WBS S0・S1）",
        ...
        exceptions=[
          ...
          ["ロールが存在しない（案件側が未作成）", "AccessDenied",
           "当該アカウントをスキップし継続。**登録リストに載っているのにロールが無い = オンボーディング未完了**であり、"
           "MM-3 で気づいて案件側へ督促する", "DiscoveryAccountErrors ≥ 1 → MM-3 → P2 Platform", "16 §16.4 / vendor-guide"],
    ...
    P("対象検索-04", "認証構成情報一覧取得", "認証構成情報一覧・版数取得", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        pre=[
          ...
          "連携バケット auth-monitoring-artifacts-{accountId} が作成済み、**バージョニング有効**"
          "（**案件側が中央提供のテンプレートで作成する**。2026-09-16 変更。16 §16.4 / WBS S2）",
        ...
        exceptions=[
          ["バケットが存在しない（案件側が未作成）", "NoSuchBucket",
           "当該アカウントをスキップして継続。**登録リストに載っているのにバケットが無い = オンボーディング未完了**",
           "DiscoveryAccountErrors → MM-3 → P2", "18 §18.5.2 / vendor-guide"],
    ...
    P("対象検索-05", "前回確認状態取得", "前回確認状態取得", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-5", "【解決・2026-09-18】**Runbook 化する**。配置バケットはバージョニング有効のため、破損時は旧版へ巻き戻す手順を運用手順書（WBS W4-1）に含める", "—（解決済み）", ""],
        ],
      )),
    ...
    P("対象検索-07", "認証構成情報取得", "認証構成情報取得", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-6", "【解決・2026-09-18】**1 ファイル 5MB** を上限とする。S3 側に API 制約は無いため、Lambda のメモリと処理時間から決めた値。超過は取り込み拒否とし不備通知する", "—（解決済み）", ""],
        ],
      )),
    P("対象検索-08", "認証構成情報検証", "認証構成情報検証", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ...
          ["M-Q-PD-8", "【解決・2026-09-18】**棚卸し通知にとどめる**（enabled は自動で変えない）。monitoring.yaml からの env 削除は誤記の可能性があり、消滅検知（対象検索-13）と同じく人の確認を挟む", "—（解決済み）", ""],
        ],
      )),
    P("対象検索-09", "認証構成情報登録", "認証構成情報登録", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-9", "【解決・2026-09-18】**3 回・短いジッタ付き**で確定。競合相手は運用者の手動更新（運用-03）か多重起動した巡回で頻度は低く、3 回で収束しなければ MM-3 で気づく", "—（解決済み）", ""],
        ],
      )),
    ...
    P("対象検索-11", "認証実装確認依頼", "認証実装確認依頼", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-10", "【解決・2026-09-18】**巡回の相関 ID を payload に含める**。「どの巡回が起こした検査か」をログで追えるようにする。検査側は判定に使わずログにのみ出す（origin と同じ扱い）", "—（解決済み）", ""],
    ...
    P("対象検索-12", "構成情報不備通知", "構成情報不備通知", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-12", "【解決・2026-09-16】**解消通知は出さない**。直した側は自覚しているのが通常であり、"
           "通知を増やすとアラート全体の重みが下がるため。解消の事実は台帳の拒否フィールドがクリアされることで確認できる",
           "—（解決済み）", ""],
    ...
    P("対象検索-13", "監視対象消滅検知", "監視対象消滅・鮮度低下検知", "対象検索（1 時間毎）",
      ...
      d=dict(
        ...
        opens=[
          ...
          ["M-Q-PD-13", "【解決・2026-09-16】**閾値を超えたら自動更新せず通知のみ**。"
           "同一アカウントで **5 件以上または対象の半数以上**が消滅判定になった場合は enabled を変更せず P2 通知に留める",
           "—（解決済み）", ""],
        ],
      )),
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-2", "【解決・2026-09-14】MM-1 の閾値は **2 時間欠損 → 6 時間欠損に緩和**。"
           ...
           "設計担当（WBS B3-i と連動）", ""],
          ["M-Q-PD-14", "【解決・2026-09-18】**アプリ単位のディメンションは持たせない**。メトリクスはディメンションの組合せごとに課金され RC-7 が費用の最大費目のため。アプリ別の巡回状況はログ（CloudWatch Logs Insights）で追う", "—（解決済み）", ""],
        ],
      )),
      ...
      d=dict(
        ...
        steps=[
          ...
          ["【注意】**servers の「パス部分」（/v1 等）は完全に無視してはならない**",
           ...
           "**servers は無視し baseUrl のみを使う**（2026-09-18 確定）。アプリ側に「baseUrl はパスプレフィックス（/v1 等）まで含めて書く」ことを規約化した（ベンダー配布資料 §2.2）。全件 404 になる場合はこの記載漏れを疑う"],
        ...
        opens=[
          ["M-Q-PD-20", "【解決・2026-09-18】**数値パラメータは 1、文字列は canary-test** を既定の dummy 値とする。実在しない ID で 404 が返りやすいが、認証が効いていれば 404 より先に 401/403 が返るため検査は成立する", "—（解決済み）", ""],
          ["M-Q-PD-21", "【解決・2026-09-18】**外部 $ref は解決しない**（スキップして不備通知）。アプリ側に「**単一ファイルに bundle してからアップロードする**」ことを規約化し、ベンダー配布資料へ明記する", "—（解決済み）", ""],
          ["M-Q-PD-22", "1 アプリあたりの endpoint 件数上限（Lambda 15 分制限との兼ね合い）。"
           "W3-5 の性能実測で 1 endpoint あたりの所要時間を確定してから決める", "設計担当（W3-5 と連動）", ""],
          ["M-Q-PD-28", "【解決・2026-09-18】**servers は無視し、baseUrl のみを使う**。アプリ側に「**baseUrl はパスプレフィックス（/v1 等）まで含めて書く**」ことを規約化し、ベンダー配布資料へ明記する。全件 404 になる場合はこの記載漏れを疑う", "—（解決済み）", ""],
        ],
      )),
    P("認証実装チェック-03", "正常系確認用トークン取得", "正常系確認用トークン取得", "共通（対象検索・全量とも）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-29", "【解決・2026-09-18】**1 回まで**。無制限に再試行するとトークン発行が増え、スコープ不足の場合は永久にループするため", "—（解決済み）", ""],
          ["M-Q-PD-30", "【解決・2026-09-18】**60 秒**に下げる。AWS 既定は 300 秒だが、ローテーション直後に古い資格情報を使う窓を短くする。GetSecretValue の呼び出しは増えるが費用は $0.05/1 万回で誤差", "—（解決済み）", ""],
      ...
      d=dict(
        ...
        exceptions=[
          ...
          ["Cookie 系で 302 だが expected-redirect と不一致", "リダイレクト先",
           "**OK とする**（リダイレクト先は検証しない。2026-09-18 確定）", "通知なし", "M-Q-PD-23"],
        ...
        opens=[
          ["M-Q-PD-23", "【解決・2026-09-18】**リダイレクト先は検証しない**。302 が返れば期待どおりとして OK 判定にする。リダイレクト先まで厳密に見ると、ログイン画面の URL 変更などで誤検知が増えるため", "—（解決済み）", ""],
          ["M-Q-PD-24", "【解決・2026-09-18】**WARN（未分類）として通知する**。判定できなかったものを OK に倒さないことを原則とし、頻出する組合せは 4×4 表へ追加していく", "—（解決済み）", ""],
        ],
      )),
    ...
    P("認証実装チェック-08", "検知結果通知依頼", "検知結果通知依頼", "共通（対象検索・全量とも）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-25", "【解決・2026-09-18】**件数を送り、詳細は上位 20 件に絞る**。1 アプリで数千件が CRITICAL になる状態は「アプリ全体で認証が外れている」ことを意味し、全件列挙より件数と代表例の方が通知として有用", "—（解決済み）", ""],
    ...
    P("通知-01", "通知先解決", "通知先解決", "共通（検知時）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-26", "【解決・2026-09-18】**P2（Platform）へ寄せる**。検査側と通知側で enum がずれても通知を落とさないための保険", "—（解決済み）", ""],
    ...
    P("通知-03", "メトリクス閾値通知", "メトリクス閾値通知（保険系）", "共通（閾値超過時）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-27", "【解決・2026-09-18】**notBreaching** とする。検査が動いていないことの検知は MM-1 / MM-6 の役割であり、本アラームで二重に見る必要はない", "—（解決済み）", ""],
        ],
      )),
    ...
    P("運用-01", "監視機構稼働監視通知", "監視機構稼働監視通知（MM-1〜5）", "随時（異常時）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-33", "【解決・2026-09-16】**まとめて通知する**。MM-1（巡回停止）と MM-6（全量確認停止）が"
           "同時に発報している場合は、個別の 2 通ではなく『**監視機構の全面停止**』として 1 通で通知する。"
           "2 つが同時に止まっている状態は個別の障害とは重大度が異なり、それが件名で伝わるようにするため",
           "—（解決済み）", ""],
          ["M-Q-PD-34", "【解決・2026-09-18】**IaC のドリフト検出で担保する**。アラーム自体が削除・無効化されたことに気づけるようにする（W1-6 の Config Rules と併せて整備）", "—（解決済み）", ""],
        ],
      )),
    P("運用-02", "処理失敗退避・再処理", "処理失敗退避・再処理（DLQ）", "随時（異常時）",
      ...
      d=dict(
        ...
        opens=[
          ["M-Q-PD-35", "【解決・2026-09-18】**14 日**（最大値）とする。休日や連休を挟んでも再処理できるようにするため。既定は 4 日", "—（解決済み）", ""],
          ["M-Q-PD-36", "【解決・2026-09-18】**追加する**。`DestinationDeliveryFailures` を監視し、**失敗の記録すら残らない状態**を検知する。これが無いと最後の砦が抜ける", "—（解決済み）", ""],
        ],
      )),
    P("運用-03", "監視設定手動更新", "監視設定手動更新", "随時（運用操作）",
      ...
      d=dict(
        ...
        opens=[
          ...
          ["M-Q-PD-37", "【解決・2026-09-16】**理由の記録を必須とし、承認フローは求めない**。"
           "台帳に停止理由と実施者・実施日時を残すことを必須とし、月次棚卸しで「止めたままのアプリ」を確認する。"
           "承認を必須にすると障害対応中の緊急停止ができなくなるため", "—（解決済み）", ""],
        ],
      )),
