...
> 最終更新: 2026-05-13（Phase 9 反映 / Cognito 2024-11 仕様変更反映 / 実装実態整合）
...
| 8 | クレームマッピング・認可（Cognito） | ✅ 完了 | Pre Token Lambda V2、ロール階層、テナントスコーピング |
| 9 | Keycloak 対応 + VPC Lambda + 完全プライベート JWKS | ✅ 完了 | Protocol Mapper で同一クレーム発行、VPC Lambda Authorizer、Internal ALB、Cognito VPCE |

**総合達成率: 9/9 Phase 完了（Phase 5 の Route 53 自動フェイルオーバーのみ未検証）**
...
| | tenant_id 属性のトークン注入（Cognito） | ✅ | Phase 8 |
| | tenant_id 属性のトークン注入（Keycloak） | ✅ | Phase 9 |
| | roles 属性のトークン注入（Cognito） | ✅ | Phase 8 |
| | roles 属性のトークン注入（Keycloak） | ✅ | Phase 9 |
| | ロール階層（employee < manager < admin） | ✅ | Phase 8 |
| | テナント分離（cross-tenant 拒否） | ✅ | Phase 8 / 9 |
| | Keycloak Protocol Mapper でのクレーム変換 | ✅ | Phase 9（User Attribute / Realm Role / User Property Mapper） |
| | VPC Lambda Authorizer + Internal ALB JWKS | ✅ | Phase 9（ADR-012） |
...
| WebAuthn / FIDO2（Passkeys） | ⚠ 未検証だが対応（Essentials+ ティア、2024-11〜） | ❌ 未検証（対応可） | **Cognito も 2024-11 から WebAuthn/Passkeys 対応**。Lite ティア不可、`ALLOW_USER_AUTH` フロー限定 |
...
| MFA | △ 同等（Cognito も WebAuthn 対応） | ◯（柔軟性 / 二重 MFA 回避 UX） | Cognito も 2024-11〜 Passkeys 対応（Essentials+ ティア）。条件付き OTP は Keycloak がシンプル |
...
| [006](../adr/006-cognito-vs-keycloak-cost-breakeven.md) | Cognito vs Keycloak コスト損益分岐点の分析 | **Proposed** | 2026-03-17 | 損益分岐 175,000 MAU（連携のみ）/ 75,000 MAU（Plus ティア利用時）、MAU 規模次第で選定 |
| [007](../adr/007-osaka-auth0-idp-limitation.md) | 大阪リージョンで Auth0 OIDC IdP 接続不可の記録 | Accepted | 2026-03-18 | `.well-known` 自動検出不可 → 手動 endpoint 指定で workaround 済（dr-osaka/cognito.tf） |
...
| [010](../adr/010-keycloak-private-subnet-vpc-endpoints.md) | Keycloak を Private Subnet + VPC Endpoint 構成へ移行 | Accepted | 2026-04-21 | カスタム VPC、Public IP なし、NAT Gateway 不要 |
| [011](../adr/011-auth-frontend-network-design.md) | 認証基盤前段ネットワーク設計（HTTPS / カスタムドメイン / WAF / CloudFront）統合判断 | Proposed | 2026-04-21 | 要件定義 Phase C で Accepted 化予定 |
| [012](../adr/012-vpc-lambda-authorizer-internal-jwks.md) | VPC Lambda Authorizer + Internal ALB による JWKS プライベート化 | Accepted | 2026-04-23 | Phase 9 で実装、/v1（非 VPC）/v2（VPC）並列稼働確認 |
...
#### 【High】PoC 実装上の欠落・運用リスク

| 項目 | 現状 | 影響 |
|------|------|------|
| **DR Cognito の Pre Token Lambda 未設定** | [infra/dr-osaka/cognito.tf](../../infra/dr-osaka/cognito.tf) に `lambda_config` なし | DR フェイルオーバー時に `tenant_id` / `roles` クレームが Access Token に注入されない → 認可ロジック動作不可。**Phase 8/9 の認可シナリオが DR では機能しない** |
| **Keycloak realm-export.json が Phase 7-9 設定を含まない** | [keycloak/config/realm-export.json](../../keycloak/config/realm-export.json) に TOTP MFA / Identity Brokering / Conditional OTP Flow / Phase 9 Protocol Mapper が**未反映**（Admin Console で動的設定したまま）| `import-realm` で再構築すると **Phase 7-9 の検証構成が消える**。設定の単一ソース化が運用要件として未整備（FR-INT-010 / NFR-OPS-007 の補強要） |
