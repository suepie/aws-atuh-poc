...
## 6. マスター表 B（顧客 IdP）の列コード: X / Y / Z / W / V

> [02-idp-federation.md マスター表 B](hearing-script/02-idp-federation.md) の列。
...
| **Z** | 接続経由（AD 利用時）| ① / ② / ③ / ④ | 直結 / AD FS 経由 / Entra Connect 経由 / **AD 直結（Keycloak 必須化）**|
| **W** | SCIM 対応 | ✅ / ⚠ / ❌ / ❓ | 標準対応 / 上位ライセンス / 未対応 / 不明 |
| **V** | テナント分離希望 | L1 / L2 / L3 | 完全集約 / 論理分離（標準）/ 物理分離（追加コスト） |

> **使用箇所**: [B-200-B マスター表 B](hearing-script/02-idp-federation.md)
...
### 10c. テナント分離レベル: L1 / L2 / L3（マスター表 B 列 V）

| コード | 意味 |
|:---:|---|
| **L1** | 完全集約（顧客 IdP なし、共通基盤ローカルのみ）|
| **L2** | 論理分離（**標準**、単一 Pool/Realm + tenant_id クレーム）|
| **L3** | 物理分離（規制業種向け、Pool/Realm per テナント、追加コスト）|
