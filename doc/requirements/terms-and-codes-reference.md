...
| **P-4** | IdP を持たない顧客のユーザー（**ローカル**）+ ゲスト/外部協力者（旧 P-5 統合）| 本基盤内ローカル + 招待 URL | 中小顧客で IdP 未導入、取引先・外部協力者など |

> **2026-06-24 体系修正**: 旧 P-5 ゲスト/外部協力者 → P-4 統合 / 旧 P-6 B2C → **本基盤対象外**（B2B SaaS 専用）。**Break Glass 用ローカル管理者**は P-1 内の役割として扱う（独立カテゴリではない）。
...
|:---:|---|---|---|---|
| **α** | 全カテゴリ受入 | P-1〜P-4 全て | **最大** | 中小 B2B |
| **β** | 管理者 + IdP なし顧客 | P-1 / P-2 / P-4 | 中 | IT 成熟度がまちまちな B2B |
| **γ** | **管理者層のみ**（**業界推奨**）| P-1 / P-2 | 小 | **エンタープライズ B2B SaaS** |
| **δ** | Break Glass のみ | P-1 のみ | 最小（数名）| 規制業種専用 SaaS（IdP 強制）|
