...
| NFR-MIG-002 | パスワード移行（ハッシュ持ち越し） | **対象外（2026-08-17、NFR-MIG-001 に連動）**。⚠ **副作用: 旧システムのローカルパスワード利用者はパスワードを引き継げず、全員に再設定が必要**（下記 §注記）| — | Custom PasswordHashProvider SPI（DU-U10-11）は不要に | ⬜ |
...
> **⚠ 注記（2026-08-17）— 移行をアプリ側に委ねたことの副作用**
>
> 基本設計 [U10 §10.4](../basic-design/10-integration-migration-design.md) は移行対象を 4 集団に分け、そのうち **集団 B（旧システムのローカルパスワード利用者）には User Storage SPI 並走**を割り当てていた。これは**初回ログイン時に裏で旧システムへ問い合わせて認証し、その場で新方式へ入れ直す**仕組みで、**利用者にパスワード再設定を求めずに移行できる**ことが最大の価値だった（[ADR-019](../adr/019-existing-system-migration.md)）。
>
> 移行をアプリ側に委ねると、アプリは SCIM や管理 API で利用者を登録することになるが、**これらの経路ではパスワードを持ち込めない**（ハッシュを転送する標準的な手段がない）。したがって:
>
> | 集団 | 影響 |
> |---|---|
> | A: 顧客 IdP を持つ利用者 | **影響なし**（認証は顧客 IdP 側。パスワードを持たない） |
> | **B: ローカルパスワード利用者**（IdP-KC 収容） | **全員パスワード再設定が必要** |
> | C / D: 属性のみ・人事 DB 起点 | 影響なし（もともとパスワードなしで作成） |
>
> **さらに重い制約**: 本基盤は「**業務用メールアドレスを持たない利用者**」を正式な前提としており、メールによる再設定リンクの送付を採用していない（[D-U4-11](../basic-design/04-auth-ux-design.md) / D-U7-14）。よって集団 B の一斉再設定には**メール以外の配布経路**（管理者による対面・別経路での配布、または予備コードの事前配布）が必須になる。
>
> **確認したいこと**: 集団 B に該当する利用者がどの程度いるか（＝顧客 IdP を持たないテナントの利用者数）。規模が大きい場合、切替時の問い合わせ集中と配布作業がアプリチーム側の負担になる。→ ヒアリング **B-MIG 系**で確認する。
