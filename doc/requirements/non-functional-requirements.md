...
| ID | 要件 | 推奨値 | Cognito | Keycloak | 状態 |
|----|------|------|--------|----------|:---:|
| NFR-MIG-001 | 既存認証システムからのユーザー移行 | **対象外（2026-08-17 ユーザー判断 — 移行はアプリ側で実施）**。本基盤は**受け入れ口（SCIM 受信 / idm-api CRUD / JIT）を提供するのみ**で、旧システムからのデータ吸い上げは行わない | — | 受け入れ口は既存スコープ内 | ⬜ |
| NFR-MIG-002 | パスワード移行（ハッシュ持ち越し） | **方向性確定（条件付き）— 引き継ぎは可能**（2026-08-17 訂正）。アプリが **Admin REST API**（SCIM 不可）で投入し、旧ハッシュが **Keycloak ネイティブ対応**（PBKDF2-SHA256/512・Argon2id）なら再設定不要。**bcrypt 等なら Custom PasswordHashProvider SPI = 基盤側の作り込みが必要** | — | DU-U10-11（8 人日）は **条件付きで復活**（B-MIG-1 の回答次第） | 🔴 |
| NFR-MIG-003 | ベンダーロックイン回避 | (D) **OIDC 標準準拠** | ⚠ AWS 依存 | ✅ OSS / 移行可能 | ✅ |
| NFR-MIG-004 | データエクスポート | (D) **必要時に可能** | ✅ ListUsers + S3 Export | ✅ Realm Export | 🟡 |
| NFR-MIG-005 | 段階的移行（並行稼働） | (D) **可能 — 維持**（新旧システムの並行稼働とテナント単位の順次切替は基盤の性質として継続。**移行作業そのものはアプリ側**）| ✅ | ✅ | ✅ |

> **⚠ 注記（2026-08-17、同日訂正）— 移行をアプリ側に委ねた場合のパスワードの扱い**
>
> **結論: アプリ側が移行しても、パスワードは変えずに引き継げる。ただし条件が 2 つある。**
>
> | 投入経路 | ハッシュを運べるか |
> |---|---|
> | **SCIM 受信** | **❌ 運べない** — SCIM 2.0 の `password` 属性は平文のみで、ハッシュを転送する標準がない |
> | **Keycloak Admin REST API**（= idm-api 経由） | **✅ 運べる** — `credentials[].credentialData`（`algorithm` / `hashIterations`）+ `secretData`（値 + ソルト）で投入できる |
>
> **条件 ①: アプリは SCIM ではなく Admin REST API（idm-api）を使うこと。**
> **条件 ②: 旧システムのハッシュ方式が Keycloak のネイティブ対応であること**（PBKDF2-SHA256 / PBKDF2-SHA512 / Argon2id）。**bcrypt や独自形式の場合は Custom PasswordHashProvider SPI（[DU-U10-11](../basic-design/00b-design-unit-breakdown.md)、8 人日）が必要**で、これは**基盤側の作り込み**になる。→ 現行方式は **B-MIG-1** で確認する。
>
> | 集団 | 条件 ①② を満たす場合 | 満たさない場合 |
> |---|---|---|
> | A: 顧客 IdP を持つ利用者 | 影響なし（パスワードを持たない） | 影響なし |
> | **B: ローカルパスワード利用者**（IdP-KC 収容） | **再設定不要** | **全員パスワード再設定が必要** |
> | C / D: 属性のみ・人事 DB 起点 | 影響なし | 影響なし |
>
> **再設定が必要になった場合の配布が重い**: 本基盤は「業務用メールアドレスを持たない利用者」を正式な前提とし、メールによる再設定リンク送付を採用していない（[D-U4-11](../basic-design/04-auth-ux-design.md) / D-U7-14）。よって**メール以外の配布経路**（管理者による対面・別経路での配布、または予備コードの事前配布）を設計する必要がある。→ **B-MIG-10** / 質問状 **Q68** で確認する。
>
> **付随論点**: 旧ハッシュをそのまま持ち込むと旧システムの強度パラメータ（反復回数など）を引き継ぐことになる。Keycloak が次回ログイン時に現行方式へ再ハッシュするかは**実機確認が必要**（A-2 G-SPI-Compat に相乗り）。また**ハッシュがアプリチームの手を経由する**ため、取り扱い手順（一時保管の禁止・経路の暗号化・作業後の破棄）を移行手順書に明記すること。
