...
| **② 高権限ロール** | テナント管理者 / 基盤運用者（P-1, P-2）等 | **ステップアップ MFA**（必要時のみ追加）| 高権限操作（ロール付与 / IdP 設定変更 等）は追加保証が必要 |
...
|:-:|---|---|---|:-:|
| **全面信頼** | 顧客 IdP の MFA をそのまま信頼 | `amr=mfa` があれば追加 MFA 不要 | 一般的な B2B SaaS | ⭐ **業界標準** |
| **部分信頼** | 高権限ロールのみ追加検証 | ロール別にステップアップ MFA | 規制業種 + 高権限混在 | ◎ |
...
| 6 | **B-508** | 高権限ロールへの追加 MFA 強制 | する / しない / ロール別 | 🟡 |
