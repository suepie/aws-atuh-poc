...
|---|---|---|---|
| **① ローカルユーザー** | P-4（現行で IdP がなかった従業員）/ P-5（Break Glass）/ P-6（B2C）| 基盤側で**必須** | 顧客 IdP が存在しないため、基盤側で MFA する以外に手段なし |
...
- ゲスト/外部協力者（取引先・外部協力者など、招待 URL 経由）
- Break Glass 用ローカル管理者（弊社運用者の緊急アクセス）
- → これらは**基盤側で MFA を提供する以外の選択肢がない**（B2C は本基盤対象外）
