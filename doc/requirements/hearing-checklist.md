...
| A-11 | 🟢 | ブランディング要件（全体方針） | FR-ADMIN-012 | §FR-8.3, §FR-2.3.3, §FR-2.3.3.A | ログイン UI のカスタマイズ要否?（**画面所在で対応範囲が決まる: ①アプリ画面のみ（パターン A 推奨、認証基盤設定不要）/ ②認証基盤画面にロゴ等（パターン B、Cognito 20 顧客上限）/ ③完全専用（パターン C、Enterprise）**）。詳細は B-612 / §FR-2.3.3.A | パターン A / B / C | | ⏳ |
...
| **B-605-3** | 🔥 | **退職反映 SLA** | FR-FED-009 / FR-USER / NFR-SEC | §FR-2.2.2, §FR-2.2.4, §FR-5 | IdP 側で退職処理後、本基盤・各アプリで認可拒否されるまでの SLA?（即時 / 数分 / 数時間 / 翌日 / SCIM 同期依存）。**「Import」で退職反映遅延は重大インシデント源** | 時間 | | ⏳ |
...
| B-612 | 🟢 | **ログイン画面のブランディング詳細**（画面所在で対応範囲が決まる）| FR-ADMIN-012 | §FR-2.3.3, §FR-2.3.3.A, §FR-8.3, §FR-8.3.A | A-11 を詳細化。**カスタマイズ対象画面はどこか?**: ①アプリ側画面のみ（ランディング / ダッシュボード / ログアウト後）= 認証基盤側設定不要 / ②認証基盤側画面（ログイン / MFA / PW リセット）も含む = Cognito Branding Style 20 顧客上限 / Keycloak Theme 開発要 | 画面範囲 + 共通/顧客別 | | ⏳ |
...
| **B-703-1** | 🟡 | **許可リダイレクト URL の事前登録**（セキュリティ）| FR-SSO-003 / NFR-SEC | §FR-5.1 | 各アプリで許可するリダイレクト URL のリストは確定しているか?（Cognito **100 URL/App Client Hard limit**、Keycloak 制限なし）。**未確定だと App Client / Client 設定不能** | URL リスト | | ⏳ |
...
| **B-703-3** | 🟢 | **テナント別カスタムランディング要否**（ブランディング）| FR-SSO-003 / FR-FED-010 | §FR-5.1, §FR-2.3, §FR-2.3.3.A | 顧客企業ごとに別のログアウト完了画面を出すか?（Acme と Globex で別ページ / ロゴ表示 等）。**ログアウト後はアプリ側画面なので、共通 URL + `?tenant=acme` クエリでアプリ側 JS が動的差替**で対応可（パターン A）。URL 肥大化を回避 | あり（パターン A 動的差替 / B 顧客別 URL）/ なし / 共通画面 | | ⏳ |
