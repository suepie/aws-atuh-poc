...
| **A-11** | 🔥 | **ブランディング基本方針合意**（パターン A/B/C） | FR-ADMIN-012 | §FR-8.3, §FR-2.3.3, §FR-2.3.3.A | 業界標準である **パターン A（認証基盤は最小ブランディング、アプリ側で `tenant_id` クレームを解釈して動的差替）** を採用することに合意いただけるか?（Slack / Notion / Microsoft 365 / Auth0 採用）。**合意なら B-612 / B-703-3 / B-208 / B-703-1 が自動決定**。別パターン B（認証基盤画面にも顧客ロゴ、Cognito 20 顧客上限）/ C（完全専用デザイン、Enterprise）の場合は下位項目で深掘り | パターン A 合意 / B / C | | ⏳ |
| **A-11-2** | 🟡 | **アプリ側の動的差替実装責務** | FR-ADMIN-012 | §FR-2.3.3.A | A-11「パターン A」採用時、各アプリチームが以下の責務を取れるか?（①JWT `tenant_id` クレームから顧客識別 / ②ロゴ・配色・文言の動的差替 / ③`tenant_id` 改竄検証 / ④共通 URL + クエリ受け取りの実装）| 取れる / 一部支援要 / 取れない（B/C 検討） | | ⏳ |
...
| B-208 | 🟡 | **Custom Domain** | FR-FED-014 | §FR-2.1, §FR-2.3.3.A | **※ A-11「パターン A」採用時の標準動作: 共通 1 Custom Domain（`auth.example.com`）で全顧客カバー、Cognito 4 Custom Domain Hard Limit 問題を回避**。認証エンドポイント URL に顧客指定ドメインを使うか?（共通 1 つ推奨 / なし=プラットフォーム標準 / 顧客別=A-11 で B/C 検討要）| 共通 1 つ / なし / 顧客別 | | ⏳ |
