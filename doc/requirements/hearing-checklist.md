...
| C-204 | 🟡 | パスワードポリシー | FR-AUTH-009 | 顧客固有要件?（最小長・複雑性）| 仕様 | | ⏳ |
| C-204-2 | 🟡 | **パスワード履歴** | FR-AUTH-010 | 過去 N 個と一致禁止が必須か? Cognito は Essentials+ で対応（ADR-016）| Yes/No + N 個 | | ⏳ |
| C-205 | 🟡 | **アカウントロック詳細設定** | FR-AUTH-011 | 自動 BF 保護で十分か / 連続失敗回数・ロック時間の**設定可能性**が必須か? 後者なら Cognito Plus ティア必要 | 自動で可 / 設定 N 回 N 分 | | ⏳ |
| C-205-2 | 🟡 | **侵害クレデンシャル検出** | NFR-SEC-010-2 | 漏洩パスワードの自動検知・拒否が必要か?（NIST SP 800-63B 推奨）Cognito Plus ティア機能 | Yes/No | | ⏳ |
