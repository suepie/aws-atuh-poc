...
**プラットフォーム選定への影響度が高い項目（🔥 最優先 39 件）を Stage 1 前半で先行確認**することで、ADR-014 / ADR-015 / ADR-016 / ADR-017 を早期確定できる。
...
| **B-102-2** | 🟡 | **バックエンド API のエンドポイント形態（JWT 検証場所の決定軸）** | FR-AUTH / FR-AUTHZ | §FR-1.1, §FR-6 | バックエンド API への通信経路は? **JWT 検証の場所**が Lambda Authorizer か / アプリ側か / Sidecar か は本問で決まる: ①**AWS API Gateway 経由**（Bearer JWT → **Lambda Authorizer** で検証、本基盤 PoC 標準）/ ②**ALB + ECS / EC2 直結**（アプリ側ライブラリで JWT 検証、Lambda Authorizer 不要）/ ③**Service Mesh**（Envoy / Istio Sidecar で JWT 検証）/ ④**完全内部完結**（SSR + 内部 API + DB が VPC 内で完結、外部 API 公開なし）/ ⑤**Cognito Authorizer**（API Gateway 標準、単一 Pool シンプル検証のみ）。**SSR / SPA / モバイルの選択とは独立した軸**で、本基盤のマルチイシュア要件では Lambda Authorizer が事実上必須 | 経路リスト + アプリ別マッピング | | ⏳ |
