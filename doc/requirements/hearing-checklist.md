...
| A. 事業要件 | 19 | 10 | 7 | 2 | ⏳ |
| B. 技術要件 | 48 | 5 | 27 | 16 | ⏳ |
| C. 運用・セキュリティ要件 | 36 | 11 | 19 | 6 | ⏳ |
| D. 最終判断 | 6 | 6 | 0 | 0 | ⏳ |
| **合計** | **109** | **32** | **53** | **24** | — |

> **マスター表 C 統合による調整**（2026-05-24）: 旧 B-101〜B-109 + B-202 + B-303 + B-504 + B-704 + C-204-5 の 15 項目を **B-100 マスター表 C（アプリ・システム構成リスト）に統合**し、見かけ上の項目数は減少。**実質の確認内容は表 C で網羅**しており、Knockout 条件（K1〜K8）の可視性が向上。

**プラットフォーム選定への影響度が高い項目（🔥 最優先 32 件）を Stage 1 前半で先行確認**することで、ADR-014 / ADR-015 / ADR-016 / ADR-017 を早期確定できる。

**ヒアリングの最上位順序**:
1. **A-5-2（受け入れ利用者カテゴリ P-1〜P-6）+ A-5-3（採用シナリオ α/β/γ/δ）+ A-5-4（インフラ運用者カテゴリ I-1〜I-5）**: 後続の A-6 / B-200 系 / C-204 系 / §NFR-6.4 の質問範囲を規定する最上位判断
2. **B-200 マスター表 A/B（弊社 / 顧客 IdP）+ B-100 マスター表 C（アプリ・システム構成リスト）**: この 3 表が埋まれば「顧客 IdP × アプリ」マトリクスが完結し、Cognito vs Keycloak 選定が事実上確定
3. **A-11（軸 1: アプリ別ブランディング）+ A-11-α（軸 2: 顧客別ブランディング）**: 両軸の Yes/No 組合せで B-612 / B-703-3 / B-208 / B-703-1 の 4 項目が自動決定（パターン A / A' / B / C のマッピングは §FR-2.3.3.A）
...
| **A-5-4** | 🔥 | **インフラ運用者カテゴリ確認（I-1〜I-5）** | NFR-OPS | §FR-1.2.0.0, §NFR-6.4 | 本基盤の**インフラを操作する人**を確認（**本基盤の認証経由ではなく AWS IAM 等で認証**）。複数該当を全選択: **I-1** AWS インフラ運用者（Cognito / Lambda / VPC 操作） / **I-2** Keycloak / RHBK 運用者（EKS / OpenShift） / **I-3** 監視・SRE 担当（CloudWatch / Datadog / Grafana） / **I-4** セキュリティ監査者（CloudTrail / SIEM Read-Only） / **I-5** ベンダー / SI サポート（Red Hat / AWS Support / 外部 SI、一時付与）。各カテゴリの認証経路・MFA 要件・最小権限ポリシー設計に直結 | 該当カテゴリ複数選択 + 各人数 | | ⏳ |
...
### B-1. 認証フロー / Grant Type + アプリ・システム構成（→ FR-AUTH §1.1 / proposal §FR-1.1）

> **構成変更**: 旧 B-101 / B-102 / B-102-2 / B-103 / B-104 / B-105 / B-106 / B-107 / B-108 / B-109 と他章の B-202（SAML IdP 発行）/ B-303（UMA）/ B-504（Back-Channel Logout）/ B-704（Access Token Revocation）/ C-204-5（既存ローカル認証）を **B-100 マスター表 C（アプリ・システム構成リスト）に統合**しました。マスター表 B（顧客 IdP）と並列構造で、「顧客 IdP × アプリ」マトリクスが完結します。
>
> 統合された旧 ID は本表で `→ B-100 マスター表 C` と表記、引き続き ID 参照は可能です。

| # | 優先度 | 項目 | 関連 FR | proposal § | 質問内容 | 期待回答 | 回答 | 状態 |
|---|:----:|------|--------|---------|---------|---------|------|:---:|
| **B-100** | 🔥 | **マスター表 C: 御社アプリ・システム構成リスト**（旧 B-101〜B-109 + B-202 + B-303 + B-504 + B-704 + C-204-5 統合）| FR-AUTH 全般 / FR-AUTHZ / FR-FED / NFR-MIG | §FR-1.1, §FR-1.2, §FR-6, §C-1.3 | アプリ・システムごとに 1 行: **システム名 / 業務カテゴリ / クライアント種別（列 P）/ SPA 認証方式（列 Q）/ Backend API 経路・JWT 検証場所（列 R）/ 特殊要件フラグ（列 S、☑ 複数可）/ 既存ローカル認証（列 T）/ 補足**。詳細選択肢・記入テンプレートは [hearing-script/01-auth-flow.md マスター表 C](hearing-script/01-auth-flow.md#マスター表-c-御社アプリシステム構成リスト) を参照。**列 S（K1〜K8）に 1 件でも☑あれば Keycloak 必須化確定** | 表 C N 行（システム数分）| | ⏳ |
| ~~B-101~~ | — | ~~SPA Auth Code + PKCE~~ | — | — | → **B-100 マスター表 C 列 P = a**（SPA）で確認 | 表で回答 | — | 統合 |
| ~~B-102~~ | — | ~~SSR Web App~~ | — | — | → **B-100 マスター表 C 列 P = b**（SSR）で確認 | 表で回答 | — | 統合 |
| ~~B-102-2~~ | — | ~~バックエンド API のエンドポイント形態（JWT 検証場所）~~ | — | — | → **B-100 マスター表 C 列 R**（①〜⑤）で確認 | 表で回答 | — | 統合 |
| ~~B-103~~ | — | ~~M2M / バッチ~~ | — | — | → **B-100 マスター表 C 列 P = d**（M2M バッチ）で確認 | 表で回答 | — | 統合 |
| ~~B-104~~ | — | ~~Token Exchange~~ | — | — | → **B-100 マスター表 C 列 S K1**（☑ で Keycloak 必須化）で確認 | 表で回答 | — | 統合 |
| ~~B-105~~ | — | ~~Device Code~~ | — | — | → **B-100 マスター表 C 列 P = e** + **列 S K2**（☑）で確認 | 表で回答 | — | 統合 |
| ~~B-106~~ | — | ~~mTLS~~ | — | — | → **B-100 マスター表 C 列 S K3**（☑ で Keycloak 必須化）で確認 | 表で回答 | — | 統合 |
| ~~B-107~~ | — | ~~ネイティブモバイル~~ | — | — | → **B-100 マスター表 C 列 P = c**（モバイル）で確認 | 表で回答 | — | 統合 |
| ~~B-108~~ | — | ~~SPA 認証方式（BFF vs PKCE）~~ | — | — | → **B-100 マスター表 C 列 Q**（α / β / γ）で確認 | 表で回答 | — | 統合 |
| ~~B-109~~ | — | ~~DPoP~~ | — | — | → **B-100 マスター表 C 列 S K4**（☑ で Keycloak 必須化）で確認 | 表で回答 | — | 統合 |
...
| ~~B-202~~ | — | ~~SAML IdP として発行~~ | — | — | → **B-100 マスター表 C 列 P = g** または **列 S K5**（☑ で Keycloak 必須化）で確認 | 表で回答 | — | 統合 |
...
| ~~B-303~~ | — | ~~細粒度認可（UMA）~~ | — | — | → **B-100 マスター表 C 列 S K6**（☑ で Keycloak Authorization Services 必須）で確認 | 表で回答 | — | 統合 |
...
| ~~B-504~~ | — | ~~Back-Channel Logout~~ | — | — | → **B-100 マスター表 C 列 S K7**（☑ で Keycloak 必須化）で確認 | 表で回答 | — | 統合 |
...
| ~~B-704~~ | — | ~~Access Token Revocation 要否~~ | — | — | → **B-100 マスター表 C 列 S K8**（☑ で Cognito 自前実装 or Keycloak Introspection）で確認 | 表で回答 | — | 統合 |
...
| ~~C-204-5~~ | — | ~~既存システムで独自ローカル認証を持つアプリの有無~~ | — | — | → **B-100 マスター表 C 列 T**（N / M1 / M2 / M3 / M4）で確認 | 表で回答 | — | 統合 |
...
|---|:---:|---|:---:|
| §FR-1.1 認証フロー / Grant Type | ✅ 記載済 | B-101〜B-108, B-102-2, B-304 | 10 |
| §FR-1.2 パスワード・ローカルユーザー管理 | ✅ 記載済 | A-9, C-204, C-204-2〜4, C-205, C-205-2 | 7 |
| §FR-1.2.0 ローカルユーザー認証の主体 | ✅ 記載済 | A-5-3, C-204-5 | 2 |
| §FR-1.2.0.0 利用者カテゴリと採用シナリオ | ✅ 記載済 | A-5-2, A-5-3, A-5-4 | 3 |
