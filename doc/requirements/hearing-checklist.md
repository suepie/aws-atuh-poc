...
| **B-SCIM-7** | `[B]` | 🔴 | **Phase Two SCIM Custom Attribute Mapping 検証**（2026-07-09 追加、V1、Phase 1 実装開始前ゲート）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V1](../common/jit-scim-coexistence-keycloak.md) | Phase Two SCIM Plugin で **`scim_active` / `provisioned_by` を user_attribute に書き込めるか** の PoC 検証: POST /Users → `scim_active=true` 反映 + PUT /Users {active: false} → `scim_active=false` 更新確認。**JIT/SCIM 判別ロジックの前提**、書けないと §10.4.B の削除禁止フラグが機能しない | 検証結果 (可 / 不可 / 一部制約) + Phase Two バージョン + 設定例 | | ⏳ |
| **B-SCIM-8** | `[B]` | 🔴 | **Sync Mode = FORCE 時の scim_active 保護検証**（2026-07-09 追加、V2、Phase 1 実装開始前ゲート）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V2](../common/jit-scim-coexistence-keycloak.md) | Identity Provider Mapper で **`syncMode=IMPORT` を Mapper 単位で override**（Realm デフォルト FORCE でも特定属性のみ IMPORT で保護）が **`scim_active` / `provisioned_by` に対して確実に動作するか** の PoC 検証: SCIM 事前作成 → JIT ログイン後の属性値保持を確認。**動作しない場合、Sync Mode = FORCE を採用できず SCIM Push 反映が遅延する** | 検証結果 (動作 / 動作せず / 部分動作) + 代替案 | | ⏳ |
| **B-SCIM-9** | `[B]` | 🟡 | **Event Listener SPI debounce 負荷試験**（2026-07-09 追加、V3、Phase 1 実装中）| NFR-PERF / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V3](../common/jit-scim-coexistence-keycloak.md) | debounce（1 日 1 回のみ user_attribute 更新）の実装で **性能問題ないか** の負荷試験: 100 RPS × 24h × 10 万ユーザ、Aurora 書き込み IOPS + Infinispan cache invalidate 遅延 + Keycloak Pod CPU/メモリ監視。**閾値超過時は debounce 期間延長 or Async 化検討** | 負荷試験結果 (IOPS / cache 遅延 / CPU) + 閾値評価 | | ⏳ |

> **B-SCIM-7〜9 追加注記**（2026-07-09）: [jit-scim §10.4](../common/jit-scim-coexistence-keycloak.md) 破綻を修正した Event Listener SPI + user_attribute.last_login 版（§10.4.A）+ JIT/SCIM 判別ロジック（§10.4.B）の **Phase 1 実装前 / 実装中の検証ゲート項目**。V1/V2 は Phase 1 実装開始前に PoC 必須、V3 は Phase 1 実装中の負荷試験必須。[ADR-060 §C.2.3](../adr/060-auth-protocol-attack-path-residual-tbd.md) の SPI 拡張の前提となる。
