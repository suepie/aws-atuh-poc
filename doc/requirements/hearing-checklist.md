...
| **B-622** | `[B]` | 🔴 | **Launchpad SPA 構築方針**（2026-06-12 追加）| FR-SSO-008 | §FR-4.3.1 | 認証後の **entitled apps 一覧画面（Launchpad）** をどう構築するか?（A 別 SPA 構築＝業界標準 / B Keycloak Account Console 流用 / C 不要・直リンクのみ）。Microsoft 365 portal / Okta Dashboard / Atlassian Cloud start が業界実例 | A / B / C + 想定アプリ数 | | ⏳ |
| **B-623** | `[B]` | 🔴 | **Sorry ページ実装パターン**（2026-06-12 追加）| FR-SSO-008 | §FR-4.3.2 | 直リンクでアプリ X にアクセス → 認証成功 → 権限なし時の Sorry ページ実装は?（A アプリ側 / B 認証基盤 Theme / **C 共通 Sorry SPA＝推奨**）| A / B / C | | ⏳ |
| **B-624** | `[B]` | 🟡 | **Launchpad のテナント別ブランディング**（2026-06-12 追加）| FR-SSO-008 / NFR-OPS | §FR-4.3.1 | Launchpad のテナント別 theme / ロゴ / 配色は必要か? | 必要（テナント別）/ 不要（共通 UI）| | ⏳ |
| **B-625** | `[B]` | 🟡 | **認証後のデフォルト着地点**（2026-06-12 追加）| FR-SSO-008 | §FR-4.3.3 | 認証完了時、`redirect_uri` を優先（直リンク尊重）か Launchpad に強制遷移か? | redirect_uri 優先（推奨）/ Launchpad 強制 / 顧客選択 | | ⏳ |
...
| **B-627** | `[B]` | 🟡 | **Launchpad 表示アプリの判定根拠**（2026-06-12 追加）| FR-SSO-008 / FR-AUTHZ | §FR-4.3.1 | Launchpad で「どのアプリを表示するか」の判定根拠は?（JWT roles / Keycloak Client ロール / 顧客別マッピング表 / 全部組合せ）| 根拠 + 設計責務 | | ⏳ |
...
| **B-TAP-1** | `[B]` | 🔴 | **Tenant Admin Portal の必要性**（2026-06-18 追加）| FR-USER / FR-ADMIN | §FR-8.5, ADR-038 | 顧客テナント管理者向け Admin UI 必須?（必須 / 不要＝弊社運用代行）| 必要性 | | ⏳ |
...
| **B-TAP-5** | `[B]` | 🟢 | **Launchpad 統合**（2026-06-18 追加）| FR-SSO-008 / FR-ADMIN | §FR-8.5, ADR-038, ADR-021 | Launchpad に管理画面タイル配置?（統合 / 独立 URL のみ）| 統合方針 | | ⏳ |
...
| **B-621** | `[B]` | 🟡 | **C 案 URL 命名方式**（2026-06-12 追加）| FR-FED-013 / NFR-OPS | §FR-2.3.3.E | 組織固有 URL を採用する場合の命名方式?（サブドメイン `acme.basis.example.com` / パスベース `basis.example.com/t/acme` / 採用しない）| 方式 + 想定顧客数 | | ⏳ |
