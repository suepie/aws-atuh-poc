...
**プラットフォーム選定への影響度が高い項目（🔥 最優先 36 件）を Stage 1 前半で先行確認**することで、ADR-014 / ADR-015 / ADR-016 / ADR-017 を早期確定できる。**特に A-11（軸 1: アプリ別ブランディング）と A-11-α（軸 2: 顧客別ブランディング）は最初に合意取得すべき**で、両軸の Yes/No 組合せで B-612 / B-703-3 / B-208 / B-703-1 の 4 項目が自動決定される（パターン A / A' / B / C のマッピングは §FR-2.3.3.A）。
