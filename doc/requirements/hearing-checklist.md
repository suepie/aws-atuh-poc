...
**プラットフォーム選定への影響度が高い項目（🔥 最優先 35 件）を Stage 1 前半で先行確認**することで、ADR-014 / ADR-015 / ADR-016 / ADR-017 を早期確定できる。**特に A-11（ブランディング基本方針）は最初に合意取得すべき**で、合意により B-612 / B-703-3 / B-208 / B-703-1 の 4 項目が自動決定される。
...
| A-6 | 🟡 | エンドユーザー（顧客企業）の IdP 種別の **概算分布**（詳細は B-200-B に統合）| FR-FED-002〜007 | §FR-2.1 | **概算分布のみ**（Entra / Okta / SAML / LDAP / Google / IdP なし の比率%）。具体的な顧客 IdP 一覧は **B-200-B マスター表で詳細記入**。本 Phase A では「LDAP 直結や物理分離が含まれそうか」の初期把握に留める | 概算分布 | | ⏳ |
...
| **A-11** | 🔥 | **ブランディング基本方針合意（サインイン + サインアウト両方）** | FR-ADMIN-012 | §FR-8.3, §FR-2.3.3, §FR-2.3.3.A | **サインイン / サインアウト両方の画面カスタマイズ**について、4 つのパターンから選択。**A**: 認証基盤は最小ブランディング、アプリ側で `tenant_id` クレーム解釈で動的差替（Slack/Notion 型、シンプル）/ **A'**: アプリ単位（client_id）で認証基盤側 Branding + アプリ側でテナント別差替（**Auth0/Entra/Okta 採用、業界主流、アプリ間で異なるブランド体験を提供したい場合に推奨**）/ **B**: 認証基盤画面にも顧客ロゴ（Cognito 20 顧客上限）/ **C**: 完全専用デザイン（Enterprise）。**A or A' 合意なら B-612 / B-703-3 / B-208 / B-703-1 が自動決定**。詳細根拠は [branding-strategy-evidence.md](../common/branding-strategy-evidence.md) | A / A' / B / C | | ⏳ |
| **A-11-2** | 🟡 | **アプリ側の動的差替実装責務（サインイン後・サインアウト後 両方）** | FR-ADMIN-012 | §FR-2.3.3.A | A-11「パターン A」採用時、各アプリチームが取れる責務確認: **【サインイン後】** ①JWT `tenant_id` 解釈 / ②ダッシュボード等ロゴ・配色・文言の動的差替、**【サインアウト後】** ③ログアウト後ランディング実装 / ④強制ログアウト・セッションタイムアウト画面の出し分け、**【共通】** ⑤`tenant_id` 改竄検証（JWT クレーム真実源） / ⑥CSS/JS/SSR いずれの実装方式 | 取れる / 一部支援要 / 取れない（B/C 検討） | | ⏳ |
| **A-11-3** | 🟡 | **カスタマイズレベル確認（パターン A' 選択時）** | FR-ADMIN-012 | §FR-2.3.3.A | A-11 で「パターン A'」採用時、認証基盤側ログイン画面に求めるカスタマイズレベルは?（**L1-L3**: ロゴ・配色・スペーシング・基本配置 = Cognito Managed Login Branding で対応可 / **L4-L8**: 文言変更・要素追加削除・並び順変更・完全 HTML = **Keycloak Theme 必須** または Cognito Custom UI 自前実装、または該当アプリ単独でアプリ側 UI に寄せる選択）。詳細: [branding-strategy-evidence.md §7.A/7.B](../common/branding-strategy-evidence.md) | L1-L3 / L4-L8（Keycloak 必須化）/ アプリ側寄せ | | ⏳ |
