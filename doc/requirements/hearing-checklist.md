...
| **B-SCIM-11** | `[B]` | 🚨 | **フェデ JIT 経路の SPI 動作検証（V3''）**（2026-07-10 追加、Phase 1 実装開始前ゲート）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.F.9 検証ギャップ](../common/jit-scim-coexistence-keycloak.md), [ADR-060 §C.2.3 F-9](../adr/060-auth-protocol-attack-path-residual-tbd.md) | **[V3' PoC はローカル PW ユーザ（P-4）で検証、フェデ JIT ユーザ（P-3 = 本基盤主用途）は未検証。Browser Flow の分岐構造上、現状 SPI 配置では動作しない]** フェデ JIT ユーザで SPI が動作するかの追加 PoC：① customer-idp Realm を新設（顧客 IdP 模擬）+ fed-jit-user 作成 / ② poc-jit-scim Realm に OIDC IdP 追加 / ③ **First Broker Login Flow + Post Broker Login Flow の 2 系統に SPI 配置** / ④ 初回ログインで JIT 作成 + `last_login` 反映 / ⑤ 2 回目ログインで `last_login` 更新確認。**Phase 1 実装で 3 系統 Flow 配置必須（Browser + First Broker Login + Post Broker Login）**。**PASS しない場合、フェデユーザで JIT deprovisioning が機能しない致命的リスク**（P-3 主用途で削除判定が動かない）| 検証結果 (PASS / FAIL / PARTIAL) + Flow 配置方式 + Phase 1 実装計画への影響 + 追加工数 | | ⏳ |

> **B-SCIM-7〜11 更新注記**（2026-07-09 追加 → **2026-07-10 実機 PoC 実施済で更新 + B-SCIM-11 新規追加**）: [jit-scim §10.4.F](../common/jit-scim-coexistence-keycloak.md) に実機検証結果を集約。**総合判定 ⚠ GO with Fallback**（V2/V3' PASS + Fallback 不要、V1 PARTIAL + 代替 A 発動 = 増分小）。実機でしか判明しない **是正 F-1〜F-6** も同節に記録。**Phase 1 実装確定事項**：(1) native inbound SCIM 非依存、(2) SPI 案 B は forms サブフロー配置必須、(3) User Profile で対象属性明示宣言 + `unmanagedAttributePolicy=ENABLED/ADMIN_EDIT`、(4) per-Mapper syncMode=IMPORT で scim_active 保護。**⚠ 2026-07-10 追加発見 F-9**：V3' PoC はローカル PW ユーザ（P-4）のみ検証、フェデ JIT（P-3 = 主用途）は Browser Flow の分岐構造上動作せず → **B-SCIM-11 V3'' 追加 PoC が Phase 1 リリース前ゲートに追加**。PoC 資産：[poc/jit-scim-verification-2026-07-10/](../../poc/jit-scim-verification-2026-07-10/)
