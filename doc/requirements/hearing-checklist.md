...
| **B-SCIM-7** | `[B]` | 🚨 | **SCIM Server で Custom Attribute Mapping 検証**（2026-07-09 追加、V1、Phase 1 実装開始前ゲート、緊急度 🚨 に上昇）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V1 + §10.4.E.1 追加調査](../common/jit-scim-coexistence-keycloak.md), [ADR-025 §I.2 追記](../adr/025-scim-positioning-and-receive-stance.md) | **[ADR-025 §I.2 で Metatavu keycloak-scim-server に決定済みだが、以下 3 点の Phase 1 実装前 PoC 検証必須]** ① Keycloak 26.6 対応（README 明記なし、E-2 参照）② `scim_active`/`provisioned_by` を user_attribute に書き込めるか（POST /Users → 反映確認）③ SCIM PUT {active: false} → `scim_active=false` 更新確認。**書けない場合、[jit-scim §10.4.E.1 代替 A/B/C](../common/jit-scim-coexistence-keycloak.md) 選択**：代替 A = Keycloak 26 native SCIM + Custom Authenticator SPI 自動セット / 代替 B = Custom Schema 拡張（2-4 週間開発）/ 代替 C = SCIM 経由書込諦め、LDAP federation_link 経由判別（LDAP 顧客のみ有効） | 検証結果 (可 / 不可 / 一部制約) + Metatavu バージョン + 動作条件 + 代替案採用時の追加工数 | | ⏳ |
| **B-SCIM-8** | `[B]` | 🔴 | **Sync Mode = FORCE 時の scim_active 保護検証**（2026-07-09 追加、V2、Phase 1 実装開始前ゲート）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V2](../common/jit-scim-coexistence-keycloak.md) | Identity Provider Mapper で **`syncMode=IMPORT` を Mapper 単位で override**（Realm デフォルト FORCE でも特定属性のみ IMPORT で保護）が **`scim_active` / `provisioned_by` に対して確実に動作するか** の PoC 検証: SCIM 事前作成 → JIT ログイン後の属性値保持を確認。**一次資料上は技術的に可能**（[E-11/E-12](../common/jit-scim-coexistence-keycloak.md) で Keycloak 10+ per-Mapper syncMode extra_config 対応済）、実動作確認のみ。**動作しない場合、Sync Mode = FORCE を採用できず SCIM Push 反映が遅延する** | 検証結果 (動作 / 動作せず / 部分動作) + 代替案 | | ⏳ |
| **B-SCIM-9** | `[B]` | 🟡 | **Event Listener SPI debounce 負荷試験**（2026-07-09 追加、V3、Phase 1 実装中）| NFR-PERF / NFR-COMPLIANCE | [jit-scim §10.4.D.2 V3](../common/jit-scim-coexistence-keycloak.md) | debounce（1 日 1 回のみ user_attribute 更新）の実装で **性能問題ないか** の負荷試験: 100 RPS × 24h × 10 万ユーザ、Aurora 書き込み IOPS + Infinispan cache invalidate 遅延 + Keycloak Pod CPU/メモリ監視。**閾値超過時は debounce 期間延長 or Async 化検討**。**実装方式は [jit-scim §10.4.E.2 案 B Custom Authenticator SPI 前提](../common/jit-scim-coexistence-keycloak.md)**（Event Listener SPI の `setSingleAttribute` は Keycloak Issue #14942 により動かない可能性、E-8 参照） | 負荷試験結果 (IOPS / cache 遅延 / CPU) + 閾値評価 | | ⏳ |
| **B-SCIM-10** | `[B]` | 🚨 | **Event Listener SPI vs Custom Authenticator SPI 実装方式決定**（2026-07-09 追加、V3'、Phase 1 実装開始前ゲート）| FR-USER / NFR-COMPLIANCE | [jit-scim §10.4.E.2 3 案 + E-8/E-9/E-10 一次資料](../common/jit-scim-coexistence-keycloak.md), [ADR-060 §C.2.3 SPI コード例警告](../adr/060-auth-protocol-attack-path-residual-tbd.md) | **[Keycloak Issue #14942 (Closed as not planned) により Event Listener SPI 内の setSingleAttribute が動かない可能性が極めて高い、Issue #22902 で enlistAfterCompletion workaround にも ConcurrentModificationException 懸念]** last_login / provisioned_by 書込の実装方式を PoC 検証で確定: **案 B Custom Authenticator SPI**（推奨、確実性最高、[ADR-055](../adr/055-hrd-implementation-method-selection.md) 同じ Java SPI 体制）/ **案 A enlistAfterCompletion workaround**（公式推奨だが Issue #22902 懸念）/ **案 C 外部 DB 別管理**（EventBridge + Lambda + DynamoDB、実装コスト大） | 採用案 (A/B/C) + PoC 結果 + Phase 1 実装計画への影響 | | ⏳ |

> **B-SCIM-7〜10 追加注記**（2026-07-09、07-09 緊急度更新）: [jit-scim §10.4](../common/jit-scim-coexistence-keycloak.md) 破綻を修正した Event Listener SPI + user_attribute.last_login 版（§10.4.A）+ JIT/SCIM 判別ロジック（§10.4.B）の **Phase 1 実装前 / 実装中の検証ゲート項目**。**2026-07-09 に 14 件の一次資料調査（[§10.4.E](../common/jit-scim-coexistence-keycloak.md)）により、V1（B-SCIM-7）と V3'（B-SCIM-10）の緊急度が 🚨 に上昇**。V1/V2/V3' は Phase 1 実装開始前に PoC 必須、V3 は Phase 1 実装中の負荷試験必須。[ADR-025 §I.2](../adr/025-scim-positioning-and-receive-stance.md) の Metatavu keycloak-scim-server + [ADR-060 §C.2.3](../adr/060-auth-protocol-attack-path-residual-tbd.md) の SPI 拡張の前提となる。
