...
| **FIPS 140-3**（旧 140-2）| 米国連邦政府用暗号モジュール認定（→ RHBK 必須化要因）。**140-2 は 2026-09-21 に CMVP historical list へ移行**（[CMVP](https://csrc.nist.gov/projects/fips-140-3-transition-effort)）|
| **HIPAA** | 米国医療情報保護法 |
...
コンプライアンスは **規制への適合**。**FIPS 140-3（旧 140-2）が Must なら RHBK 必須**（本基盤は C-201 で **不要確定**）、**ISMAP 対応なら AWS リージョン制約**等、プラットフォーム選定に直結。
...
| **FIPS 140-3**（旧 140-2）| 米国政府 / 一部金融 | **RHBK 必須化** — ただし本基盤は **不要で確定**（C-201、2026-07-30）|
...
> **主な判断軸**: 顧客業界、契約要件   
> **§NFR-7 全体との関係**: **FIPS 140-3（旧 140-2）/ 24/7 サポートはプラットフォーム選定に直結**（FIPS は C-201 で不要確定、24/7 は [ADR-040](../../../adr/040-pam-jit-admin-privilege-management.md) §I の α/β 段階）
...
| PCI DSS | 顧客要件次第 |
| **FIPS 140-3**（旧 140-2）| **不要で確定**（C-201、2026-07-30）。Must に転じた場合のみ RHBK 必須 |
...
| ~~FIPS 140-3 Must~~ | ✅ 解消 — **いいえ**（C-201、2026-07-30 確定）|
