...
- **JWKS 経路のレイテンシ**: アプリアカウントから共通基盤の JWKS endpoint への経路がパブリックインターネット経由 / VPC PrivateLink で性能が変わる。Lambda Authorizer 内 JWKS キャッシュ TTL（1 時間）で吸収済（[§NFR-4.5](04-security.md)）
- **Admin API のレイテンシ**: アプリ運用が委譲管理者として Admin REST API を呼ぶ場合、クロスアカウントの OAuth トークン取得 + API 呼び出しが発生。Token キャッシュ前提で実用上問題なし（[§NFR-6.5 C-1](06-operations.md)）
- **構成変更の所要時間**: 性能劣化時の対応（しきい値変更 / スケール変更）に [§NFR-6.4 構成変更プロセス](06-operations.md) の SLA が影響
