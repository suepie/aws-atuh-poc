...
- **アカウント分離による追加コスト**: 専用アカウント運用に伴う固定費（AWS Organizations / Control Tower / Security Hub 等）は **既存 AWS 運用枠で吸収可能**な範囲（年数万円規模）
- **クロスアカウント運用工数の削減効果**: [§FR-1.2.0.B Layer 1-4](../fr/01-auth.md) + [§NFR-6.4 / §NFR-6.5](06-operations.md) の運用モデル採用で、**共通基盤運用チーム規模を MAU 増加に対して亜線形に抑制**できる
- **請求集約**: AWS Organizations Consolidated Billing で共通基盤 + アプリの合算請求。ボリュームディスカウントも適用可能
- **コスト配賦**: Cognito MAU 課金は共通基盤アカウントに集約 → アプリへの内部配賦は別途 Cost Allocation Tag で按分
