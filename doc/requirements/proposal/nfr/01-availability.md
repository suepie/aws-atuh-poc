...
共通基盤は専用 AWS アカウントに置く前提のため、可用性に関わる以下の点も併せて見ること:

- **クロスアカウント緊急対応**: 障害時、共通基盤運用 on-call と影響アプリ運用の連絡経路を [§NFR-6.5 E-1〜E-4](06-operations.md) で定義。インシデント司令塔の役割明示
- **管理画面アクセス継続性**: 共通基盤運用が緊急時にアクセスできる経路を二重化（[§NFR-4.5 クロスアカウント IAM](04-security.md) 参照、Break Glass Role を含む選択肢）
- **アプリ側からの可用性 SLI**: アプリ側で計測した認証成功率も SLO 達成判定に含める（アプリアカウントの CloudWatch から共通基盤の Health Check に依存）
