...
**最終更新**: 2026-07-24（基本設計 Wave 1-3 反映） / **対象**: 要件定義フェーズ確定構成 + 基本設計確定事項（ADR-001〜060 + basic-design U1〜U10 反映）

> **🆕 2026-07-24 基本設計 Wave 1-3 反映（本章の役割再定義を含む）**
>
> 基本設計 10 冊（[basic-design/00-basic-design-plan.md](../../../basic-design/00-basic-design-plan.md) / [01-architecture-baseline.md](../../../basic-design/01-architecture-baseline.md) Baseline v1（P-01〜P-18）+ U2〜U10）の確定を受け、本章の旧記述を最新化した。**本章は実装アーキテクチャの SSOT を継続するが、詳細設計値（サイジング / IAM / 経路 / Runbook 等）は basic-design 各冊へ委譲する**（本章 = 決定要約 + 参照リンク。旧記述は取り消し線・注記で履歴保持）。主な改訂 10 点:
>
> 1. **実行基盤 EKS → ROSA HCP + RHBK Operator**（インスタンス r6g → c7g、Machine Pool は 2 系統 = KC 専用 taint Pool + default/infra Pool — [U6 D-U6-03/04](../../../basic-design/06-infra-network-design.md)。Pod 数・Aurora サイジング詳細は U6 §6.2/§6.4/§6.5）
> 2. **5 → 6 アカウント体系**（ネットワーク監査 Acct は**他組織管理（P-18）**となり、当該設定は要求仕様 REQ-IN-01〜12 / REQ-OUT-01〜06 / REQ-DR-01〜05 として扱う — [U6 §6.0.2 A 部（自管理）/ B 部（要求仕様）構造 + §6.7](../../../basic-design/06-infra-network-design.md) / [U8 §8.4.5](../../../basic-design/08-availability-dr-design.md)）
> 3. **旧 Auth Platform Acct → Broker Acct / IdP-KC Acct に分割**（Route 53 PHZ = Broker Acct / ユーザ管理の専用 API 層 = IdP-KC Acct / ITDR・管理画面 Backend = Broker Acct — [01-architecture-baseline.md §1.2](../../../basic-design/01-architecture-baseline.md)、U6 D-U6-01）
> 4. **CI/CD = GitHub Actions + OIDC / OpenShift GitOps / ECR**（旧「ROSA = Tekton + Quay」は破棄 — [U9 D-U9-12](../../../basic-design/09-operations-observability-design.md)）
> 5. **DR = パイロットライト（インフラ Warm + KC Scale 0）+ Realm Export 全廃 → 復元 2 経路**（① リージョン障害 = Aurora Global 一体復元 ② 論理破壊 = PITR + 差分 IaC 再生 — [U8 D-U8-06/07](../../../basic-design/08-availability-dr-design.md)、ADR-051 2026-07-23 正式改訂。Tier 1（RTO 30 分）は Phase 2 検討）
> 6. **Workload Identity = ROSA pod identity webhook + IRSA 方式**（旧 EKS Pod Identity — ADR-041 2026-07-23 改訂、実装 SSOT = [U7 §7.5](../../../basic-design/07-security-compliance-design.md)）
> 7. **`legacy_user_id` Conditional クレーム廃止**（旧 ID は idmap API `GET /idmap/{sub}` 参照へ — [U10 D-U10-12](../../../basic-design/10-integration-migration-design.md)）
> 8. **Turnstile L2 = Phase 2 オプション + 前倒しトリガー明確化**（REQ-IN-01（WAF Bot Control/ATP）が他組織に受け入れられない場合 — [U7 D-U7-17](../../../basic-design/07-security-compliance-design.md)、ADR-042 整合）
> 9. **IaC 2 層分離**（基盤層 Terraform + テナント層オンボーディング API — keycloak-config-cli 不採用、[U9 D-U9-09/10](../../../basic-design/09-operations-observability-design.md)）
> 10. **SCIM 受信 = 自作 SCIM Facade**（native / Metatavu 非依存 — [U3 D3-11](../../../basic-design/03-identity-provisioning-design.md)）

---

> **⚠ 2026-06-24 設計レビュー Phase 1-5 反映完了**（履歴）:
>
...
>
> **Phase 3**：ADR-039 v2 — **5 アカウント体系**（Network / ネットワーク監査 / 監査 / Auth / App）+ **アプリごと独立 CloudFront/WAF** + Route 53 各 App Acct 別管理 + **/admin パス保護方針**（→ 2026-07-24 基本設計反映で **6 アカウント体系**に更新、冒頭改訂サマリ 2/3 と [§C-7.2.3](#§c-7.2.3-aws-アカウント境界の整理) 参照）
>
...
>
> **Phase 6（2026-06-25）**：**HRD 実装方式 = Custom Authenticator SPI（Java、社内開発）採用確定** — [ADR-055](../../../adr/055-hrd-implementation-method-selection.md) で 3 方式裏どり調査の上、Phase 1 = 方式 A（SPI、共通 URL、社内 Java 開発）/ Phase 2 候補 = 方式 C（URL + CloudFront Function）併用。~~実行基盤次第で CI/CD ツールチェーン変化（EKS = GitHub Actions + ECR + Helm / ROSA = Tekton + Quay.io + rhbk-operator）~~（2026-07-24 基本設計反映: 実行基盤 = **ROSA HCP + RHBK Operator** 確定、CI/CD = **GitHub Actions + OIDC / OpenShift GitOps / ECR** — 詳細 = [basic-design/09](../../../basic-design/09-operations-observability-design.md) §9.6.1 D-U9-12）。詳細は §C-7.3.4.4 Custom SPI / §C-7.3.29 ID 統合構成 参照。
...
| [ADR-025](../../../adr/025-scim-positioning-and-receive-stance.md) SCIM（**§H 追記 2026-07-08**：LDAP(s) 顧客の JIT/SCIM 扱い）| ~~SCIM 2.0 Server（Phase Two plugin）~~ → **自作 SCIM Facade（native / Metatavu 非依存）**（2026-07-24 基本設計反映: 詳細 = [basic-design/03](../../../basic-design/03-identity-provisioning-design.md) §3.5 D3-11）+ **LDAP User Federation Provider（Keycloak 標準）で LDAP 顧客対応**（§C-7.3.4.4.B）|
...
| **[ADR-039 v2](../../../adr/039-centralized-network-account-edge-layer.md) ネットワーク監査アカウント設計** | ~~**5 アカウント体系**（Network / **ネットワーク監査** / 監査 / Auth / App）~~ → **6 アカウント体系**（Broker / IdP-KC 分割 + ネットワーク監査 Acct = **他組織管理（P-18）、設定は要求仕様 REQ-IN/REQ-OUT/REQ-DR で扱う**）+ **アプリごと独立 CloudFront/WAF** + /admin パス保護方針（2026-07-24 基本設計反映: 詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.1.1 D-U6-01 / §6.7）|
...
| **[ADR-041](../../../adr/041-workload-identity-spiffe.md) Workload Identity** | ~~**EKS Pod Identity**~~ → **ROSA pod identity webhook + IRSA 方式 + Keycloak Federated Identity Credentials**（client_secret 廃止、ADR-041 2026-07-23 改訂。2026-07-24 基本設計反映: 実装 SSOT = [basic-design/07](../../../basic-design/07-security-compliance-design.md) §7.5）|
...
| **[ADR-055](../../../adr/055-hrd-implementation-method-selection.md) HRD 実装方式選定** | **Phase 1 = Custom Authenticator SPI（Java、社内開発）採用確定** / Phase 2 = URL + CloudFront Function 併用候補。~~実行基盤別 CI/CD・バージョン追従併記（EKS / ROSA Classic / HCP）~~ → 実行基盤 = **ROSA HCP + RHBK Operator** 確定（2026-07-24 基本設計反映: 詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2 / [basic-design/09](../../../basic-design/09-operations-observability-design.md) §9.6.1）|
| **[ADR-051](../../../adr/051-multi-region-dr-failover.md) Multi-Region DR** | ~~**Active-Passive Warm Standby**~~ → **Active-Passive パイロットライト（インフラ Warm + KC Scale 0）+ Aurora Global + DynamoDB Global Tables + S3 CRR + KMS MRK**（Tier 2 RTO 1h / RPO 1min、~~Tier 1 規制業種 30min オプション~~ Tier 1 は Phase 2 検討。2026-07-24 基本設計反映: 詳細 = [basic-design/08](../../../basic-design/08-availability-dr-design.md) D-U8-05〜07、ADR-051 2026-07-23 正式改訂）|
...
| **効率よく** | 顧客追加で各システム変更不要、IaC ~~（Terraform）~~ = **2 層分離（基盤層 Terraform + テナント層オンボーディング API、keycloak-config-cli 不採用）**（2026-07-24 基本設計反映: 詳細 = [basic-design/09](../../../basic-design/09-operations-observability-design.md) §9.5 D-U9-09/10）|
...
| 判断 | 採用方針 | 根拠 ADR |
|---|---|---|
| プラットフォーム | **Keycloak（自社ホスト）**、Cognito / Entra External ID は MAU 単価でコスト破綻。実行基盤 = **ROSA HCP + RHBK Operator**（2026-07-24 基本設計反映: 詳細 = [basic-design/01](../../../basic-design/01-architecture-baseline.md) P-01 / [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2）| ADR-032, 056 |
...
| Workload 認証 | ~~**EKS Pod Identity**~~ → **ROSA pod identity webhook + IRSA + Keycloak FedID**（Secret ゼロ化、PCI DSS §8.6。2026-07-24 基本設計反映: 詳細 = [basic-design/07](../../../basic-design/07-security-compliance-design.md) §7.5）| ADR-041（2026-07-23 改訂）|
...
| Multi-Region DR | ~~**Active-Passive Warm Standby**~~ → **パイロットライト（インフラ Warm + KC Scale 0）+ Aurora Global + KMS MRK**（Tier 2 RTO 1h / RPO 1min。2026-07-24 基本設計反映: 詳細 = [basic-design/08](../../../basic-design/08-availability-dr-design.md) D-U8-05〜07）| ADR-051（2026-07-23 改訂）|
...
| オレンジ枠 | **AWS サービス**（~~EKS~~ **ROSA HCP** / Aurora / Lambda 等。2026-07-24 基本設計反映: [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2）|
...
> **🆕 2026-07-24 基本設計反映 — 6 アカウント体系へ更新（最新 SSOT は basic-design）**:
>
> | 決定要約 | 内容 |
> |---|---|
> | アカウント体系 | **6 アカウント**（🔷 ネットワーク / 🟣 ネットワーク監査 / 🔵 監査 / 🟠 **Broker** / 🟠 **IdP-KC** / 🟢 App × N）。旧 Auth Platform Acct を **Broker Acct / IdP-KC Acct に分割**（P-17）|
> | 他組織管理境界 | 🔷 ネットワーク Acct + 🟣 ネットワーク監査 Acct は**他組織管理（P-18）**。CloudFront / WAF / Network Firewall 等の設定は**要求仕様（REQ-IN-01〜12 / REQ-OUT-01〜06 / REQ-DR-01〜05）**として起こし、我々は要求と受入確認のみ |
> | 設計書構造 | [U6 §6.0.2](../../../basic-design/06-infra-network-design.md) の **A 部（自管理: Broker / IdP-KC / 監査 Acct 内設計）/ B 部（他組織への要求仕様 §6.7）** の 2 部構成。B 部が満たされなくても A 部単独で破られないことが設計原則 |
> | 参照 | 最新の全体図・アカウント体系 = [basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md) / [basic-design/06 §6.1.1 D-U6-01](../../../basic-design/06-infra-network-design.md)、DR 要求 = [basic-design/08 §8.4.5](../../../basic-design/08-availability-dr-design.md) |
>
> 以下の 5 アカウント体系サマリ図・詳細図は**旧版（2026-06-24 v2 時点）の参考残置**。
>
...
> ```
>
> 以下の詳細図は **v1（4 アカウント体系）時の概念図**として参考残置（EKS / Auth Platform Acct / Phase Two SCIM 等の表記は旧構成のまま。最新は 2026-07-24 基本設計反映: ROSA HCP + RHBK Operator / Broker・IdP-KC Acct 分割 / 自作 SCIM Facade — [basic-design/06](../../../basic-design/06-infra-network-design.md) 参照）。v2 対応の詳細図は drawio 化時に作成予定（[§C-7.7](#§c-7.7-drawio-転記時の注意) 参照）。
>
...
flowchart TB
    ...
    subgraph AuthAcct["🟠 AWS アカウント: Auth Platform Account"]
        ...
        subgraph Tier1["🟡 Tier 1: Broker Keycloak"]
            direction TB
            subgraph BrokerEKS["ROSA HCP Cluster (Broker)（旧 EKS、2026-07-24 更新）"]
                BKPods["Keycloak Pods (RHBK Operator)<br/>Multi-AZ<br/>サイジング = U6 §6.5"]
            ...
            subgraph BrokerDB["Aurora PostgreSQL (Broker)"]
                BAurora[("旧 r6g → c7g 系へ更新<br/>サイジング = U6 §6.4/6.5<br/>Multi-AZ / I/O-Optimized")]
            end
        ...
        subgraph Tier2["🟢 Tier 2: IdP Keycloak"]
            direction TB
            subgraph IdPKCEKS["ROSA HCP Cluster (IdP-KC)（旧 EKS、2026-07-24 更新）"]
                IKPods["Keycloak Pods (RHBK Operator)<br/>Multi-AZ<br/>サイジング = U6 §6.5"]
            end
            ...
            subgraph IdPKCDB["Aurora PostgreSQL (IdP-KC)"]
                IAurora[("旧 r6g → c7g 系へ更新<br/>サイジング = U6 §6.4/6.5<br/>Multi-AZ")]
            end
        ...
        subgraph SCIMLayer["🔄 SCIM / Webhook"]
            SCIMServer["自作 SCIM Facade<br/>(旧 Phase Two plugin、U3 D3-11)<br/>受信 /Users /Groups"]
...
> **🆕 2026-07-24 基本設計反映**：5 アカウント → **6 アカウント体系**（Broker / IdP-KC 分割〔P-17〕+ ネットワーク監査 Acct = **他組織管理〔P-18〕**。詳細 = [basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md) / [basic-design/06 §6.1.1 D-U6-01](../../../basic-design/06-infra-network-design.md)）

| アカウント | 管理主体 | 役割 | 主要リソース（決定要約） | 数 |
|---|---|---|---|:---:|
| 🔷 **ネットワーク Acct** | **他組織（想定・要確認）** | **インフラネットワーク（L3 接続性）**| Transit Gateway / DX / VPN — 設定は要求仕様として扱う | **1** |
| 🟣 **ネットワーク監査 Acct** | **他組織（P-18）** | **エッジ層 + 通信監視（L4-L7）**| CloudFront + WAF / ALB or NLB / Network Firewall（In/Out）/ Shield / ACM — **当該設定は要求仕様 REQ-IN-01〜12 / REQ-OUT-01〜06 / REQ-DR-01〜05 として起こす**（[U6 §6.7](../../../basic-design/06-infra-network-design.md) / [U8 §8.4.5](../../../basic-design/08-availability-dr-design.md)）| **1** |
| 🔵 **監査 Acct** | 弊社（Compliance）| **組織統制 + コンプライアンス監査**| AWS Organizations / CloudTrail Org Trail / 監査ログ集約 S3（Object Lock 7 年）/ Security Hub / GuardDuty 集約 | **1** |
| 🟠 **Broker Acct** | 認証基盤チーム | Broker KC コア | **ROSA HCP クラスタ #1（Broker KC）**+ Aurora + KMS + SPA S3 + **ITDR** + **ユーザ管理画面 Backend** + Trust Center 内部 + **Route 53 PHZ（basis.example.com）**（U6 §6.1/§6.2）| **1** |
| 🟠 **IdP-KC Acct** | 認証基盤チーム | IdP-KC + ユーザ CRUD | **ROSA HCP クラスタ #2（IdP-KC）**+ Aurora + KMS + **専用 API 層（ユーザ管理、同居アプリからのユーザ CRUD — U3 D3-05 経路 ⑤）**（U6 §6.1/§6.3）| **1** |
| 🟢 **App Acct A, B, C...** | 各アプリチーム | 各業務アプリ稼働 | Internal ALB + アプリ実体 + App API + Route 53 Hosted Zone（各アプリチーム管理）| **N** |

→ **最小構成 = ネットワーク + ネットワーク監査 + 監査 + Broker + IdP-KC + App の 6 アカウント体系**。設計書は [U6 §6.0.2](../../../basic-design/06-infra-network-design.md) の **A 部（自管理: §6.1〜6.6）/ B 部（他組織への要求仕様: §6.7）** の 2 部構成で、混在させない。

> ~~**⚠ 2026-06-24 Phase 3 v2 更新**：4 アカウント → **5 アカウント体系**（[ADR-039 v2](../../../adr/039-centralized-network-account-edge-layer.md)）。旧表: 🟠 Auth Platform Acct（EKS（Broker KC + IdP-KC）+ Aurora × 2 + KMS × 2 + SPA S3 + ITDR Lambda + ユーザ管理画面 Backend + Trust Center 内部 + Route 53 Hosted Zone）を単一アカウントとし、ネットワーク監査 Acct は Network 監査 / Security チーム（自組織）管理としていた~~（2026-07-24 上表へ全面更新）
...
| 👤 I-2 | Keycloak 運用者 | B | AWS IAM + oc/kubectl auth | ~~EKS~~ **ROSA HCP（Broker / IdP-KC 両クラスタ）** / Bastion 経由（2026-07-24 基本設計反映: 詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2 / §6.6.2）| ADR-029 |
...
| 領域 | 主要リソース | 構成 | 根拠 |
|---|---|---|---|
| Network | VPC + Private Subnet（Multi-AZ）+ NAT GW + VPC Endpoint | Broker / IdP-KC 各 Acct に VPC、Multi-AZ | ADR-010、U6 §6.1 |
| Tier 1 Broker（🟠 Broker Acct）| ~~EKS Cluster~~ **ROSA HCP クラスタ #1 + RHBK Operator** + Aurora PostgreSQL（I/O-Optimized）+ KMS CMK | ~~r6g.2xlarge × 3-4~~ **KC 専用 taint Pool（c7g.xlarge、スケール上限 c7g.2xlarge 別 Pool）+ default/infra Pool（c7g.large）**。Pod 数・Aurora サイジング = [U6 §6.2/§6.4/§6.5](../../../basic-design/06-infra-network-design.md)（2026-07-24 基本設計反映: D-U6-03/04/07）| ADR-033, 056 |
| Tier 2 IdP-KC（🟠 IdP-KC Acct）| ~~EKS Cluster（別）~~ **ROSA HCP クラスタ #2 + RHBK Operator（別 Acct）** + Aurora PostgreSQL（別）+ KMS CMK（別）| ~~r6g.xlarge × 2-3~~ **Machine Pool 2 系統構成は Broker と同型（c7g 系）**。サイジング = [U6 §6.2/§6.4/§6.5](../../../basic-design/06-infra-network-design.md)（2026-07-24 基本設計反映）| ADR-033, 056 |
...
>
> **🆕 2026-07-24 基本設計反映（v2 → 6 アカウント体系）**: ネットワーク Acct / ネットワーク監査 Acct は**他組織管理（P-18）**となり、本節のエッジ層設定（CloudFront / WAF / Network Firewall / Shield / ACM）は我々が実装できない。**要求仕様 REQ-IN-01〜12（Inbound）/ REQ-OUT-01〜06（Egress）/ REQ-DR-01〜05（DR 切替）として起こし、受入確認のみ行う**（詳細 = [basic-design/06 §6.0.2（A 部/B 部構造）/ §6.7](../../../basic-design/06-infra-network-design.md)、[basic-design/08 §8.4.5](../../../basic-design/08-availability-dr-design.md)）。また旧 Auth Platform Acct は Broker Acct / IdP-KC Acct に分割（Route 53 PHZ = Broker Acct）。
...
#### §C-7.3.3.2 Auth Platform Acct（🟠）配置リソース（→ 2026-07-24 Broker Acct / IdP-KC Acct に分割）

> **🆕 2026-07-24 基本設計反映**: 下表のリソースは分割後、**External/Internal ALB・SPA S3・Route 53 PHZ（basis.example.com）= Broker Acct**、**IdP-KC クラスタ + ユーザ管理の専用 API 層 = IdP-KC Acct** に配置（詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.1〜6.3）。
...
| カテゴリ | コンポーネント | 詳細 | 根拠 |
|---|---|---|---|
| Compute | ~~EKS Cluster（Tier 1 専用）~~ **ROSA HCP クラスタ #1（Broker Acct）+ RHBK Operator** | Keycloak 26.x（RHBK）、HA Multi-AZ、**Machine Pool 2 系統（KC 専用 taint Pool + default/infra Pool）**（2026-07-24 基本設計反映: 詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2 D-U6-03/04）| ADR-033, 056 |
| Pod | Keycloak Pods | ~~10-20 pod、auto-scaling、1,250 MB / pod~~ Pod 数・HPA・メモリは [U6 §6.2/§6.5](../../../basic-design/06-infra-network-design.md) 参照（2026-07-24 基本設計反映）| ADR-033 |
| DB | Aurora PostgreSQL（I/O-Optimized）| ~~r6g.2xlarge × 3-4~~ **c7g 系、サイジング・コネクション設計 = [U6 §6.4/§6.5](../../../basic-design/06-infra-network-design.md) D-U6-07/08**、Multi-AZ（2026-07-24 基本設計反映）| ADR-033 |
...
| **初期開発工数** | **1-2 週間** |
| **メンテ頻度** | ~~EKS Upstream Keycloak 採用時：年 4-6 回 /~~ **ROSA HCP + RHBK 採用確定：年 1-2 回**（[ADR-055 §A.7](../../../adr/055-hrd-implementation-method-selection.md)。2026-07-24 基本設計反映: 実行基盤 = [basic-design/01](../../../basic-design/01-architecture-baseline.md) P-01）|
...
| **CI/CD ツールチェーン** | ~~EKS：GitHub Actions + ECR + Helm + ArgoCD / ROSA：OpenShift Pipelines (Tekton) + Quay.io + rhbk-operator + OpenShift GitOps~~ → **CI = GitHub Actions + OIDC / CD = OpenShift GitOps / Registry = ECR**（2026-07-24 基本設計反映: 詳細 = [basic-design/09](../../../basic-design/09-operations-observability-design.md) §9.6.1 D-U9-12。旧 [ADR-055 §A.6](../../../adr/055-hrd-implementation-method-selection.md) の Tekton + Quay 案は不採用）|
...
| ~~Conditional~~ **廃止** | ~~`legacy_user_id`（並走期、後で外す）~~ **`legacy_user_id` クレームは発行しない** — 旧 ID を必要とするアプリは JWT `sub` を鍵に idmap API `GET /idmap/{sub}` で解決（2026-07-24 基本設計反映: 詳細 = [basic-design/10](../../../basic-design/10-integration-migration-design.md) §10.4 D-U10-12、ADR-019 上書き）| ~~ADR-019~~ U10 D-U10-12 |
...
| カテゴリ | コンポーネント | 詳細 | 根拠 |
|---|---|---|---|
| Compute | ~~EKS Cluster（Tier 2 専用、Tier 1 と分離）~~ **ROSA HCP クラスタ #2 + RHBK Operator（IdP-KC Acct、Broker Acct と別アカウント）** | Keycloak 26.x（RHBK）、Machine Pool 2 系統（2026-07-24 基本設計反映: 詳細 = [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.2 D-U6-03/04）| ADR-033, 056 |
| Pod | Keycloak Pods | ~~5-10 pod、auto-scaling~~ Pod 数・フェデ比率感度 = [U6 §6.5.3](../../../basic-design/06-infra-network-design.md) 参照（2026-07-24 基本設計反映）| ADR-033 |
| DB | Aurora PostgreSQL（別クラスタ）| ~~r6g.xlarge × 2-3~~ **c7g 系、サイジング = [U6 §6.4/§6.5](../../../basic-design/06-infra-network-design.md)**（2026-07-24 基本設計反映）| ADR-033 |
...
| カテゴリ | コンポーネント | 役割 | 根拠 |
|---|---|---|---|
| 受信 SCIM | ~~SCIM 2.0 Server（Phase Two Keycloak plugin）~~ **自作 SCIM Facade（`/scim/v2`、テナント別 URL + Bearer、native / Metatavu 非依存）**（2026-07-24 基本設計反映: 詳細 = [basic-design/03](../../../basic-design/03-identity-provisioning-design.md) §3.5 D3-11）| 顧客 IdP からの自動同期 | ADR-025 §I.2、U3 D3-11 |
...
| エラー / 案内画面 SPA（`launchpad.example.com/sorry`）| `/sorry?app=x&reason=Y` で表示 | 🟠 ~~Auth Platform Acct~~ **Broker Acct** S3（OAC で ネットワーク監査 Acct CF から配信。2026-07-24 基本設計反映: [basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md)）|
...
| DynamoDB（監査ログ）| ユーザ管理画面 操作ログ | ~~Auth Platform Account~~ **Broker Acct**（ADR-038。2026-07-24 基本設計反映: [basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md)）|
| SIEM 連携 | 顧客 SIEM へ出力 | EventBridge + Lambda（OCSF 変換）|
| Trust Center（公開部）| 標準 10 アーティファクト公開 | ~~Auth Platform Account~~ **Broker Acct**（2026-07-24 基本設計反映）|
| Customer Portal（認証要部）| NDA 下エビデンス | OIDC 認証要 |

### §C-7.3.13 ユーザ管理画面 バックエンド（ADR-038）

> **🆕 2026-07-24 基本設計反映（配置アカウント）**: 管理画面 Backend（API GW + Lambda + DynamoDB）と ITDR は **Broker Acct** 配置。一方、**ユーザ CRUD の専用 API 層（アプリ発 CRUD の経路 ⑤）は IdP-KC Acct** に配置され、idmap 更新は EventBridge クロスアカウントイベントで Broker 側に伝播する（詳細 = [basic-design/03](../../../basic-design/03-identity-provisioning-design.md) D3-05 / [basic-design/06](../../../basic-design/06-infra-network-design.md) §6.1.2）。
...
| **L3 アプリ層** | Keycloak Realm / IdP 設定 | Composite Role `<role>-eligible` / `-active` + **EventBridge Lambda**（期限到来で自動 unassign）| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（2026-07-24 基本設計反映: [basic-design/07 §7.6](../../../basic-design/07-security-compliance-design.md)）| PCI DSS 10.2.1 |
| **L4 テナント特権** | 全ユーザー削除 / 全 MFA リセット | ユーザ管理画面 内 **JIT 承認ワークフロー**（SoD：申請者 ≠ 承認者）| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（ADR-038。2026-07-24 基本設計反映）| APPI 第 23 条 |
...
### §C-7.3.16 Workload Identity（ADR-041、2026-07-23 改訂）

> **🆕 2026-07-24 基本設計反映**: 実行基盤 ROSA HCP 化に伴い、~~EKS Pod Identity~~ → **ROSA 標準の pod identity webhook + IRSA 方式**へ変更（ADR-041 2026-07-23 改訂、STS 2 段チェーン設計は維持）。**実装 SSOT = [basic-design/07 §7.5](../../../basic-design/07-security-compliance-design.md)**（IRSA Role 設計 D-U7-09 / FedID 適用範囲 D-U7-10a / private_key_jwt 昇格 D-U7-10b）。以下の表は決定要約のみ。
>
> マイクロサービス前提（顧客打ち合わせ確認済み）。PCI DSS §8.6.1 / §8.6.2 を ~~Pod Identity~~ **IRSA** + Federated Identity Credentials で同時充足、Client Secret ゼロ化。

#### §C-7.3.16.1 4 認証境界とコンポーネント

| 境界 | 方式 | コンポーネント | 配置 | Token |
|---|---|---|---|---|
| **Pod → AWS リソース** | ~~**EKS Pod Identity**（2024 GA、IRSA 後継）~~ → **ROSA pod identity webhook + IRSA**（2026-07-24 基本設計反映: 詳細 = [basic-design/07](../../../basic-design/07-security-compliance-design.md) §7.5.1 D-U7-09）| ~~pod-identity-agent DaemonSet~~ pod identity webhook + K8s SA + IAM Role（OIDC Provider 信頼）| 🟢 App Acct（Pod 単位）+ 🟠 Broker/IdP-KC Acct（基盤 Pod）| STS Session Token（1h）|
...
| Phase | コンポーネント | 採用判断 |
|---|---|---|
| **Phase 1（採用）** | ~~EKS Pod Identity~~ **ROSA pod identity webhook + IRSA** + Keycloak FedID + 既存 Task / Execution Role（2026-07-24 基本設計反映: [basic-design/07 §7.5](../../../basic-design/07-security-compliance-design.md)）| AWS / ROSA 標準、追加製品ゼロ |
...
| ログ | 内容 | 保管先 |
|---|---|---|
| ~~EKS~~ **ROSA HCP** Audit Log | K8s API 全リクエスト（SA Token 発行含む。2026-07-24 基本設計反映: [basic-design/09](../../../basic-design/09-operations-observability-design.md)）| 🟢 App Acct CloudWatch → 🔵 Audit Acct S3 |
| CloudTrail | ~~Pod Identity → STS AssumeRole~~ **IRSA → STS AssumeRoleWithWebIdentity**（2026-07-24 基本設計反映）| CloudTrail Organization Trail |
...
>
> **🆕 2026-07-24 基本設計反映（前倒しトリガー追加）**: L1 の WAF Bot Control / ATP / Rate Limit は**他組織管理のネットワーク監査 Acct への要求仕様 REQ-IN-01** となった（P-18）。**REQ-IN-01 が他組織に受け入れられない場合、L2 Turnstile（+ Keycloak Custom Authenticator）を Phase 1 に前倒しするトリガー**とする。自管理側の最低線 = KC Brute Force + Enumeration 対策 + ITDR。REQ-IN-01 不成立のまま PCI 顧客契約は禁止（詳細 = [basic-design/07 §7.8.1 D-U7-17](../../../basic-design/07-security-compliance-design.md)、ADR-042 整合）。
>
...
| **L2 アプリ層**（Phase 2 オプション + REQ-IN-01 不成立時の前倒しトリガー）| **Cloudflare Turnstile**（Invisible、`data-size="invisible"`）+ **Keycloak Authenticator SPI**（カスタム TurnstileAuthenticator）| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（2026-07-24 基本設計反映: [basic-design/07 §7.8.1 D-U7-17](../../../basic-design/07-security-compliance-design.md)）| デバイス指紋 + ふるまい | Invisible 時なし |
| **L3 アカウント層** | ITDR Anomaly Login + Adaptive Auth（既存 ADR-034 / 035）| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（2026-07-24 基本設計反映）| アカウント単位異常 | ステップアップ MFA |
...
| **L5 配布** | **Kyverno Cluster Policy**（Cosign Verify、未署名 Image deploy ブロック）| 🟠 ~~Auth Acct EKS~~ **Broker / IdP-KC Acct ROSA HCP** / 🟢 App Acct ~~EKS~~ 実行基盤（2026-07-24 基本設計反映: [basic-design/09 §9.6](../../../basic-design/09-operations-observability-design.md)）|
...
### §C-7.3.26 Multi-Region DR / Failover（ADR-051、2026-07-23 正式改訂）

> **🆕 2026-07-24 基本設計反映（DR モデル全面更新）**: ~~Warm Standby（DR 側 KC Scale 1）~~ → **パイロットライト（インフラ Warm + KC Scale 0）**。Aurora Global Secondary は read-only であり Keycloak は起動時に DB 書込を要するため、DR 側 KC 常時起動（Scale 1）は**成立しない**ことが実機制約で判明（詳細 = [basic-design/08](../../../basic-design/08-availability-dr-design.md) §8.0.1 / D-U8-07、ADR-051 2026-07-23 正式改訂）。**Tier 1（RTO 30 分）はパイロットライトでは不成立のため Phase 2 検討**（U8 §8.4.4）。DNS・エッジ DR 切替は他組織要求仕様 REQ-DR-01〜05（U8 §8.4.5）。
>
> ~~Active-Passive Warm Standby~~ **Active-Passive パイロットライト** + Aurora Global + KMS MRK で SOC 2 A1.2 / PCI DSS §12.10 / ISO 22301 / DORA を 1 つの設計で同時充足。

#### §C-7.3.26.1 Region 配置

| Region | 役割 | Aurora | ~~EKS~~ **ROSA HCP**（2026-07-24 基本設計反映）| DR Replica |
|---|---|---|---|---|
| **ap-northeast-1（東京）** | Primary | Writer + 2 Reader | Pod 数 = [U6 §6.5](../../../basic-design/06-infra-network-design.md) 参照（~~6 Replicas~~）| — |
| **ap-northeast-3（大阪）** | DR | Secondary（Read-Only、Warm）| ~~1 Replica（Warm Standby）~~ **KC Scale 0（パイロットライト、インフラのみ Warm — [U8 §8.6](../../../basic-design/08-availability-dr-design.md) D-U8-11）**| ✅ |

#### §C-7.3.26.2 Tier 別 RTO / RPO

| Tier | RTO | RPO | 適用 |
|---|---|---|---|
| Tier 1 Premium（規制業種）| ~~**30 分**~~ | ~~**1 分**~~ | ~~金融 / 医療 / 公的機関 / DORA~~ **Phase 1 は提供しない（パイロットライトでは RTO 30 分不成立、Phase 2 検討 — 2026-07-24 基本設計反映: [basic-design/08 §8.4.4](../../../basic-design/08-availability-dr-design.md)）** |
| Tier 2 Standard | **1 時間**（条件付き成立 — [U8 §8.4.3 D-U8-09](../../../basic-design/08-availability-dr-design.md)）| **1 分** | デフォルト |
...
| Keycloak Realm 設定 | ~~**GitOps（Terraform）+ Realm Export 日次自動 → S3 → DR Import**~~ → **Realm Export は全用途で禁止（U2 §2.7.4）。復元 2 経路に全面差替**: **① リージョン障害 = Aurora Global 一体復元**（Realm 構成は DB に一体複製、Promote のみで復元完了）/ **② 論理破壊 = Aurora PITR + 差分 IaC 再生**（基盤層 Terraform + テナント層オンボーディングパイプライン。RPO 24h → 5min に改善）。整合性検証 = IaC ドリフト検知（Git ⇔ 稼働 KC 突合）（2026-07-24 基本設計反映: 詳細 = [basic-design/08](../../../basic-design/08-availability-dr-design.md) §8.3 D-U8-06/07、ADR-051 2026-07-23 正式改訂）|
...
| 障害 | 自動 / 手動 |
|---|---|
| 単一 AZ / Multi-AZ / ~~EKS~~ **ROSA HCP** Cluster（2026-07-24 基本設計反映: [basic-design/08 §8.1](../../../basic-design/08-availability-dr-design.md)）| **自動** |
...
| L5 ~~EKS~~ **ROSA HCP** HPA | CPU 70% で Pod 自動拡張（2026-07-24 基本設計反映: HPA/PDB 方針 = [basic-design/08 §8.1.2 D-U8-02](../../../basic-design/08-availability-dr-design.md)）|
...
| 場所 | 役割 |
|---|---|
| ~~**EKS**~~ **ROSA HCP** DaemonSet + **infra Pool Deployment** | Pod メトリクス + Trace 受信（KC Pool taint への toleration 必須。2026-07-24 基本設計反映: OTel Collector 配置 = [basic-design/09 §9.1](../../../basic-design/09-operations-observability-design.md)。ADR-053 の「Cross-Acct Collector（ECS Fargate）」は ROSA 内 infra Pool 配置に置換）|
...
flowchart LR
    ...
    subgraph AP["🟠 Broker / IdP-KC Acct（旧 Auth Platform Acct、2026-07-24 更新）"]
        SCIM[自作 SCIM Facade<br/>旧 Phase Two plugin<br/>U3 D3-11]
...
| ~~**SCIM 2.0 Server**（Phase Two）~~ **自作 SCIM Facade**（2026-07-24 基本設計反映: [basic-design/03 §3.5 D3-11](../../../basic-design/03-identity-provisioning-design.md)。D1 経路 = IdP-KC Acct 側受信）| 🟠 ~~Auth Platform Acct~~ Broker / IdP-KC Acct | 人事 DB からの SCIM Push 受信 |
| **Keycloak User Attribute（主）**| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（2026-07-24 基本設計反映: idmap DDL = [basic-design/03 D3-03](../../../basic-design/03-identity-provisioning-design.md)）| 統合 ID + 各システム ID マッピング集約 |
| **Aurora 補助テーブル（補助 = `idmap`）**| 🟠 ~~Auth Platform Acct~~ **Broker Acct**（Aurora 別 DB に暫定配置 — [basic-design/06 §6.4](../../../basic-design/06-infra-network-design.md)）| 多属性時の補助（Phase 2 候補）|
| **ユーザ管理画面** | 🟠 ~~Auth Platform Acct~~ **Broker Acct**（バックエンド。2026-07-24 基本設計反映）| 人事 DB 非カバー分の手動補完（派遣 / 業務委託 / ゲスト）|
...
> **🆕 2026-07-24 基本設計反映（読み替え注記）**: 以下の各図のアカウント表記「🟠 Auth Platform Acct」は **Broker Acct / IdP-KC Acct（分割後）**、「EKS」は **ROSA HCP** に読み替える。最新の経路設計（フロント/バックチャネル分離・PrivateLink・EventBridge クロスアカウント）は [basic-design/06 §6.1.2/§6.3](../../../basic-design/06-infra-network-design.md) 参照。
...
flowchart LR
    ...
    subgraph Receive["📥 受信 SCIM (本基盤)"]
        SCIMServer["自作 SCIM Facade<br/>(旧 Phase Two plugin、U3 D3-11)"]
...
| 項目 | 推奨 |
|---|---|
| アカウント境界 | **太枠 + アカウント色**で明示（~~🟠 Auth Platform~~ **🟠 Broker + 🟠 IdP-KC** / 🟢 App / 🔵 Audit / 🟣 ネットワーク監査〔他組織〕/ 🔷 ネットワーク〔他組織〕— 2026-07-24 基本設計反映: 6 アカウント体系 [basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md)）|
...
- **🆕 基本設計との分担（2026-07-24）**: 本章 = 決定要約 + 参照リンクの SSOT、**詳細設計値（サイジング / IAM / 経路 / Runbook / IaC 分割等）の SSOT = [doc/basic-design/](../../../basic-design/00-basic-design-plan.md) 各冊**。basic-design 側の決定（D-Ux-nn）変更時は本章の該当要約を追随更新する
...
| 日付 | 内容 |
|---|---|
| **2026-07-24** | **基本設計 Wave 1-3 反映の一括改訂**（冒頭改訂サマリ参照）: EKS → ROSA HCP + RHBK Operator / 5 → 6 アカウント体系（Broker・IdP-KC 分割 + NW 監査 Acct 他組織管理 P-18、要求仕様 REQ-IN/OUT/DR）/ CI/CD = GitHub Actions + OpenShift GitOps + ECR / DR = パイロットライト + Realm Export 全廃（復元 2 経路）/ Workload Identity = IRSA / `legacy_user_id` 廃止 / Turnstile 前倒しトリガー / IaC 2 層分離 / 自作 SCIM Facade。詳細委譲先 = [basic-design/00](../../../basic-design/00-basic-design-plan.md)〜10 |
| 2026-06-24〜25 | 設計レビュー Phase 1-6 反映（冒頭履歴ブロック参照）|
| 〜2026-06-23 | ADR-001〜055 反映の逐次更新 |

---

## §C-7.9 関連ドキュメント

### §C-7.9.0 基本設計（doc/basic-design/、2026-07-24 追加 — 詳細設計値の委譲先）

- [00-basic-design-plan.md](../../../basic-design/00-basic-design-plan.md) — 基本設計 計画書（暫定前提 P-01〜 / 単元分割 U1〜U10）
- [01-architecture-baseline.md](../../../basic-design/01-architecture-baseline.md) — Baseline v1（P-01〜P-18 凍結、6 アカウント体系）
- [02-keycloak-logical-design.md](../../../basic-design/02-keycloak-logical-design.md) — U2 Keycloak 論理設計（Realm / Flow / SPI / Realm Export 禁止）
- [03-identity-provisioning-design.md](../../../basic-design/03-identity-provisioning-design.md) — U3 ID・プロビジョニング（idmap / SCIM Facade D3-11 / ライフサイクル）
- [04-auth-ux-design.md](../../../basic-design/04-auth-ux-design.md) — U4 認証 UX
- [05-token-session-authz-design.md](../../../basic-design/05-token-session-authz-design.md) — U5 トークン・セッション・認可
- [06-infra-network-design.md](../../../basic-design/06-infra-network-design.md) — U6 インフラ・NW（ROSA HCP × 2 / Machine Pool / Aurora / 要求仕様 REQ-IN/OUT）
- [07-security-compliance-design.md](../../../basic-design/07-security-compliance-design.md) — U7 セキュリティ（KMS / ITDR / Workload Identity §7.5 / Bot 分離 D-U7-17）
- [08-availability-dr-design.md](../../../basic-design/08-availability-dr-design.md) — U8 可用性・DR（パイロットライト / 復元 2 経路 D-U8-06/07 / REQ-DR）
- [09-operations-observability-design.md](../../../basic-design/09-operations-observability-design.md) — U9 運用・監視・IaC（GitHub Actions + GitOps + ECR D-U9-12 / IaC 2 層 D-U9-09/10）
- [10-integration-migration-design.md](../../../basic-design/10-integration-migration-design.md) — U10 統合・移行（idmap API / D-U10-12）
...
| [architecture-v2-overview.drawio](../../../common/drawio/architecture-v2-overview.drawio) | **5 アカウント体系全体俯瞰**（5 Swim Lane + 主要コンポーネント + 凡例）**⚠ 2026-07-24 注記: 6 アカウント体系（[basic-design/01 §1.2](../../../basic-design/01-architecture-baseline.md)）への更新が未反映（要改版）** |
