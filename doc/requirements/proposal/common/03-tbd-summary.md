...
| 12a | [§FR-7.4.0 / §FR-2.1 B'](../fr/07-user.md#fr-740-scim-の位置づけと本基盤のスタンス) | **顧客 IdP の SCIM Provisioning 対応** (Q1) — Entra P1+ / Okta / 自社製 / なし | 採用顧客 → 自動 deprovisioning、未対応 → JIT のみ |
| 12b | 同上 | **顧客の SCIM 連携採用意思** (Q2) — IdP ライセンス・設定コスト負担を認識した上で | 連携設定の有無、オンボーディング工数 |
| 12c | 同上 | **SCIM 不採用時の deprovisioning 責任所在** (Q4) — 顧客責任 / 弊社定期バッチ | 契約条件 + [§NFR-6.5 D-3](../nfr/06-operations.md) 運用差 |
