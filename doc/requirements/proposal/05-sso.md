...
> カバー範囲: FR-SSO §4.1 SSO（SSO はここ、ログアウト・セッション管理は [§6](06-logout-session.md)）
...
| **クロステナント SSO** | テナント境界で意図的に切断（[§3.3.C](03-federation.md#33c-マルチテナント環境での-sso-挙動) 参照）|
...
flowchart LR
    ...
    S3["§3 フェデレーション"]
    S4["§4 MFA"]
    S5["§5 SSO ← イマココ<br/>ログインの広がり"]
    S6["§6 ログアウト・<br/>セッション管理<br/>ログインの終わり方"]
...
SSO は「**一度のログインで複数システムを利用できる**」UX を提供する核機能。Broker パターン（[§10/§11 アーキテクチャ](11-architecture.md)）の前提でもある。ログアウト・セッション管理は **§6 で対** として扱う（性質が逆向き：SSO は広がり、ログアウトは終わり）。
...
| サブセクション | 内容 | 関連 FR |
|---|---|---|
| §5.1 同一 IdP 内 SSO | 同じ Pool/Realm 内のアプリ間 SSO | FR-SSO-001 |
| §5.2 クロス IdP SSO | フェデレーション IdP 経由の SSO 伝播 | FR-SSO-002 |

---

## §5.1 同一 IdP 内 SSO（→ FR-SSO-001）

> **このサブセクションで定めること**: 同じ顧客テナント内の複数アプリ間で SSO セッションを共有する範囲とテナント境界の遮断ルール。
> **主な判断軸**: SSO で繋ぐシステム範囲、同一テナント内でも切り離したいシステムの有無
> **§5 全体との関係**: §5.1 = テナント内 SSO、§5.2 = テナント跨ぎ SSO（フェデ）。マルチテナント挙動の詳細は [§3.3.C](03-federation.md#33c-マルチテナント環境での-sso-挙動)
...
| SSO セッション TTL 設定 | ✅ App Client 設定 | ✅ Realm 設定 | [§6.3 セッション管理](06-logout-session.md#63-セッション管理) で詳述 |
...
| クロステナント | **遮断**（tenant_id クレームベース、[§3.3.C](03-federation.md#33c-マルチテナント環境での-sso-挙動) 参照）|
| SSO セッション保持期間 | [§6.3 セッション管理](06-logout-session.md#63-セッション管理) 参照 |
...
> **§5 全体との関係**: §5.1 がテナント内 SSO、§5.2 が**外部 IdP との SSO セッション伝播**。MFA 重複回避は [§3.2.3](03-federation.md#323-mfa-重複回避--fr-fed-012) と整合
...
|---|---|
| **絶対安全** | 外部 IdP の認証 assertion を検証（[§3.2.3 MFA 重複回避](03-federation.md#323-mfa-重複回避--fr-fed-012) と整合）|
...
| 外部 IdP の MFA 主張尊重（amr / AuthnContext） | ⚠ Pre Token Lambda 個別実装 | ✅ Conditional OTP 標準 | [§3.2.3](03-federation.md#323-mfa-重複回避--fr-fed-012) |
...
| 外部 IdP の MFA 主張 | **信頼**（[§3.2.3](03-federation.md#323-mfa-重複回避--fr-fed-012)）|
