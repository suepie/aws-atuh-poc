...
本資料および [../hearing-checklist.md](../hearing-checklist.md)（全 124 項目、§0〜§5 構造）の検討事項は、以下の **4 軸を交差させて抽出**している。1 軸だけでは漏れが出るため、複数軸が同じ項目に行き着くかをチェックしながら積み上げる方針を採った。
...
**軸 4: ステークホルダー視点（Phase A / B / C / D は項目タグとして保持）**
...
> **構造再編成**（2026-05-25）: [../hearing-checklist.md](../hearing-checklist.md) は **§0〜§5（subject-matter 軸）**に再編成済（旧 Phase A/B/C/D の章分けから移行）。Phase タグは **項目に保持**し、会議組み立てに使う 2 つ目の軸として併用：
>
> - **subject-matter 軸（主軸）**: §0 全般 / §1 前提合意 / §2 サービス要件 / §3 既存接続元 / §4 技術仕様 / §5 NFR — 「何を決めるか」で分類
> - **ステークホルダー軸（タグ）**: `[A]` `[B]` `[C]` `[D]` — 「誰に聞くか」で分類
>
> hearing-script/ 配下の各ファイル（00-common.md / 01-auth-flow.md 等）は会議組み立て用に **旧 Phase 軸**でファイル分割を維持しつつ、各ファイル冒頭に **新 §X.Y 構造との対応マップ**を記載している。
...
124 項目のうち **🔥 最優先 37 件** をプラットフォーム選定（Cognito vs Keycloak、ティア選定）に直結する判断材料として Stage 1 前半で先行確認する。**ヒアリングの最上位順序**:

1. **§1 前提合意 6 件すべて**（A-5-2/3/4、A-11、A-11-α、D-6）— 後続全質問の範囲を規定
2. **§3.1〜§3.3 の 3 大マスター表**（B-200 弊社 IdP / B-200-B 顧客 IdP / B-100 アプリ）— 「弊社 × 顧客 × アプリ」マトリクス完結で Cognito vs Keycloak が事実上確定
3. **§2.1〜§2.2 の事業・規制要件**（A-1〜A-4、A-8、A-9、C-201、C-202）— コスト試算とコンプラ範囲確定の入力

Phase タグ別の担当者分離（`[A]` 20 件 / `[B]` 63 件 / `[C]` 35 件 / `[D]` 6 件）により、**1 人あたりの回答負担は 20〜60 項目程度**に抑える設計。回答形式も `Yes/No` / `選択肢` / `具体値` で事前定義し、自由記述で考え込ませない。
