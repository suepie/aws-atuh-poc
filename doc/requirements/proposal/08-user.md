...
| **フェデユーザー** | 外部 IdP（Entra ID / Okta 等）から JIT で本基盤に作成されたユーザー（[§3.2.1 JIT](03-federation.md#321-jit-プロビジョニング--fr-fed-008)）|
| **CRUD** | Create / Read / Update / Delete の基本ユーザー操作 |
| **SCIM 2.0**（System for Cross-domain Identity Management）| ユーザーライフサイクル自動化の業界標準プロトコル |
| **JIT プロビジョニング** | SSO 初回ログイン時の自動ユーザー作成（[§3.2.1](03-federation.md#321-jit-プロビジョニング--fr-fed-008)）|
...
flowchart LR
    S2["§2 認証<br/>(パスワード)"]
    S3["§3 フェデレーション<br/>(JIT)"]
    S7["§7 認可<br/>(クレーム発行)"]
    S8["§8 ユーザー管理 ← イマココ<br/>誰を基盤で管理するか"]
    S9["§9 管理機能<br/>(管理者操作)"]
...
**[§7](07-authz.md) で「認証基盤は最小限」のスタンスを採用**した。それは認可だけでなく、**ユーザー管理の範囲（=基盤で何を保持するか）にも同じ原則が適用される**。本章で「基盤が持つユーザー情報の範囲」と「各アプリが持つユーザー情報の範囲」の責務分界を明確化する。

### §8.0.A 本基盤のユーザー管理スタンス（§7 と整合）
...
| サブセクション | 内容 | 関連 FR |
|---|---|---|
| §8.1 ユーザー CRUD | 基本操作・検索・有効化/無効化・削除 | FR-USER-001, 005, 006, 011 |
| §8.2 属性・ロール | 基盤が持つ属性の範囲 / ロール定義 | FR-USER-002, 007, 008 |
| §8.3 セルフサービス | ユーザー自身による操作（招待・プロフィール編集） | FR-USER-004, 012 |
| §8.4 プロビジョニング | SCIM / バルクインポート / 管理者操作 | FR-USER-003, 009, 010 |

---

## §8.1 ユーザー CRUD（→ FR-USER §6.1）
...
> **§8 全体との関係**: §8.1 = 「**操作**」、§8.2 = 「持つ属性」、§8.3 = 「ユーザー自身の操作」、§8.4 = 「外部からの自動投入」
...
| 削除時の関連データ | **基盤内データ Cascade**（[§3 フェデユーザーリンク](03-federation.md) / [§7 ロール](07-authz.md) 含む）|
...
> **§8 全体との関係**: §8.0.A「**基盤は最小限保持**」スタンスの具体化。[§7.1 JWT クレーム発行](07-authz.md#71-認証基盤が発行する-jwt-クレーム--fr-authz-51) と保持属性が連動
...
| **どんなアプリでも** | 必要最小限のクレームを発行（[§7.1](07-authz.md#71-認証基盤が発行する-jwt-クレーム--fr-authz-51)）、業務属性はアプリで保持 |
| **効率よく** | Progressive Profiling、必要時に取得 |
| **運用負荷・コスト最小** | カスタム属性は要件次第。デフォルトは最小 |

### 基盤が保持する属性の 3 段階（[§7.1](07-authz.md#71-認証基盤が発行する-jwt-クレーム--fr-authz-51) と整合）

| 段階 | 属性 | 採用判断 |
|---|---|---|
| **A. 最小（Must）**| `sub`、`email`、`tenant_id`、`password_hash`（ローカルユーザーのみ） | 全顧客 Must |
| **B. 認証拡張（Should）**| `roles`（[§7.1](07-authz.md#71-認証基盤が発行する-jwt-クレーム--fr-authz-51) パターン B 選択時）、`name`（UI 表示用）| 採用パターン次第 |
...
| ロール採用判断 | [§7 認可](07-authz.md) で顧客選択パターンに依存 |
...
> **§8 全体との関係**: §8.1 が管理者操作中心、§8.3 が**ユーザー自身による操作**。管理者負荷削減の核
...
| MFA セルフ登録 | **Must**（[§4.1 MFA 要素](04-mfa.md#41-mfa-要素--fr-mfa-31) で詳述）|
| パスワードリセット | **Must**（[§2.2 ローカル PW](02-auth.md#22-パスワードローカルユーザー管理-fr-auth-12) で詳述）|
...
> **§8 全体との関係**: §8.1 が個別操作、§8.4 は**自動化・大量処理**。JIT は [§3.2.1](03-federation.md#321-jit-プロビジョニング--fr-fed-008) と整合
...
**プロビジョニング方式の使い分け**:
- **JIT**: 初回 SSO 時の自動作成（[§3.2.1](03-federation.md#321-jit-プロビジョニング--fr-fed-008)）
...
|---|:---:|:---:|---|
| JIT プロビジョニング | ✅ | ✅ | [§3.2.1](03-federation.md#321-jit-プロビジョニング--fr-fed-008) |
...
| 項目 | ベースライン |
|---|---|
| JIT プロビジョニング | **Must**（[§3.2.1](03-federation.md#321-jit-プロビジョニング--fr-fed-008)）|
