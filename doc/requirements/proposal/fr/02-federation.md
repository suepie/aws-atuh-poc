...
##### email 非保有顧客向けの周知差分（2026-08-03 追記、是正 3 / FC-1・FC-2）

上記 6 変化のうち **変化 1（PW 再設定）・変化 2（MFA 再登録）は「メール配信」を暗黙前提**にしており、**工場・現場系の email 非保有顧客ではそのまま成立しない**。email 保有前提の標準版とは別に、以下の**配信チャネル置換版**を適用する:

| 変化 | 標準版（email 保有） | **email 非保有版** |
|:---:|---|---|
| 1 PW | リセットメール → リンク | **① 方式② User Storage SPI で強制リセット自体を回避（第一選択、[ADR-019](../../adr/019-existing-system-migration.md) §B）** / ② 不可時は**管理者が初期 PW・ワンタイムコードを対面/帳票で配布**（[ADR-038](../../adr/038-tenant-admin-portal.md) §C 資格情報配布モード）|
| 2 MFA | QR をメール案内 | **現地登録会（TOTP QR 掲示 / Passkey 登録端末を用意 / Push 端末セットアップ）**。回復手段は Recovery Codes（紙）・Admin Reset（[ADR-018](../../adr/018-user-identifier-3layer-emailless.md) Account Recovery、[B-IDM-6](../../hearing-checklist.md)）|
| 3〜6 | 変更なし | HRD は email ドメイン（案 A）不可 → **D 識別子先行 + C 組織固有 URL + B セレクタ**（§FR-2.3.3.E）|

- **周知チャネルも置換**: 「顧客 IT 担当者経由メール（必須）」→ email 非保有ユーザーには届かないため、**顧客管理者経由の現地掲示・朝礼周知・作業手順書配布**を必須チャネルとする。
- **設計判断は既に確定**（方式②・回復手段・HRD の D/C/B）しており、本差分は**配信手段の置換のみ**。顧客が email 非保有か否かは B-IDM-1 / B-IDM-10 で判定し、非保有なら本差分版のチェックリストを適用する。
...
### §FR-2.4.B-2 既存 sys_user 保持ルール（履歴保全の絶対要件、2026-07-03 追加）

> **重要**: ServiceNow 移行時に「既存ローカルユーザーを削除して SSO で作り直す」アプローチは**絶対禁止**。ServiceNow の全業務レコード（インシデント / タスク / 承認 / 監査ログ）は `sys_user.sys_id` を外部キーとして参照しており、sys_id が変わると過去履歴すべてが「不明ユーザ」化する。

#### 保持ルール（Must）

| ルール | 内容 |
|---|---|
| **R1: sys_user レコード非削除** | 既存 ServiceNow ユーザーの `sys_user` レコードは移行時に**削除しない**。SSO は Matching Field でリンクする方式で実装 |
| **R2: sys_id 不変性** | Matching Field で既存 `sys_user` にマッチした場合、その `sys_id` を維持したままセッション作成（JIT は matching miss 時のみ発火）|
| **R3: Matching Field 事前確定** | 移行前に Matching Field を確定し、移行後は変更しない（変更する場合は sys_user 統合作業が必要）|
| **R4: 並走期間の維持** | password auth を無効化する前に、全ユーザが SSO ログイン成功したことを `login_history` で確認 |
| **R5: 退職者の非削除** | 退職 = `active=false` 化のみ。sys_user 削除は原則不可（履歴壊れる）|

#### Matching Field 選定（第一推奨: `employee_number`）

本基盤の識別子形式 `<tenant>-<userid>` （[ADR-055](../../../adr/055-hrd-implementation-method-selection.md)）と整合させ、`userid` 部分 = 社員番号を SAML 属性として送出、ServiceNow 側 `employee_number` にマッチ:

```
Keycloak SAML Assertion:
  NameID: "acme-EMP-001234"
  attribute employee_number: "EMP-001234"

ServiceNow SSO:
  User Field = "employee_number"
  → sys_user{employee_number="EMP-001234"} を検索・リンク（sys_id 不変）
```

判断フローと詳細手順は **[reference/servicenow-sso-user-linking-guide.md](../../../reference/servicenow-sso-user-linking-guide.md)** 参照。

#### 影響を受ける主要テーブル（削除するとどこが壊れるか）

`sys_user.sys_id` を参照する主要テーブル:

| テーブル | フィールド | 影響 |
|---|---|---|
| `incident` | `caller_id` / `assigned_to` / `opened_by` | 全インシデント履歴 |
| `problem` / `change_request` / `sc_request` / `task` | `assigned_to` 等 | 業務履歴 |
| `sysapproval_approver` | `approver` | 承認ワークフロー履歴 |
| `sys_journal_field` | `sys_created_by` | Journal コメント履歴 |
| `sys_audit` | `user` | 監査ログ |
| `sys_user_grmember` / `sys_user_delegate` | `user` / `delegate` | グループ所属 / 代理承認設定 |

→ **sys_user 削除 = 上記全テーブルの参照が Orphan 化 = 業務不能**

#### 並走期間 4 Phase

| Phase | 内容 | 期間目安 |
|---|---|---|
| **Pilot** | 10-20 名で SSO 動作確認 + 履歴保全確認 | 2-4 週間 |
| **全社 SSO 有効化** | 全ユーザに sso_source 設定、PW 認証も並走 | 1-2 ヶ月 |
| **PW 段階無効化** | login_history で全員 SSO 成功確認後、一般ユーザの password 無効化 | 1 ヶ月 |
| **Break Glass 残置** | Break Glass 管理者 2-3 名のみ PW 認証残置、それ以外は SSO のみ | 継続運用 |

#### 特殊対応

| ケース | 対応 |
|---|---|
| **重複 sys_user 統合** | 移行前に検出・統合。参照テーブル洗い替え + inactive 化（削除は不可）|
| **退職者データ APPI Article 30 / GDPR RTBF** | 個人特定情報のみ匿名化（`first_name = "[Anonymized-xxxxxx]"` 等）、sys_id は保持 |
| **SSO 障害時** | Break Glass 管理者（`sso_source` 空欄 + Hardware MFA + IP 制限）でローカル PW ログイン |

詳細手順・スクリプト例・テストシナリオは **[reference/servicenow-sso-user-linking-guide.md](../../../reference/servicenow-sso-user-linking-guide.md)** 参照。
...
| Matching Field 選定（2026-07-03 追加）| **B-SN-18** | **推奨: `employee_number`** / user_name / email / カスタム属性 + 埋込率 |
| 重複 sys_user 統合作業（2026-07-03 追加）| **B-SN-19** | 件数 + 統合方針 + 実施主体 |
| 並走期間の長さ（2026-07-03 追加）| **B-SN-20** | **推奨: 2-4 ヶ月** / 顧客ポリシー次第で短縮 or ビッグバン |
| 退職者データ匿名化要件（2026-07-03 追加）| **B-SN-21** | **推奨: 個人特定情報のみ匿名化** / active=false のみ / 完全削除 |
