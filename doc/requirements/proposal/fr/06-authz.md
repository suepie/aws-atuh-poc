...
> **このサブ・サブセクションで定めること**: 「最小限のクレーム」を採用した場合の具体的な JWT 構造、特に **`aud` / `azp` / `client_id` の使い分け（接続元アプリの表現）** と、Cognito の `aud` 罠の対処。   
> **主な判断軸**: PII 漏洩リスク最小化、トークンサイズ抑制、認可機能の保持   
> **§FR-6.1 内の位置付け**: §FR-6.1 の「3 段階クレーム」を**実際の JWT 構造に落とし込む**サブセクション。`aud` / `azp` / `client_id` の OAuth/OIDC 標準準拠の使い分けを明示
...
#### 「接続元アプリ」を JWT で表現する：`aud` / `azp` / `client_id` の使い分け
...
| **`aud`** | ❌ 削るな（Cognito Access Token は注意、後述）| token confused deputy 攻撃 |
| **`exp`** | ❌ 削るな | 永久トークン化 |
| **`tenant_id`** | ❌ 削るな（B2B SaaS では必須）| テナント分離崩壊、IDOR 攻撃 |
| **`azp`** | △ aud が単一なら削れる | 接続元アプリの追跡不能。**推奨は残す** |
| **`scope`** | △ 単一スコープ運用なら削れる | 最小権限の表現不能 |
...
| **`amr` / `acr`** | △ MFA 重複回避（[§FR-2.2.3](02-federation.md)）使わないなら削れる | 認証強度の伝達不能 |
...
Keycloak は標準で `aud` を埋めるため、この罠はない（[Audience Mapper](https://www.keycloak.org/docs/latest/server_admin/#_audience) で複数 `aud` も可能）。

#### 接続元アプリ表現の 3 つのユースケース
...
| 項目 | ベースライン |
|---|---|
| **デフォルト発行クレーム** | **Stage 1**: `iss` / `sub` / `aud` / `azp` / `tenant_id` / `exp` / `iat` |
| **接続元アプリ表現** | `azp` を使う（OIDC 標準、Validator 対応）|
| **`aud` の単一/複数** | 単一を基本、複数 API 横断時のみ配列 + `azp` 必須化 |
| **PII の扱い** | **JWT に入れない**（email / name 等は userinfo / DB 参照）|
| **Cognito 採用時の対応** | Pre Token Lambda V2 で `aud` / `azp` 注入（**Essentials+ 必須**） |
| **マイクロサービス時の拡張** | Stage 2 へ昇格（scope / client_id / auth_time / jti 追加）|
| **RBAC 採用時の拡張** | Stage 3 へ昇格（roles 追加）|
...
| 接続元アプリ表現の要否（`azp`）| 必要（複数 SPA / モバイル / BFF 等）/ 不要（単一 SPA のみ）|
| PII を JWT に入れるか | 入れない（推奨）/ email のみ入れる / 全部入れる |
| 多重 `aud` の要否 | 必要（API Gateway で fan-out）/ 不要（1:1 マッピング）|
