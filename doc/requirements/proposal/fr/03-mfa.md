...
| **P-3 現行で IdP があった従業員** | フェデ | フェデ側 | 全シナリオで対象（最大ボリューム）|
| **P-4 現行で IdP がなかった従業員**（旧 P-5 ゲスト/外部協力者 統合）| ローカル | **本基盤 MFA 強推奨** | α / β |

> **対象外**: 旧 P-6 B2C は本基盤対象外（B2B SaaS 専用）。
...
> **§FR-3 全体との関係**: §FR-3.1 = 「どの MFA 手段を備えるか」、§FR-3.2 = 「いつ MFA を要求するか（適用ポリシー）」、§FR-3.3 = 「**操作ごとに段階的に認証強度を引き上げるか**」（静的判定、操作の重要度ベース）、[§FR-3.6 Adaptive Authentication](#fr-36-adaptive-authenticationrisk-based-認証) = 「**コンテキストで動的にリスク評価**」（動的判定、IP/地理/履歴ベース）。**§FR-3.3 静的 + §FR-3.6 動的の補完二重防御**が業界標準（[ADR-026](../../../adr/026-aal-mismatch-stepup-mfa.md) + [ADR-034](../../../adr/034-adaptive-authentication.md)）
...
> **⚠ 2026-07-26 基本設計での方針改訂（[U4 D-U4-04](../../../basic-design/04-auth-ux-design.md) / [U7 §7.7.2](../../../basic-design/07-security-compliance-design.md)）**: 本節の案 3（amr 評価 + 不足時は基盤側 WebAuthn/TOTP 補完）のうち、**外部フェデユーザ（P-3）への基盤側補完は撤去**された。改訂後 = **MFA 実施は顧客 IdP の責任（契約条項）+ 基盤は `mfa_indicator` の検証・記録・監視のみ**。旧ケース B/C の基盤側 MFA は **IdP-KC 収容ユーザにのみ適用**。PCI DSS Req 8.4.2 を IdP 非実施顧客では充足できないリスクは明示の上での方針決定（契約前ゲート B-MFA-PCI-1）。以下の記述は歴史的検討として保持。
...
> **本サブセクションの位置付け**: §FR-3.4.0.A の方針を **利用者カテゴリ（P-1〜P-4、[§FR-1.2.0.0](01-auth.md#fr-1200-ローカルユーザーとは何か--利用者カテゴリ別の分析)）別** に具体化。フェデユーザー（A/B/C）+ ローカルユーザー（D）の **4 ケース完全整理** により、ユーザータイプごとの「持つ / 持たない」を明示する。
