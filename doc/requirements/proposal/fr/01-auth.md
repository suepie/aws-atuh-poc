...
| **ローカルユーザー** | フェデレーション IdP を経由せず、本基盤の User DB に直接登録されているユーザー |
...
| §FR-1.2.0 ローカルユーザー認証の主体 | 共通基盤集約 / 各アプリ独自 / ハイブリッドの選択 | — |
...
> **主な判断軸**: 基本方針整合（特に「絶対安全」「効率よく」「運用負荷低」）、既存システムの認証実装の有無、移行戦略
