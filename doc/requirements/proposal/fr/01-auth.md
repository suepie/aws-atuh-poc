...
> **このサブ・サブセクションで定めること**: 「ローカルユーザー」を素朴に扱う前に、**利用者カテゴリ P-1〜P-6（人間ユーザー）+ I-1〜I-5（インフラ運用）+ M-1〜M-5（M2M）+ T-1〜T-2（脅威）** を整理してから「どこまでをローカルにするか」のシナリオ（α/β/γ/δ）を選ぶ。あわせて「アプリ集約 3 選択肢（A 共通基盤集約 / B 各アプリ独自 / C ハイブリッド）」も確定する。
...
| **P-4. IdP なし顧客従業員** | 一部顧客 × 数十〜数百 | ローカル / 顧客に IdP を持ってもらう |
| **P-5. ゲスト / 外部協力者** | 不定・短期 | 招待 URL + ローカル / ソーシャル |
| **P-6. B2C エンドユーザー** | 不定 | ローカル + ソーシャル + Passkey |

#### 4 カテゴリ責務分担サマリ（詳細は ADR-029）

| カテゴリ | 認証提供主体 | 関連 |
|:---:|---|---|
| **A. 人間ユーザー（P-1〜P-6）** | 本基盤（Cognito / Keycloak）| §FR-1.2.0 / §FR-2 |
...
| シナリオ | 範囲 | 採用判断 |
|---|---|---|
| α 全カテゴリ | P-1〜P-6 全員ローカル可 | ❌ 不採用（要件混在で最適化困難）|
| β 管理者+IdPなし顧客 | P-1, P-2, P-4 ローカル可、P-3 はフェデ強制 | △ フォールバック |
| **γ 管理者のみ**（推奨）| P-1, P-2 のみローカル、P-3〜P-6 は IdP 経由 | ✅ **第一推奨** |
...
| **B** `external_id` / `preferred_username` | `ACME-EMP-0042` | 顧客 | 可（運用上）| 顧客向け表示、検索 |
