...
| **インフラ運用** | 顧客 IT | **弊社** |
| **CRUD 実施** | 顧客 IdP で | **弊社が提供する ユーザ管理画面（[ADR-038](../../../adr/038-tenant-admin-portal.md)）で顧客が実施** |
...
本章で扱う「ユーザー」は **利用者カテゴリ P-1〜P-4 すべてを含む**が、CRUD 規模や運用主体はカテゴリ・採用シナリオ次第で大きく変わる:
...
| **P-3 現行で IdP があった従業員** | 共通基盤（フェデユーザーレコード）| **顧客 IdP 側が真実**、本基盤は影**像のみ** | 顧客数 × 数百〜数千 |
| **P-4 現行で IdP がなかった従業員**（旧 P-5 ゲスト/外部協力者 統合）| 共通基盤（ローカルユーザー）| 顧客（委譲管理者、招待者）| シナリオ次第（γ では原則ゼロ）|
...
| **運用負荷・コスト最小** | Keycloak は アカウント設定画面 標準、Cognito はアプリ側で UI 実装 |

### 対応能力マトリクス

| 機能 | Cognito | Keycloak (OSS/RHBK) |
|---|:---:|:---:|
| セルフサービスプロフィール編集 | ⚠ アプリ側 UI 実装必要 | ✅ **アカウント設定画面**（標準）|
...
| 確認項目 | 回答例 |
|---|---|
| セルフサービス UI 提供方針 | 基盤標準 UI（Keycloak アカウント設定画面）/ アプリ側実装 |
...
| 経路 | 用途 |
|---|---|
| **メイン: ユーザ管理画面 経由**（[ADR-038](../../../adr/038-tenant-admin-portal.md)）| 顧客 DPO が主導、Shared Responsibility 整合 |
| データ主体権利申請画面（Phase 2）| `account.basis.example.com/rights`、エンドユーザー直接 |
| 法定書面（紙）| ユーザ管理画面 で手動登録 |
...
| 確認項目 | ヒアリング ID | 回答例 |
|---|---|---|
| データ主体権利申請画面 の必要性 | **B-DSAR-1** | Phase 1 から / Phase 2 / 不要 |
