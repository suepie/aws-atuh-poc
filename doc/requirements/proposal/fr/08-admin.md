...
> **⚠ 関連 ADR**: 顧客向け監査ログ提供（Customer Audit Support）は [ADR-036](../../../adr/036-customer-audit-support.md) / [§NFR-7.5](../nfr/07-compliance.md) で扱う。テナント管理者が UI で監査ログを閲覧する実装は [ADR-038 ユーザ管理画面](../../../adr/038-tenant-admin-portal.md) / [§FR-8.5](#fr-85-tenant-admin-portal顧客テナント管理者向け-admin-ui) Phase 1 機能セット。本サブセクションは「**何をログとして記録するか・保存するか**」、ADR-036/038 は「**それを顧客にどう見せるか**」を扱う
...
> **⚠ UI 実装**: テナント管理者が実際に使う UI は **[§FR-8.5 ユーザ管理画面](#fr-85-tenant-admin-portal顧客テナント管理者向け-admin-ui)（[ADR-038](../../../adr/038-tenant-admin-portal.md)）**。Keycloak Admin Console の直接開放は不可（業界実例なし）。本サブセクションは「**何を委譲するか**」、§FR-8.5 は「**どうやって UI を提供するか**」を扱う
...
## §FR-8.5 ユーザ管理画面（顧客テナント管理者向け Admin UI）

> **詳細は [ADR-038 ユーザ管理画面](../../../adr/038-tenant-admin-portal.md) を参照**
...
| **アクセス導線** | [サービス選択画面（ADR-021）](../../../adr/021-post-login-landing-ux.md) に「管理画面」タイル、admin ロール保有時のみ表示 |
...
## §FR-8.6 PAM / JIT 管理者権限管理（APPI / PCI DSS 準拠）

> **詳細は [ADR-040 PAM / JIT 管理者権限管理](../../../adr/040-pam-jit-admin-privilege-management.md) を参照**

> **このサブセクションで定めること**: §FR-8.3 で「JIT 管理者」「SoD」の概念を導入したが、**具体的な PAM 設計（特権アカウントのライフサイクル / 承認フロー / セッション記録 / Break-Glass）**は未確定だった。本節で 4 層モデル（L1 Break-Glass / L2 インフラ / L3 アプリ / L4 テナント特権）を確定。
> **主な判断軸**: PCI DSS v4.0 §7 / §8 / §10 + APPI 第 23 条 安全管理措置の同時充足、外部 PAM 製品（CyberArk 等）の要否
> **§FR-8 全体との関係**: §FR-8.3 JIT 概念の**実装層**、§FR-8.5 ユーザ管理画面 の**特権操作部分の実現方式**

### 結論サマリ

| 項目 | 採用方針 |
|---|---|
| **特権ロール定義** | 常時付与禁止、`<role>-eligible` / `<role>-active` の 2 状態モデル |
| **基盤管理者の昇格** | AWS IAM Identity Center + Session Manager + 承認（SoD：申請者 ≠ 承認者）|
| **テナント管理者の破壊的操作** | ユーザ管理画面 内 JIT 承認ワークフロー |
| **セッション記録** | Session Manager → CloudWatch Logs / Keycloak Admin Events → Audit Acct |
| **Break-Glass** | 物理金庫 + FIDO2 + 2 名同時 + 自動アラート + 24h 期限 |
| **承認 SLA** | 通常 4h / 緊急 15min（Pager 起動）|
| **定期レビュー** | 半年ごと（PCI DSS 7.2.4）|

### 4 層 PAM モデル（要点）

| 層 | 対象操作 | 承認者 | 規制対応 |
|---|---|---|---|
| **L1 Break-Glass** | 全システム障害時の最高権限 | 物理鍵 2 名 + 役員 | PCI DSS 8.2.2 / APPI 安全管理 |
| **L2 インフラ層** | AWS Console / kubectl / DB | Manager 1 名 + Security 1 名 | PCI DSS 10.2.1 |
| **L3 アプリ層** | Keycloak Realm / IdP 設定 | 認証基盤 Lead | PCI DSS 10.2.1 |
| **L4 テナント特権** | 全ユーザー削除 / 全 MFA リセット | 顧客側 SoD 承認者 | APPI 第 23 条 |

### 規制要件マッピング

| 規制 | 条項 | 本節での充足方法 |
|---|---|---|
| PCI DSS v4.0 | 7.2.4 / 7.2.5 — 特権アカウント定期レビュー | 半年ごと Access Certification |
| PCI DSS v4.0 | 8.2.2 — 共有アカウント禁止 | 個人 ID + JIT 昇格 |
| PCI DSS v4.0 | 8.6.1 / 8.6.2 — システム / アプリアカウントの hardcode 禁止 | Workload Identity（§FR-8.7 / ADR-041）|
| PCI DSS v4.0 | 10.2.1 / 10.3 — 全特権操作の改ざん不能監査 | Session Manager / Admin Events + S3 Object Lock |
| APPI 第 23 条 | 安全管理措置 | アクセス権限ライフサイクル + 監査ログ |

### 採用しない選択肢

- **CyberArk PAM 製品**: 年 $200K+ で過剰、AWS IIC + Session Manager で代替
- **管理者常時付与**: PCI DSS §7 / §8 違反
- **Break-Glass 廃止**: 全 PAM 障害時の救済手段が消失

### TBD / 要確認

| 確認項目 | ヒアリング ID | 回答例 |
|---|---|---|
| 規制業種顧客の特定 | **B-PAM-1** | 金融 / 医療 / 自治体 / なし |
| 承認 SLA 要件 | **B-PAM-2** | 通常 4h / 1h 厳守 / 緊急 15min |
| 承認者の最小人数 | **B-PAM-3** | 2 名 / 3 名 / 役員必須 |
| Break-Glass 訓練頻度 | **B-PAM-4** | 半年 / 四半期 / 月次 |
| 顧客テナント側 SoD 必須化 | **B-PAM-5** | 必須 / 推奨のみ / 顧客判断 |
| セッション記録保管期間 | **B-PAM-6** | 1 年 / 3 年 / 7 年（PCI DSS）|
| 外部 PAM 製品連携 | **B-PAM-7** | 不要 / 顧客側 CyberArk 連携 / 弊社導入 |

---

## §FR-8.7 Workload Identity（サービス間認証）

> **詳細は [ADR-041 Workload Identity 設計](../../../adr/041-workload-identity-spiffe.md) を参照**

> **このサブセクションで定めること**: 顧客打ち合わせで「マイクロサービス前提はあった方が良い」と確認済み（[project-coverage-audit-2026-06-18](../../../../.claude/projects/-Users-suepie-Develop-10-project-aws-atuh-poc/memory/project_coverage_audit_2026-06-18.md) Phase B 案 K）。**サービス間 M2M 認証**を Service Account + Client Secret 方式から脱却し、Pod Identity + Federated Identity Credentials で Secret ゼロ化。
> **主な判断軸**: PCI DSS v4.0 §8.6.1 / §8.6.2（hardcode 禁止）対応、SPIFFE/SPIRE 採用タイミング
> **§FR-8 全体との関係**: §FR-8.6 PAM のヒト側 Zero Trust に対する**マシン側 Zero Trust** 補完

### 結論サマリ

| 用途 | 採用方式 | Phase |
|---|---|---|
| EKS Pod → AWS リソース | **EKS Pod Identity**（2024 GA、IRSA 後継）| Phase 1 |
| ECS Task → AWS リソース | Task Role（既存）| Phase 1 |
| Lambda → AWS リソース | Execution Role（既存）| Phase 1 |
| アプリ → Keycloak（M2M）| **Federated Identity Credentials**（K8s SA JWT → Keycloak Token Exchange、client_secret 廃止）| Phase 1 |
| Cross-Acct アプリ間通信 | mTLS + Service Mesh or VPC Endpoint + IAM | Phase 1 |
| SPIFFE/SPIRE 全面採用 | SPIRE Server + SDS | **Phase 2** 候補 |

### 採用方針の核

- **Secret ゼロ化**: K8s SA Token（自動配布、1h 自動更新）→ Keycloak Token Exchange で完結
- **PCI DSS §8.6 同時充足**: hardcode 禁止 / 対話的利用不可 / 自動ローテーション
- **AWS 標準採用**: Pod Identity + Federated Identity Credentials、追加製品ゼロ
- **Phase 2 拡張**: マイクロサービス 50 個超 / マルチクラウド時に SPIFFE/SPIRE 採用

### TBD / 要確認

| 確認項目 | ヒアリング ID | 回答例 |
|---|---|---|
| マイクロサービス想定数 | **B-WL-1** | 10 / 30 / 50+ |
| EKS 採用予定 | **B-WL-2** | あり / なし（ECS / Lambda のみ）|
| Cross-Acct アプリ通信頻度 | **B-WL-3** | 多い / 少ない / なし |
| Service Mesh 導入 | **B-WL-4** | Phase 1 / Phase 2 / 不採用 |
| SPIFFE 採用 | **B-WL-5** | Phase 1 / Phase 2 / 不採用 |
| 外部 Partner API 連携 | **B-WL-6** | mTLS / OAuth / 両方 / なし |
