...
| **テナント** | 顧客企業 1 社の単位（[§3.3](03-federation.md#33-マルチテナント運用--fr-fed-23) 参照）|
...
```mermaid
flowchart LR
    S3["§3 フェデレーション<br/>(IdP 設定)"]
    S8["§8 ユーザー管理"]
    S9["§9 管理機能 ← イマココ<br/>誰がどう運用するか"]
    S10["§10 外部統合<br/>(監査ログ出力)"]
...
§2-§8 までは「**基盤が何を提供するか**」だった。§9 は「**それを誰がどう運用するか**」の話。
...
| サブセクション | 内容 | 関連 FR |
|---|---|---|
| §9.1 基盤設定管理 | 管理コンソール / テナント・IdP・Client・ロール管理 / テナント分離 | FR-ADMIN-001〜005, 009 |
| §9.2 監査・可視性 | 監査ログ閲覧 / 設定変更履歴 | FR-ADMIN-007, 008 |
| §9.3 権限委譲・カスタマイズ | 管理者 RBAC / テナント管理者委譲 / ログイン UI ブランディング | FR-ADMIN-006, 010〜012 |
...
> **§9 全体との関係**: §9.0.A 2 層構造のうち「**基盤管理者の操作**」を扱う。§9.2 が監査、§9.3 がテナント管理者委譲とカスタマイズ
...
| Terraform IaC | ✅ | ⚠ Realm 部分は別管理（[§10 外部統合](10-integration.md) 参照）| ✅ |
...
| テナント追加方式 | IaC 自動化（[§3.3.2 オンボーディング](03-federation.md#332-顧客追加オンボーディング--fr-fed-011)）|
| IdP / Client / ロール管理 | Admin Console + Terraform 両方提供 |
| テナント別設定分離 | **Must**（[§3.3.1](03-federation.md#331-複数-idp-並行運用--fr-fed-010) のテナント分離と整合）|
...
> **§9 全体との関係**: §9.1 で設定変更を行う → §9.2 でその履歴を記録 → [§10.2](10-integration.md#102-ログ監視--fr-int-82) で外部 SIEM に流す
...
| 保存期間 | **法定要件に従う**（通常 1 年、医療は 6 年、業種次第 → [§13.7 コンプラ](13-nfr.md) と連動）|
...
> **§9 全体との関係**: §9.0.A 2 層構造のうち「**テナント管理者への委譲**」を扱う。基盤管理者の操作は §9.1
...
| 細粒度パーミッション管理 | ⚠ アプリ側 | ✅ Authorization Services | [§7.2 細粒度認可](07-authz.md#72-各アプリの認可設計パターン--fr-authz-52) 参照 |
...
| 項目 | ベースライン |
|---|---|
| 基盤管理者（弊社）vs テナント管理者（顧客）| **2 層構造で運用**（§9.0.A）|
| テナント管理者の委譲 | **Should**（顧客自律運用、Keycloak で標準）|
| 管理者 RBAC | **Must**（最小権限原則）|
| 管理者 MFA | **Must**（[§4.2 MFA 適用](04-mfa.md#42-mfa-適用ポリシー--fr-mfa-32)）|
