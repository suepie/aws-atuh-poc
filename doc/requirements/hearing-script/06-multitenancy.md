...
フェデユーザーの属性更新タイミングについてご希望をご教示ください:
- **毎回上書き（Sync Mode = Force）**: 業界推奨デフォルト、退職反映即時
- **初回 JIT のみ**: 後続の IdP 側変更は本基盤に反映されない（退職遅延リスクあり）
- **別途トリガー**（SCIM 同期 / イベント駆動）
...
顧客 IdP 側で退職処理が完了してから、本基盤・各アプリで認可拒否されるまでの SLA（時間）をご教示ください。
- 即時（< 1 分）
- 数分以内
- 数時間以内
- 翌日まで許容
- SCIM 同期に依存（顧客側 SCIM 連携状況次第）
...
### 【ログイン画面のブランディング詳細】 (B-612, 🟢)

A-11 を詳細化します。カスタマイズ対象画面はどこになりますでしょうか:
- **アプリ側画面のみ**（ランディング / ダッシュボード / ログアウト後） = 認証基盤側設定不要
- **認証基盤側画面**（ログイン / MFA / パスワードリセット）も含む = Cognito Branding Style 20 顧客上限 / Keycloak Theme 開発要

共通 UI / 顧客別カスタマイズ / ハイブリッドのどれをご希望か、対象画面範囲と併せてお答えいただけますと幸いです。
**目的**: [§FR-2.3.3.A](../proposal/fr/02-federation.md) 画面所在マトリクスの判断。Cognito Managed Login Branding（Essentials+、20 顧客上限）の適用範囲、Keycloak カスタム Theme 開発工数、URL allowlist 設計に直結する情報です。
