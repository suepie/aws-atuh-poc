...
- 必要
- 不要
- 別の方式（URL ベース = §FR-2.3.3 パターン C 連動）

**Q2. 必要時の実装場所**（業界推奨は**アプリ / BFF 側**）:
- ① **アプリ側 SPA で実装**（推奨、業界標準）— `memberships` クレームを SPA が読んで自前 UI 描画。Slack / Notion / GitHub / Atlassian Cloud 等の業界実例
- ② **BFF 側で実装** — BFF セッションに active_tenant 保持、SPA はサーバーレンダリング画面で切替（[B-108 BFF パターン](../hearing-checklist.md) 採用時の推奨）
- ③ **共通基盤側で実装** — Keycloak Required Action / Cognito Hosted UI カスタム（**業界実例なし、非推奨**、本基盤の「最小限スタンス」と相反）
- ④ ハイブリッド（基盤側で初回選択、アプリ側で切替）

> **基盤側で実装しない理由**:
> - Cognito / Keycloak ともテナント切替 UI の標準機能なし（[§FR-2.3.C シナリオ C](../proposal/fr/02-federation.md#fr-23c-マルチテナント環境での-sso-挙動) で明記）
> - 業務文脈に依存する UX（表示順 / お気に入り / 切替後の遷移先）はアプリ側の方が柔軟
> - クレーム統一（[B-606-4 memberships 配列](../hearing-checklist.md)）が前提なら、アプリは JWT を読むだけで切替 UI を構築可能
> - 業界実例（Slack / Notion / GitHub Org / Linear / Atlassian Cloud）はすべてアプリ側実装

**併せて確認**（既存項目との整合）:
- [B-606-2 権限モデル](../hearing-checklist.md): 横断 / 切替 / 別ユーザー — 切替モデル採用時に本問必要
- [B-606-3 切替時の MFA 再要求](../hearing-checklist.md): 切替時の JWT 再発行に絡む
- [B-606-4 属性ソース](../hearing-checklist.md): memberships 配列 / 統合 — クレーム構造を決定

**目的**: テナント選択画面の実装スコープと**責務分担**（基盤 vs アプリ / BFF）の確定、Slack 型（ログイン後切替）/ GitHub 型（横断アクセス）の UX 設計、ブランディング要件（テナント別表示）、切替時の JWT 再発行 API 提供範囲の判断に必要な情報です。**業界標準は「基盤は `memberships` クレーム発行 + 切替時新 JWT 発行 API」、アプリ側で UI 実装** です。
