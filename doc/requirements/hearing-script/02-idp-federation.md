...
| コード | プロトコル | 本基盤対応 |
|:---:|---|---|
| **α** | OIDC（OpenID Connect 1.0） | ✅ Cognito / Keycloak 両対応、推奨 |
| **β** | SAML 2.0（基盤が SP として受信） | ✅ Cognito / Keycloak 両対応 |
| **γ** | LDAP 直結 | ⚠ **Keycloak のみ対応**（Cognito 不可）|
...
**前出のマスター表 B が「顧客 IdP → 本基盤（受信側）」の話**でしたが、本問は**逆方向**:
**本基盤を SAML IdP（発行側）として動作させ、既存の SAML SP（業務システム）と連携する必要があるか?**

- レガシー SaaS（Salesforce Classic 等）が SAML SP として動作している
- オンプレ業務システムが SAML SP で繋ぐ前提

有無 + 対象 SP システム名でお答えいただけますと幸いです。
**目的**: **Yes の場合、Cognito はネイティブ非対応のため Keycloak 必須化** ([K-11](../../reference/cognito-knockout-conditions.md))。基盤からの SAML 発行は受信とは別の対応が必要。
