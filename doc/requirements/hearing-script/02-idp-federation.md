...
> - **§3.1 マスター表 B: 事業者・顧客 IdP 統合表**: B-200（本ファイルのマスター表 B、**旧マスター表 A + B を 1 表に統合**）+ B-609（IdP 情報受領形式）
> - **§3.4 ブランディング詳細**: B-208（Custom Domain、基盤全体ポリシーとして残置）
> - 旧 B-202（SAML IdP 発行）は **§3.2 マスター表 C 列 P=g / 列 S K5** に統合済（本ファイル末尾の B-202 項は用語整理として残置）
>
...
IdP 関連は質問が多岐にわたるため、**「マスター表 B に一括記入していただく」** 形式に統合しました。これにより:

- 旧 B-201（Entra ID）/ B-203（LDAP）/ B-204（Okta）/ B-205（Google）/ B-206（SAML SP）/ B-207（独自プロトコル）/ A-6（IdP 種別分布）が **マスター表 B（顧客 IdP 行）に集約**
- 旧 A-13（弊社内 IdP）と旧マスター表 A（弊社 IdP、1 行）が **マスター表 B（事業者行）に統合**（2026-05-25 統合、シート数削減 + 弊社 IdP 登録漏れ防止）
- 「どの IdP を使っているか」と「どの接続プロトコルで繋ぐか」を **事業者 1 行 + 顧客 N 行**で一括確認、区分列で役割を明示
...
## マスター表 B: 事業者・顧客 IdP 統合表（旧マスター表 A + B 統合、🔥）

> **構造変更**（2026-05-25）: 旧マスター表 A（弊社内 IdP、1 行）と旧マスター表 B（顧客 IdP、N 行）を **1 つの表に統合**しました。区分列（事業者 / 顧客）と用途列（P-X カテゴリ）で行ごとに役割を明示。**シート数削減 + 弊社 IdP 登録漏れ防止**が目的。

> **問いの位置づけ**: 本基盤と連携する全 IdP（**事業者**の運用管理者用 + **顧客企業**のフェデユーザー用）を 1 行ずつ把握する。**Cognito vs Keycloak 選定の 2 大判定表のひとつ**（もう 1 つは [マスター表 C](01-auth-flow.md#マスター表-c-御社アプリシステム構成リスト)）。
> **回答で決まること**: ①P-1（弊社運用管理者）の認証経路 + Break Glass 設計 / ②P-3（顧客従業員）の受信側プロトコルの種類 / ③**列 Y γ LDAP 直結が 1 件でもあれば Keycloak 必須化** / ④列 W SCIM 対応次第で B-401 SCIM 採否の現実性 / ⑤列 V テナント分離レベル次第で物理分離（L3）対応要否 / ⑥Cognito 採用時の Custom Domain 4 個 Hard Limit / Pool 上限への抵触判定。

#### なぜ統合表にしたか

- **弊社内 IdP は 1 行のみ**で、独立シートにする規模ではない
- **「弊社 IdP の登録忘れ」を防げる**（1 シートを埋めれば全 IdP がカバーされる）
- 区分列で「事業者 / 顧客」を、用途列で「対象ユーザー（P-1 vs P-3 等）」を明示できる
- ヒアリング進捗管理が 1 表で完結
...
> **記入ヒント**:
> - **行 0** は事業者（弊社）の 1 行で、用途は **P-1 基盤運用管理者の認証**
> - **行 1 以降**は顧客企業の N 行で、用途は **P-3 一般従業員の認証**
> - 事業者行では **列 V（テナント分離）と列 Z（接続経由）は「-」**（弊社は顧客テナントではない、AD 経由構成は問わない）
> - **dogfood ケース**（弊社が自社向けにも本基盤を利用）は、弊社を行 0（事業者）+ 別行（顧客として）の 2 行で記入

| # | **区分** | 企業名 | IdP 製品（列 X 選択肢から）| 接続プロトコル（列 Y 選択肢から）| 接続経由（列 Z 選択肢から）| SCIM 対応（列 W 選択肢から）| テナント分離希望（列 V 選択肢から）| 特殊要件 | **用途** |
|:---:|:---:|---|---|---|---|:---:|---|---|---|
| 0 | **事業者** | _弊社_（例）| _A-3_ Entra ID P2 | _α_ OIDC | _①_ 直結 | _✅_ 標準対応 | -（該当なし）| -（該当なし）| **P-1 基盤運用管理者の認証** |
| 1 | 顧客 | _Acme Corp_（例）| _A-2_ Entra ID P1 | _α_ OIDC | _①_ 直結 | _✅_ 標準対応 | _L2_ 論理 | なし | P-3 一般従業員の認証 |
| 2 | 顧客 | _Globex Inc_（例）| _B_ Okta | _α_ OIDC | _①_ 直結 | _✅_ 標準対応 | _L2_ 論理 | なし | P-3 一般従業員の認証 |
| 3 | 顧客 | _HENNGE Cust_（例）| _D_ HENNGE One | _β_ SAML | _①_ 直結 | _❓_ 不明 | _L2_ 論理 | なし | P-3 一般従業員の認証 |
| 4 | 顧客 | _DefenseCo_（例）| _G_ オンプレ AD のみ | **_γ_ LDAP 直結** | _④_ AD 直結 | _❌_ 未対応 | _L3_ 物理 | **クラウド禁止規制** | P-3 一般従業員の認証 |
| 5 | 顧客 | _TechStart_（例）| _I_ IdP なし | （該当なし）| （該当なし）| （該当なし）| _L2_ 論理 | IdP 検討中、当面ローカル | P-3（ローカル P-4 含む）|
| 6 | 顧客 | _SaaS Vendor X_（例）| **_L_ Keycloak**（顧客自社運用）| _α_ OIDC | _①_ 直結 | _⚠_ プラグイン依存 | _L2_ 論理 | 同製品同士のフェデ、特別対応不要 | P-3 一般従業員の認証 |
| 7 | 顧客 | _AWS-Native Cust_（例）| **_M_ Cognito**（顧客自社、別 AWS） | _α_ OIDC | _①_ 直結 | _❌_ 未対応 | _L2_ 論理 | Cross-account Cognito federation | P-3 一般従業員の認証 |
| 8 | 顧客 | _Enterprise Y_（例）| **_N_ PingOne** | _β_ SAML | _①_ 直結 | _✅_ 標準対応 | _L2_ 論理 | 大企業向け IDaaS | P-3 一般従業員の認証 |
| 9 | 顧客 | （以下、顧客の数だけ追記）| | | | | | | |

**事業者行の回答例**:
- _A-3_ Entra ID P2 / α OIDC / ① 直結 / ✅ SCIM 対応 / 用途: **P-1 基盤運用管理者の認証**
- _B_ Okta / β SAML / ① 直結 / ✅ SCIM 対応 / 用途: **P-1 基盤運用管理者の認証**
- _I_ なし / -（弊社内 IdP なし、Break Glass ローカル管理者のみで運用）/ 用途: **P-1（Break Glass のみ、δ シナリオ）**
...
### Q3: 弊社の社内 IdP（マスター表 B 事業者行）が未確定の場合
...
| A-13 | 貴社の社内 IdP | **マスター表 B 事業者行**（旧マスター表 A は 2026-05-25 統合表 B に統合）|
