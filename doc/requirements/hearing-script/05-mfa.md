...
> **本問は [マスター表 C](01-auth-flow.md#マスター表-c-御社アプリシステム構成リスト) 列 S K7（Back-Channel Logout）に統合済**です。該当アプリで列 S K7 を☑してください。本ページの本項は**該当判定の参考情報**として残しています。

全クライアント連動ログアウト（1 つのアプリでログアウトしたら、同 IdP の全 RP にも確実に伝播）の要件があるアプリが**1 つでも該当する場合**、表 C で該当アプリの列 S K7 を☑してください。

**目的**: OIDC Back-Channel Logout（RFC 8417）採用要否の判断。**☑の場合（=表 C 列 S K7 がいずれかのアプリで含まれる）、Cognito はネイティブ非対応のため Keycloak 必須化**となります。Front-Channel Logout（ブラウザ依存）で代替する場合は信頼性が低下します。
