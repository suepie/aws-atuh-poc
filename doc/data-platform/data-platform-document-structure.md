...
| 9 | hearing-slides-organization.md | 構成選択ヒアリングスライド **14 論点 / 21 スライド**（**Part 0（前段 3 枚）**概念 + メリット比較 + 判断条件 → **Part 1（10 枚）集約の是非を 5 観点で総合判断** → **Part 2（7 枚: 前段 + 6）「処理はどこでやるか?」フレーミング + A案/B案 詳細**、日本語中心、60-90 分想定。§2.4 に顧客への説明スクリプト 3 パターン + 4 フレーミング を追加）| ✅ v4.5 |
