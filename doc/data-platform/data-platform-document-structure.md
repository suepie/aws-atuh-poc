...
| 9 | hearing-slides-organization.md | 構成選択ヒアリングスライド **14 論点 / 23 スライド**（**Part 0（前段 3 枚）**概念 + メリット比較 + 判断条件 → **Part 1（10 枚）集約の是非を 5 観点で総合判断** → **Part 2（9 枚: 前段 + 8）「処理はどこでやるか?」フレーミング + A案/B案 詳細 + 運用イメージ + 必要体制**、日本語中心、78-108 分想定。§2.4 に顧客への説明スクリプト。スライド 8-前段-1/2 で運用の具体イメージと必要体制を提示してから体制ヒアリングへ）| ✅ v4.6 |
...
| 11 | glue-and-operations-detail.md | **【補足資料】Glue と運用の具体作業・必要スキル**（8 カテゴリー × 頻度 × 工数 × 必要スキル、役割別スキルマトリクス、Phase 別人員計画。§0.4 Data Contract パターン、§0.5 Data Contract 実装ガイド（5 役割 + RACI + Contract YAML テンプレート + Phase 0 の 10 項目 + ワークショップ 3 回 + アンチパターン）、§0.6 足りない項目対応 4 パターン、§0.7 Pattern 2 での役割変化 も収録）| ✅ v2 |
...
| [strawman-proposal.md](strawman-proposal.md) | ヒアリング用 仮案（β + Path C + **Option B Catalog 同居 +1** + 共通参照データ D-2 中央同居、7 役割定義、Phase 計画、ヒアリング項目）| 🚧 仮案 |
...
| [DP-ADR-003](adr/DP-ADR-003-common-domain-account-placement.md) | 共通参照データの配置 — Phase 1 中央同居（D-2）採用 / Phase 2 D-1 新設再評価 | ✅ Accepted |
