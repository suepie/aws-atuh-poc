...
| 9 | hearing-slides-organization.md | 構成選択ヒアリングスライド **13 論点 / 20 スライド**（**Part 0（前段 3 枚）データプラットフォームとは何か + 構築 vs 個別 のメリット比較 + 判断条件** → **Part 1（10 枚）集約の是非を 5 観点で総合判断**（① 目的 / ② データ特性 / ③ 運用体制 / ④ スケール / ⑤ コスト価値バランス + 総合判定 + 補足 2 枚） → **Part 2（6 枚）集約する場合の A案 / B案 選択（体制で判断）**、日本語中心、60-90 分想定）| ✅ v4.4 |
