...
#### 4.5.2.C 必須のみ集計（厳密に「必須」フラグの行だけ）

§4.5.2.A の小計は**任意行も含めた実態値**（ベースライン = Transfer Family 除外）。「**必須項目のみ厳密に採用**」した場合の小計は以下:

| 配置場所 | カテゴリ | ベースライン $ | **必須のみ $** | 差分 | 除外される任意項目 |
|---|---|---:|---:|---:|---|
| Producer（1 アプリ）| 取込層 | 47 | **46** | -1 | Firehose $1.0 / AppFlow $0.13 |
| Producer（1 アプリ）| ストレージ層 | 9 | **8** | -1 | S3 IA / Glacier / Lifecycle Transition |
| Producer（1 アプリ）| Glue 層 | 13 | **10** | -3 | Glue Data Quality $2.6 |
| Producer（1 アプリ）| オーケスト層 | 16 | **10** | -6 | カスタムメトリクス $6 / SNS $0.01 |
| **Producer 合計**（1 アプリ）| | **85** | **74** | **-11** | |
| 中央（Phase 1）| カタログ層 | 8 | **8** | 0 | LF Storage Optimizer（数 $）|
| 中央（Phase 1）| Athena 層 | 1.5 | **1.5** | 0 | Provisioned / Spark / Result Reuse |
| 中央（Phase 1）| QuickSight 層 | 386 | **286** | -100 | Paginated Reports $100 |
| 中央（Phase 1）| 結果・派生 | 3 | **3** | 0 | — |
| **中央 合計**（Phase 1）| | **399** | **299** | **-100** | |
| 横断 | リソース共有 + 監査ログ + ネットワーク + 構成監視 + データ転送 | 342 | **214** | -128 | Insights $17.5 / Lake $4.25 / Config Rules $100 / VPC Flow Logs $5 / egress $0.6 |

→ **厳密な必須のみ**では **10 アプリで $1,253/月**（詳細は §4.5.3 参照）
...
**厳密に必須のみ採用時**（§4.5.2.C 参照）:

| カテゴリ | 内訳 | 月額 |
|---|---|---|
| Producer アカウント | $74/アプリ × N | $74 × N |
| 中央 BI / Catalog アカウント | 単一 | $299 |
| 横断インフラ | 全アカウント分散負担 | $214 |
| **必須のみ合計（1 アプリ時）** | $74 + $299 + $214 | **~$587/月** |
| **必須のみ合計（5 アプリ時）** | $74 × 5 + $299 + $214 | **~$883/月** |
| **必須のみ合計（10 アプリ時）** | $74 × 10 + $299 + $214 | **~$1,253/月** |
| **必須のみ合計（20 アプリ時）** | $74 × 20 + $299 + $214 | **~$1,993/月** |

→ 「必須のみ」= ベースラインより **10 アプリで $338 少ない**（Firehose / AppFlow / Data Quality / カスタムメトリクス / Paginated Reports / Config Rules / Insights / Lake / VPC Flow Logs 等を除外）
→ さらに **レベル 1 設定最適化**（§4.5.4）を全適用すると **$1,000/月** まで削減可能
...
### 4.5.4 コスト最適化の主要レバー（10 項目 + レベル 1 追加 6 項目）
...
#### B. レベル 1 最適化: 「必須のみ」構成をさらに絞る（6 項目、機能維持）

「必須のみ $1,253/月」（§4.5.2.C / §4.5.3）を出発点に、**サービスは維持したまま設定を絞ることでさらに削減**できるレバー。**機能変更なし**、単に設定・容量・スコープを絞るだけ。

| # | 最適化 | 現状 | 最小化後 | 削減効果（10 アプリ）| 前提・制約 |
|---|---|---|---|---:|---|
| L1-1 | **VPC Interface Endpoint 削減**（8 EP × 3 AZ → 4 EP × 2 AZ）| $172.8 | $57.6 | **-$115/月** | S3/DynamoDB は Gateway EP へ移行、他は NAT + IGW でカバー |
| L1-2 | **AWS Config 記録範囲絞込**（全リソース → 監査対象のみ）| $30 | $10 | **-$20/月** | Recording を特定リソースタイプに限定 |
| L1-3 | **DMS Storage 容量削減**（100 GB → 50 GB gp3）| $9.6/アプリ | $4.8/アプリ | **-$48/月** | CDC トラフィックが安定していることが前提、Phase 2 で再評価 |
| L1-4 | **Glue Continuous Logging 無効化**（本番ジョブのみ）| $6/アプリ | $2/アプリ | **-$40/月** | デバッグは開発環境で完結、本番は Executor 集約ログのみ |
| L1-5 | **CloudWatch Logs Retention 30 日**（デフォルト never → 30d）| $1.5/アプリ | $0.5/アプリ | **-$10/月** | 監査要件がなければ短縮可 |
| L1-6 | **Lambda Graviton2 適用**（重複、A #10 と同じ）| $4.3/アプリ | $3.4/アプリ | **-$9/月** | — |
| **レベル 1 合計削減** | | | | **~$242/月** | |

→ **必須のみ $1,253 → レベル 1 適用後 ~$1,011/月**（10 アプリ、機能維持のまま 19% 削減）

#### C. さらに削減する余地（レベル 2〜3）

「機能同等の代替サービス採用」「機能削減」でさらに $625/月 / $375/月 まで削減可能だが、リアルタイム性・UX・監査を犠牲にするため **別途 ADR で判断**（本節では扱わない）。参考: DMS → Aurora Export to S3、Glue ETL → Athena CTAS、Glue Crawler → Partition Projection、QuickSight 廃止、監査要件緩和 等。
...
**Phase 1 月額**: 10 アプリで **~$1,253/月**（厳密に必須のみ採用時、[§4.5.2.C](#452c-必須のみ集計厳密に必須フラグの行だけ) / [§4.5.3](#453-全体合計phase-1) 参照）。レベル 1 最適化（[§4.5.4.B](#b-レベル-1-最適化-必須のみ構成をさらに絞る6-項目機能維持)）全適用で **~$1,011/月** まで削減可能。
