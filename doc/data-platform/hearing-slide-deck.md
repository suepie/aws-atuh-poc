...
| 1.3 | 利用者別の見たい・したい | §1.2 の **4 区分すべて**の具体役割（業務利用者 4 サブ / 開発者 / BI チーム / 監査担当者の 7 役割）の **What to see・What to do・Why**、Bain & Company 解約率研究 | 各役割の優先度 |
...
§1.2 の 4 区分それぞれを **具体的な役割に分解**し、**What to see**（見たい）・**What to do**（したい）・**Why**（業務上の必要性）を整理:

| §1.2 区分 | 具体役割 | 何を見たい（What to see）| 何をしたい（What to do）| Why（業務上の必要性）|
|---|---|---|---|---|
| **(a) 業務利用者** | 経営層 | ARR / MRR / 解約率トレンド / 顧客企業別契約状況 | 経営判断・投資配分・経営会議資料作成 | 投資家報告・予算配分・経営戦略決定の根拠データ |
| **(a) 業務利用者** | カスタマーサクセス | 顧客企業別の利用率・健全性スコア・サポート問い合わせ件数 | 解約予兆顧客へのアプローチ | **LTV 最大化と解約率削減**。SaaS 事業では解約率 5% 改善で利益 25-95% 増加 ([Bain & Company](https://media.bain.com/Images/BB_Prescription_cutting_costs.pdf)) |
| **(a) 業務利用者** | プロダクトマネージャー | 機能別利用率・新機能コンバージョン・離脱率 | 製品ロードマップ優先順位 | 開発リソース配分の合理化、機能リリース効果測定の高速化（PDCA 短縮）|
| **(a) 業務利用者** | マーケティング | 業界別利用パターン・リード獲得経路別の成約率 | キャンペーン最適化 | CAC 削減 / コンバージョン向上 |
| **(b) 開発者** | SaaS 開発・運用 / SRE | 障害影響顧客数・回復時間・SLA 達成率・パフォーマンス劣化 | 製品品質改善・障害対応・SLO 設計 | SLA 遵守 / 顧客満足度向上 / 障害ポストモーテム |
| **(c) データ分析担当者** | 中央 BI チーム | 5 区分横断のデータ・メタデータ（スキーマ・鮮度・品質）・ML 用特徴量 | 探索的分析・モデル構築・新規 KPI 発見・他役割向けダッシュボード提供 | データドリブン経営の中核、各部門の意思決定支援 |
| **(d) 監査担当者** | 内部監査 / コンプラ | アクセスログ・個人情報棚卸し結果・権限変更履歴・テナント越境アラート | 監査証跡確認・コンプラ報告・リスク評価 | SOC 2 / 個人情報保護法 / GDPR / J-SOX 等への対応 |
