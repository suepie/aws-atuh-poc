...
| 4.2 | **監査アカウントとの責務分離** | **⚠ Option B 成立の必須前提**。**新規「監査アカウント」が別組織により生成される**（データプラットフォーム標準のスコープ外、Transit GW 等と同類）。監査アカウントが以下を担当する: CloudTrail Org Trail / Security Hub Delegated Admin / GuardDuty / Macie / Config Aggregator / クロスアカウントログ保管バケット。データプラットフォーム側は **「何を送るか / 何を期待するか」を定義する側**。詳細は [account-architecture-analysis.md §5.8](account-architecture-analysis.md#58-監査責務分離の具体設計option-b-採用時--新規監査アカウント前提) | (a) 監査アカウントは立てられる / (b) 立てられないので Option C / (c) 既存の親会社統制を流用 / (d) ログ送付仕様の合意が必要 |
...
| 2026-06-17 | **§4.2 を監査アカウントとの責務分離に書き換え**: 旧「親会社統制の責務範囲確認」を「**監査アカウントとの責務分離**」に再構成。新規「監査アカウント」が別組織により生成される（データプラットフォーム標準のスコープ外、Transit GW 等と同類）前提を明示。詳細設計は [account-architecture-analysis.md §5.8](account-architecture-analysis.md#58-監査責務分離の具体設計option-b-採用時--新規監査アカウント前提) に集約 |
