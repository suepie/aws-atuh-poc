...
| **⑤** | **顧客への開示** | 「顧客企業名・IdP 製品の秘匿は保証しない（Identifier-First 方式に内在）。代わりに大量列挙を検知・遮断する」を**顧客説明資料と責任分担表に明記**する。保証できないものを黙っていると、後で「漏洩」と評価される | 🆕 **未起票** |
...
- **根拠**: [U2 §2.7.2 / §2.7.3](02-keycloak-logical-design.md)（K-8）、[U4 §4.1.3 D-U4-02](04-auth-ux-design.md)（応答同一化の適用範囲と限界）、ADR-020 §F、ADR-042 §D。
