...
| D-U4-04 | MFA UX（**2026-07-30 D-17 反映**）= 外部フェデ（顧客 IdP 保有テナントのユーザ・テナント管理者）は**検証・記録のみ（MFA は顧客 IdP 責任）**/ B WebAuthn・C TOTP は **IdP-KC 収容ユーザ（非 IdP テナントの一般ユーザ + テナント管理者 P-2）** / D 管理者 PW+WebAuthn は **基盤運用者 P-1 のみ（踏み台/SSM）**。※PCI 対応は 2026-07-30 に対象外化（B-MFA-PCI-1 廃止）、記録のみ方針は維持 | §4.3 |
...
> （2026-07-26 図修正）D-U4-04 改訂を反映: 旧図の「MFA 画面群（ケース B/C/D）」ノードを撤去。フェデ後の基盤側追加画面はなし（`mfa_indicator` 記録のみ）、ケース B/C は IdP-KC 収容ユーザ限定で IdP-KC ログイン画面内、ケース D は基盤運用者 P-1 ローカル経路のみ（§4.3.1、2026-07-30 D-17）。
...
| ③ `@` なし・ハイフンなし（素の userid） | `attempted()` 降格 → PW フォーム | PW フォームへ遷移（**2026-08-24 是正: 「管理者ローカルの正規経路」ではない** — D-17 により P-1 も公開経路から除外済み。①②と同じ**列挙防止のダミー終端**）。フォーム上部に恒常ヒント「一般利用者の方は `会社コード-ユーザーID` を入力してください」を表示（U2 §2.3.3 未決事項への回答: **エラー扱いにせず管理者ローカル試行 + ヒント表示**とする） |
...
- **応答同一化の適用範囲と限界（2026-08-24 追記、重要）**: 上表の同一化は**失敗どうしの区別を消すもの**（① Org 不在 / ② domain 未登録 / ③ 素の userid → いずれも同じ PW フォーム）であり、**「解決成功」と「解決失敗」の区別は消せない**。解決成功時は顧客 IdP へ 302 が返り、**リダイレクト先 FQDN から当該企業の IdP 製品まで判別できる**。これは Identifier-First / HRD 方式に**原理的に内在する**性質（Entra の `GetCredentialType` 等、業界共通）であり、本設計だけの弱点ではない。**したがって「テナント存在の秘匿」は設計目標として成立しない**。目標は**大量列挙の検知と抑止**に置き、対策は [U7 §7.8](07-security-compliance-design.md)（列挙探索の検知）へ引き渡す。**顧客説明でも「顧客企業名の秘匿は保証しない」ことを明示する**（過大な期待を持たせない）。
...
**採用（D-U4-04、2026-07-26 改訂）**: §FR-3.4.0.B の 4 ケースのうち、**外部フェデユーザ（P-3）への基盤側 MFA 補完（旧ケース B/C）を撤去**する — **MFA の実施は顧客 IdP の責任**とし、基盤は `mfa_indicator` の**検証・記録のみ**行う（ブロック・補完・追加画面なし。ユーザー方針決定 2026-07-26。記録のみ方針は維持。**※2026-07-30: PCI DSS 対応は対象外化（B-PCI-1）につき、旧 PCI Req 8.4.2 リスク明示・契約ゲート B-MFA-PCI-1 は廃止**。外部フェデの MFA は顧客 IdP 責任、基盤は amr/mfa_indicator 記録 + 監視のみ）。

改訂後の画面遷移: ケース A'（P-3 全員）= 追加画面なし（mfa_indicator を記録のみ）/ ケース B/C = **IdP-KC 収容ユーザにのみ適用**（WebAuthn 優先 / TOTP 代替 — 実施場所は IdP-KC のログイン画面）/ ケース D = **基盤運用者 P-1**（PW + WebAuthn 必須、踏み台/SSM 接続）。**非 IdP テナントのテナント管理者 P-2 はケース D でなくケース B〔IdP-KC〕**（2026-07-30 D-17）。エンロール強制（§4.3.2）は IdP-KC 収容ユーザ（非 IdP テナント管理者含む）と P-1 運用者のみ。判定は `mfa_indicator` 正規化属性（ADR-031 / U2 §2.4.4、fail-safe の帰結を「補完」から「記録 + 監視」へ変更）による。
...
|:-:|---|---|---|
| **A'** | **P-3（外部フェデ）全員 — IdP の MFA 実施有無を問わない**（2026-07-26 改訂） | **ゼロ**（`mfa_indicator` を記録・監視のみ。未済でもブロック・補完しない — MFA は顧客 IdP 責任。※PCI 対象外化で B-MFA-PCI-1 廃止・記録のみ方針は維持） | 二重 MFA 回避（ADR-024 §D アンチパターン 1 の防止）が本基盤の中核 UX 価値。画面には何も出ない = 設計成功の状態 |
| **B** | **IdP-KC 収容ユーザ**（非 IdP テナントの一般ユーザ + **テナント管理者 P-2**）・WebAuthn 可（2026-07-30 D-17: 非 IdP テナント管理者を本ケースに明記） | WebAuthn 認証 1 画面（**実施場所 = IdP-KC のログイン画面**） | プラットフォーム認証器（Touch ID / Windows Hello / パスキー）優先 |
...
| **D** | **基盤運用者 P-1 のみ**（Broker/IdP-KC ローカル、踏み台/SSM 接続。2026-07-30 D-17: 非 IdP テナント管理者はケース B〔IdP-KC〕へ移動） | PW → WebAuthn 必須 | TOTP への緩和は不可（攻撃の主要標的、§FR-3.4.5）。Break Glass はランブック管理の別手順（U7/U9）で本 UX の対象外 |
...
- **決定 D-U4-10: セキュアなエンロール（登録 ATO 対策）**: **新規 MFA/パスキーの登録は「検証済み文脈」でのみ許可**する。①初回エンロール = 上記フロー（初回ログイン直後・強制）②**2 台目以降・再登録は step-up（既存 WebAuthn or Recovery Code での再認証）を必須**とし、第 1 要素のみでの追加登録を禁止（盗んだ第 1 要素で攻撃者が自分のパスキーを登録し正規利用者を締め出す ATO の防止）。hearing **B-ENROLL-1**。
- **決定 D-U4-10: セキュアなエンロール（登録 ATO 対策）**: **新規 MFA/パスキーの登録は「検証済み文脈」でのみ許可**する。①初回エンロール = 上記フロー（初回ログイン直後・強制）②**2 台目以降・再登録は step-up（既存 WebAuthn or Recovery Code での再認証）を必須**とし、第 1 要素のみでの追加登録を禁止（盗んだ第 1 要素で攻撃者が自分のパスキーを登録し正規利用者を締め出す ATO の防止）。hearing **B-ENROLL-1**。
...
- **決定 D-U4-11: パスキー専用/email 非保有ユーザのリカバリ**: email OTP 除外（D-U7-14）ゆえメールリセット経路がないため、**①2 台目パスキー/クレデンシャルの事前登録を強く推奨**（推奨案内 = エンロール直後）**②リカバリは assurance を下げない**（Recovery Code or 管理者 JIT 承認経路のみ、SMS/email へのフォールバック禁止）**③永久ロックアウトを避ける**設計（管理者リセット経路 = ケース B/C はテナント管理者、D は運用ランブック）。hearing **B-RECOV-1**。
- **決定 D-U4-11: パスキー専用/email 非保有ユーザのリカバリ**: email OTP 除外（D-U7-14）ゆえメールリセット経路がないため、**①2 台目パスキー/クレデンシャルの事前登録を強く推奨**（推奨案内 = エンロール直後）**②リカバリは assurance を下げない**（Recovery Code or 管理者 JIT 承認経路のみ、SMS/email へのフォールバック禁止）**③永久ロックアウトを避ける**設計（管理者リセット経路 = ケース B/C はテナント管理者、D は運用ランブック）。hearing **B-RECOV-1**。
...
### 4.3.5 セルフサービス セッション/デバイス管理（D-U4-09）

**採用**: 利用者が**自分のアクティブセッション/デバイス一覧を閲覧し自己失効**できる画面を提供する（Keycloak Account Console の Sessions/Devices 相当をブランドテーマで提供、または管理画面 SPA に組込）。ATO 対応（不審端末の即時失効）と DSAR 隣接（自分のアクセス状況の可視化）を利用者セルフで担保し、サポート負荷とゾンビ露出を縮小。管理者側の強制失効（U5 §5.4.2）とは別レイヤ。**フェデユーザ（顧客 IdP セッション）は本基盤セッションのみ対象**（顧客 IdP 側セッションは顧客責任）。Phase 1/2 の切り分けは hearing **B-SESS-2** で確定。
...
- 2026-08-24: **§4.1.3 ケース③ の「管理者ローカルの正規経路」を是正**（D-17 により P-1 も公開経路から除外済み。①②と同じ列挙防止のダミー終端）。**応答同一化の適用範囲と限界を明記** — 同一化できるのは失敗どうしの区別のみで、**「解決成功 vs 失敗」の区別は Identifier-First に原理的に内在**し消せない。よって**テナント存在の秘匿は設計目標として成立しない**ことを確定し、対策は [U7 §7.8.1a](07-security-compliance-design.md) へ引き渡し。
- 2026-08-12: **網羅性再監査反映** — D-U4-09 セルフサービス セッション/デバイス管理（§4.3.5 新設、B-SESS-2）/ D-U4-10 セキュアなエンロール（登録 ATO、§4.3.2、B-ENROLL-1）/ D-U4-11 パスキー専用・email 非保有リカバリ（§4.3.3、B-RECOV-1）を追加。
