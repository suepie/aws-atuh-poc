...
> **アカウントリンクの方針（2026-08-18 追記）**: 同一テナント内で同一人物が複数の IdP からログインしうる場合の扱いは **[ADR-027](../adr/027-tenant-user-duplication-handling.md)** が定める（**統合（リンク）派**を採用、**自動リンクは原則行わない**、突合キーは `sub` / 雇用 ID をプライマリとし email は補助）。本書の [D3-04 プロビジョニング経路](03-identity-provisioning-design.md) と [ADR-025 §H.4.B Case 1-5 混在パターン](../adr/025-jit-scim-provisioning.md) の前提。ADR-027 は設計書から参照されていなかったため接続。
...
| — 🆕 | **旧基盤からの引継ぎ（2026-08-27 追加 — C-4、[U10](10-integration-migration-design.md) からの依頼）** | 両方 | 移行元の旧認証システム | **`legacy_spi`** | 移行時の一括取込（移行完了後は発生しない） | ❌ | **対象外**（移行元の情報が正） | **禁止**（移行完了までは触らない） |
...
| ~~U3-OP-2~~ | 対応づけデータの置き場所とアプリからの参照経路 | ✅ **2026-08-27 クローズ（C-5）= 権限側 DB に同居**（旧推奨「Broker 側 DB に別 DB として同居」から変更）。[ADR-063](../adr/063-brand-unit-architecture.md) でブランド単位を主役としたため、対応づけ・権限・参照用データは**同一の権限側 DB** に集約。アプリからは管理 API 経由のみ（直接続禁止） | 決着済 |
| ~~U3-OP-3~~ | 受け取り窓口の実装形態 | ✅ **2026-08-27 クローズ（C-5）= 関数として実装**（旧推奨「クラスタ上の常駐サービス」から変更）。[ADR-062](../adr/062-idm-api-execution-form-lambda.md) で管理系を関数に統一したため、受け取り窓口も同じ土台に載せる。応答時間は負荷試験で実測（[PT-03](research/wbs-test-refinement-2026-08-26.md)） | 決着済 |
...
flowchart TB
  ...
  RC -.->|"deprovisioned_at↔shadow 突合(数分)"| SH
```
...
sequenceDiagram
    ...
    A2->>DB: 2 projection deprovisioned + outbox 行 を 1 Tx
    Note over A2,DB: 削除したら必ず outbox が残る(喪失なし)
    DB->>REL: 3 リレーが outbox を拾う
    REL->>EB: 4 user.deprovisioned を PutEvents(成功まで再送)
    EB->>SH: 5 配信 (at-least-once)
    SH->>BK: 6 shadow を enabled=false + not_before + revoke(冪等・内部NLB)
    Note over SH,BK: 未達/遅延は数分リコンサイルが補正<br/>窓 = 通常数秒 / worst = リコンサイル間隔
```
