...
- **工数（人日・工程分割、2026-08-26 精緻化）**: 全 DU（既存 + §12c GAP 28）= 詳細設計+構築 **ベース 1,030 人日**（**②詳細設計 412 + ③構築 618**、40/60 分割。IaC/Config/Doc 系は②③融合）。**正は [wbs-build-refinement-2026-08-26.md](research/wbs-build-refinement-2026-08-26.md)** — 全 DU を 1〜6 人日の work item に**内訳分解**、**構築分岐プール CB-*（+73、トリガー顕在時のみ）** を併設、[gap-audit §8](research/wbs-gap-audit-2026-08-12.md) の stale 5 件（U2-09 18→8 / U7-16 未計上 +10 / **U10-04 対象外なのに 14 計上 → 除外** / U10-11 条件付 → CB-4 へ / **U6-14 新設 +8**）を是正（1,044→1,030）。**①基本設計 596 + 条件付き 152**（[00a §1-4 v2.2](00a-remaining-tasks-and-effort.md)。**H 設計成果物の執筆 327 人日/116 項目**）・**④テスト ≈421**（50 項目、[wbs-test-refinement-2026-08-26](research/wbs-test-refinement-2026-08-26.md) が正。共有基盤 TE・異常系 AB・2-VPC/issuer/SRE ログの計上漏れを是正。旧 363）は別掲。**ベース総計 ≈2,047 / 条件付き 275（設計 152 + 構築 73 + テスト 50）で上限 ≈2,322**。**注: 00a §6「構築+テスト 325–455」は詳細設計を含まない別スコープ**で単純合算不可。
