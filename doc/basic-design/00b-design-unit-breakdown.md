...
[00a（残タスク・工数）](00a-remaining-tasks-and-effort.md) は「**基本設計を閉じる**ための調査・検討タスク（**v2: 269 人日 + 条件付き 65**、2026-08-26 粒度精緻化。旧 244）」を棚卸しした。本書はその次段 —— **基本設計で確定した決定を、詳細設計・構築・PoC・契約が着手できる「実装可能単位（Design Unit, DU）」まで分解**する WBS である。
...
| DU-U7-16 🆕 | SRE ライブログ可視性対策（APPI 28） | App/Config/IaC: ① Keycloak 構造化ログ + ソース側 stdout PII マスキング + 本番ログレベル `DEBUG`/`TRACE` 禁止（K-13）② Approved Access 有効化（要サポートチケット）③ SRE アクセス監査を Cluster Logging Operator→CloudWatch で顧客保全（[ADR-056 L7](../adr/056-rosa-adoption-decision.md) / [research 2026-08-14](research/rosa-sre-live-log-visibility-2026-08-14.md)） | `oc logs` にマスク前 PII が出ない実機確認・SRE 昇格に顧客承認ゲート・アクセス監査を顧客側保全（M-1〜14 は下流の第二防壁として維持） | DU-U7-04, DU-U9O-02, DU-U7-14 | G-SRE-LogVis | M | 10 | 🟡 |
...
| DU-U10-11 🆕 | **Custom PasswordHashProvider SPI（条件付・2026-08-17 復活。2026-08-26: ベース集計から除外し構築分岐 [CB-4](research/wbs-build-refinement-2026-08-26.md) へ移動）** — アプリが Admin REST API でハッシュを持ち込む場合、旧方式が Keycloak ネイティブ（PBKDF2-SHA256/512・Argon2id）でなければ本 SPI が必要。**要否は B-MIG-1 の回答次第** | SPI: bcrypt/旧 Argon2 恒久維持用ハッシュプロバイダ(B-MIG-10 該当時のみ、§10.4.2) | 旧 algo で既存 PW 検証が RHBK 26.x 成立・G-SPI-Compat 追加 | DU-U2-04, DU-U10-04 | G-SPI-Compat | M | 8 | 🟡条件付 |

**GAP 小計 = 226 人日**（🔴Phase1 必須 ≈196 / 🟡将来・条件付 ≈30）。→ **2026-08-26 是正後 = 226 人日**（U2-09 -10 / U7-16 +10 / U10-11 -8 を CB-4 へ / **U6-14 新設 +8**、差引 ±0。全体ベースは U10-04 除外 -14 で 1,044→**1,030**。内訳分解と分岐は [wbs-build-refinement](research/wbs-build-refinement-2026-08-26.md)）。
...
- **工数（人日・工程分割、2026-08-26 精緻化）**: 全 DU（既存 + §12c GAP 28）= 詳細設計+構築 **ベース 1,044 人日**（**②詳細設計 441**〔[ボトムアップ 64 項目](research/wbs-detailed-design-refinement-2026-08-26.md)、按分 418 では不足〕**+ ③構築 626**〔×60%〕）**＋ ③横断・共通 BX 117**（開発環境/レビュー/手戻り/脆弱性是正/リリース準備 = 従来ゼロ計上）。**正は [wbs-build-refinement-2026-08-26.md](research/wbs-build-refinement-2026-08-26.md)** — 全 DU を 1〜6 人日の work item に**内訳分解**、**構築分岐プール CB-*（+73、トリガー顕在時のみ）** を併設、[gap-audit §8](research/wbs-gap-audit-2026-08-12.md) の stale 5 件（U2-09 18→8 / U7-16 未計上 +10 / **U10-04 対象外なのに 14 計上 → 除外** / U10-11 条件付 → CB-4 へ / **U6-14 新設 +8**）を是正（1,044→1,030）。**①基本設計 693 + 条件付き 152**（[00a §1-4 v2.3](00a-remaining-tasks-and-effort.md)。**H 設計成果物の執筆 424 人日/148 項目**）・**④テスト ≈421**（50 項目、[wbs-test-refinement-2026-08-26](research/wbs-test-refinement-2026-08-26.md) が正。共有基盤 TE・異常系 AB・2-VPC/issuer/SRE ログの計上漏れを是正。旧 363）は別掲。**ベース総計 ≈2,298 / 条件付き 300（設計 152 + 構築 98 + テスト 50）で上限 ≈2,598**。**注: 00a §6「構築+テスト 325–455」は詳細設計を含まない別スコープ**で単純合算不可。
- **🚫 2026-08-17 スコープ縮小（移行のアプリ側移管、NFR-MIG-001）**: **DU-U10-04（14 人日）が対象外**。**DU-U10-11（8 人日）は同日訂正で条件付き復活**（アプリが Admin REST API でハッシュ投入する場合、旧方式が Keycloak ネイティブでなければ基盤側 SPI が必要。要否は B-MIG-1 の回答次第）。**詳細設計+構築 ≈1,044 → ≈1,030 人日**（U10 は 110 → 96。DU-U10-11 の 8 人日は条件付きで残す）。別枠の [00a §6 構築見積](00a-remaining-tasks-and-effort.md) からも **I-12 移行ツール 12–18 人日が落ちる**（325–455 → **313–437**）。**この 2 枠はスコープが異なるため合算しないこと**（前者=詳細設計+構築 / 後者=構築+テスト）。テスト枠 363 に移行専用項目は無いため変動なし。**ServiceNow の並走 4 Phase（DU-U10-08）は旧システム移行とは別物のため対象外にしない**。
