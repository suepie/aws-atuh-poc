...
- **工数（人日・工程分割、[網羅監査 §9](research/wbs-gap-audit-2026-08-12.md)）**: 全 DU（既存 + §12c GAP 28）= 詳細設計+構築 ≈**1,044 人日**（**②詳細設計 418 + ③構築 626**、40/60 分割。IaC/Config/Doc 系は②③融合）。**①基本設計 269 + 条件付き 65**（[00a §1-4 v2](00a-remaining-tasks-and-effort.md)、2026-08-26 粒度精緻化。旧 244）・**④テスト ≈363**（フルスコープ+フル再計上、[監査レポート §10](research/wbs-gap-audit-2026-08-12.md) で 30 項目に精緻化）は別掲。総計 ≈**1,676**（+① 条件付き 65 で上限 ≈1,741）。**注: 00a §6「構築+テスト 325–455」は詳細設計を含まない別スコープ**で単純合算不可。S/M/L は配分目安、確定は詳細設計時に PoC 実測で再見積。
