...
> ⚠ **旧記述の訂正**: 当初「**事前に利用者を知れないテナントには適用できない**」と書いたが**誤り**。テナント管理者が画面から利用者を追加していく運用なら、**2 人目以降は全員このフローに乗る**（名簿も SCIM も不要）。事前に知る必要があるのは**最初の 1 人だけ**で、これは初期構築の問題（`QA-BIND-07`）。

**判断軸は「利用者を知れるか」ではなく「権限を個別に付けるか」。**

| [`Q86`](../../requirements/customer-questionnaire/06-additional.md) 初期権限の回答 | 事前登録の負担 | 素直な方式 |
|---|---|---|
| **A: 何も与えない**（最小権限・弊社推奨） | **ゼロ**。どのみち管理者が権限を付ける操作が要る。**先にやるか後にやるかの違いだけ** | **案 Y（事前登録）** |
| **B: 全員に共通の基本権限** | **純増**。本来は無操作でよかったのに登録作業が発生する | **現行 §8.1（JIT）** |

**`Q86` で A（最小権限）を選ぶなら、事前登録は「もともと必要な操作を前倒しするだけ」**であり負担にならない。むしろ「ログインできたが権限が無い」という中途半端な状態が消える。

**併存が必要になる本当の条件**は「利用者を知れないから」ではなく、**テナントが「IdP にいる人は全員入れてよい」という運用を望むから**。したがって**テナントごとの選択**として設計できれば、併存は弱点ではなく**顧客の運用に合わせた選択肢**になる。

| テナントの運用 | 方式 |
|---|---|
| 権限を個別に付ける | **案 Y（事前登録）** |
| IdP にいる人は全員入れてよい | 現行 §8.1（JIT） |
| 大量採用があり登録漏れが業務を止める | 現行 §8.1（または登録の自動化 = SCIM） |

**`Q86` と `QA-BIND-06` は実質同じ判断**（テナントが権限を個別管理するか否か）であり、統合を検討する。
