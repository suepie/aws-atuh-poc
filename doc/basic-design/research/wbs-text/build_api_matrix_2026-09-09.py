...
('Broker','F-ADM-02','ユーザの詳細参照','GET /admin/realms/{r}/users/{id}',ADMIN,LIM,''),
('Broker','F-ADM-03','ユーザの登録','POST /admin/realms/{r}/users',ADMIN,LIM,''),
('Broker','F-ADM-04','ユーザの変更','PUT /admin/realms/{r}/users/{id}',ADMIN,LIM,'属性の追加は User Profile で宣言した項目のみ'),
...
('Broker','F-ADM-06','ユーザの削除','DELETE /admin/realms/{r}/users/{id}',ADMIN,LIM,'Phase 1 は消さずに止める方針のため通常は使わない'),
...
('Broker','F-PROV-04','ユーザ更新の受信（SCIM）','（標準に無い）',SYS,CUS,'同上'),
('Broker','F-PROV-05','削除通知の受信（SCIM）','（標準に無い）',SYS,CUS,'同上'),
('Broker','F-PROV-06','ユーザ検索の受信（SCIM）','（標準に無い）',SYS,CUS,'同上'),
...
('IdP','F-AUTH-11','認証手段の一覧（本人）','GET /realms/{r}/account/credentials',USER,STD,'本人のトークンで呼べる。登録済みの手段の確認に使う'),
('IdP','F-AUTH-13','認証手段の削除（本人）','DELETE /realms/{r}/account/credentials/{id}',USER,LIM,'標準にあるが非推奨の扱い。画面へ誘導する方が安全'),
('IdP','F-ADM-04','自分の情報の参照・更新','GET/POST /realms/{r}/account/',USER,STD,'**本人のトークンで呼べる**。管理 API を渡さずにプロフィール編集を実現できる'),
...
('IdP','F-AUTH-11','同意した先の確認・取り消し','GET /realms/{r}/account/applications・DELETE .../applications/{clientId}/consent',USER,STD,'**G-11**。本人が「どのアプリに情報を渡したか」を見て取り消す。同意画面を使わない運用なら不要'),
...
SKIP = {
 ...
 'F-PROV-07': 'Phase 1 対象外（SCIM のグループ連携）',
