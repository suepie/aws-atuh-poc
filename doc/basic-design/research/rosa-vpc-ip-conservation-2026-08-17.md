...
- Node 3×/26 = 合計 **/24 相当 = ROSA HCP 最小 Machine CIDR と一致**。使える Node IP 合計 ~177 に対し最大ノード **~32（KC max 29＋Infra、2026-08-27 フェデ 50:50 改訂）** → **なお枯れないが、ヘッドルームは約 5.5 倍**（旧 max 18 時は ~9 倍）。**ローカル 100% 振れ（KC 58 ノード）では Node 層を /25 へ拡張が必要**。Pod はオーバーレイなのでノード増でも ENI 分しか増えない。
