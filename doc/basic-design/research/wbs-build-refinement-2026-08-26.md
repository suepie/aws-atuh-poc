...
# 【v2 追補 2026-08-26】一般観点 × ADR 突合による ②③ の漏れ是正

①基本設計で行った二段突合（汎用体系 → 案件固有 ADR）を ②詳細設計・③構築にも適用した結果、**2 つの重大な欠落**が判明した。

## 7. 欠落① 横断・共通作業が完全にゼロ計上（③構築）

00b の DU は「**作るモノ**」の単位であり、**モノに紐づかない横断作業が 1 つも計上されていない**。機械検索の結果:
...
→ **新クラスタ BX（横断・共通）= 117 人日**を新設する。実装 618 人日に対し **約 19%**（SIer 一般の「管理・レビュー・手戻りは実装の 15〜25%」の範囲内）。

| ID | 項目 | 内訳 | 人日 |
|---|---|---|---:|
| BX-01 | **開発環境構築** | ローカル開発環境（コンテナ/モック KC）5; 共有 dev 環境 3; オンボーディング手順 2 | 10 |
| BX-02 | **共通ライブラリ・実装標準・サンプル** | 共通部品（ログ/エラー/認可クライアント）6; 実装標準ドキュメント 3; リファレンス実装 3 | 12 |
| BX-03 | **コードレビュー**（全 DU 横断） | 実装 618 × 約 4% | 25 |
| BX-04 | **バグ修正・手戻り**（単体〜結合前） | 実装 618 × 約 5% | 30 |
| BX-05 | **静的解析・脆弱性の是正対応** | Trivy/SAST 検出後の修正・例外申請（検出基盤は DU-U7-13） | 10 |
| BX-06 | **実装ドキュメント** | README/構成図/運用引き継ぎ資料 | 8 |
| BX-07 | **リリース準備** | 本番切替手順 4; リリース判定基準 3; 切り戻し手順 3 | 10 |
| BX-08 | **進捗・課題管理**（構築フェーズ） | 課題管理・週次同期・エスカレーション | 12 |
| | **BX 小計** | | **117** |

> **注**: BX-03/04 は各 DU の人日に含まれていない（DU の内訳は「設計・実装・単体」で、レビュー往復と手戻りは別）。TE-01（テスト環境、④テスト側）とは別物 — BX-01 は**開発**環境。

## 8. 欠落② ADR 突合による DU の抜け（③構築）

**00b が明示参照している ADR は 11 本のみ**（040/046/051/056/059/062〜067）。残り 57 本を内容キーワードで突合した結果:

### 8.1 カバー済み（名称違いで参照が無いだけ・対応不要）

ADR-023(SN→DU-U10-01) / 025(SCIM→U3-03) / 038(管理画面→U10-02) / 041(SPIFFE→**IRSA 採用で U7-05**) / 042(Bot→U7-09 最低線) / 043(A11y→U4-06) / 044(Tabletop→U9O-03) / 045(鍵→U7-01) / 048(DSAR→U10-05) / 051(DR→U8 群) / 052(Rate Limit→U6-10) / 053(Observability→U9O-01) / 054(**マッピング DB→U3-01 idmap**) / 055(HRD→U2-04) / 057(CSRF→U4-02)

### 8.2 真の漏れ → ベースへ追加（+14 人日）

| DU | 由来 ADR | 内容 | 人日 |
|---|---|---|---:|
| **DU-U9O-10** 🆕 | **ADR-061** | **デプロイ検知の pull 型統一**（中央巡回による発見・差分検知）。DU-U9O-05 Canary は外形監視で別機能 | 8 |
| **DU-U7-18** 🆕 | **ADR-060** | **攻撃経路 33 の実装対応確認・トレース表**（各 DU に散在する対策の突合と証跡化） | 6 |

### 8.3 Phase 1 スコープが未確定 → 分岐プールへ（CB-7/8）

| 分岐 | 由来 ADR | 内容 | 増分 |
|---|---|---|---:|
| **CB-7** | **ADR-034** | **Adaptive Authentication（リスクベース認証）**が Phase 1 スコープと確定した場合。現状 DU 皆無で、ITDR（U7-03＝検知）とは別機能（ログイン時のリスク評価→step-up） | +15 |
| **CB-8** | **ADR-036** | **顧客監査支援（監査ログ提供機能 / Trust Center）**を実装する場合。B-PCI-7 未回答。現状「都度メール対応」に縮小中 | +10 |

### 8.4 Phase 2/3 として明示（Phase 1 は 0 計上・対象外を宣言）

**ADR-050 モバイル SDK**（Phase 2）/ **ADR-047 PQC 移行**（Phase 3、姿勢のみ）/ **ADR-042 Turnstile 導入**（Phase 2）/ **ADR-049 TPRM**（契約・運用側で 00a F へ）。→ **「未検討」ではなく「意図的に Phase 1 対象外」であることを WBS 上で可視化**する（後から「漏れ」と誤認されないため）。

## 9. ②詳細設計の成果物定義（按分 412 の内訳を明示）

②は「1,030 × 40%」の**機械按分のみで、成果物が未定義**だった（①で H を作ったのと同じ問題）。按分値 412 に対し、成果物ベースで積むと:

| 成果物 | 単位 | 人日 |
|---|---|---:|
| DU 別 詳細設計書（クラス/シーケンス/処理フロー） | 103 DU × 2 | 206 |
| DB 物理設計書（DDL・インデックス・パーティション） | 一式 | 15 |
| 内部モジュール I/F 設計 | 一式 | 20 |
| **単体テスト仕様書** | 103 DU × 0.8 | 82 |
| 画面詳細設計（イベント・バリデーション） | 32 画面 × 0.5 | 16 |
| バッチ詳細設計（処理フロー・リカバリ） | 8 本 × 1 | 8 |
| 詳細設計レビュー・改訂 | 一式 | 25 |
| **積み上げ計** | | **372** |

→ **按分 412 の範囲内（余裕 40）で成立**。よって**②の金額は据え置き**とし、上表を「詳細設計フェーズで何を作るか」の定義として採用する。**単体テスト仕様書（82）が②に含まれる**点は要注意（③の「実装+単体」は実施側）。

## 10. 是正後の集計

| 区分 | 旧 | 新 | 差分 |
|---|---:|---:|---:|
| DU ベース（詳細設計+構築） | 1,030 | **1,044** | +14（8.2 の 2 DU） |
| ② 詳細設計（×40%） | 412 | **418** | +6 |
| ③ 構築（×60%） | 618 | **626** | +8 |
| **③ 横断・共通 BX** 🆕 | — | **117** | +117 |
| **③ 合計** | 618 | **743** | +125 |
| 構築分岐 CB-* | 73 | **98** | +25（CB-7/8） |

**工程総計**: ① 693 + ② 418 + ③ 743 + ④ 421 = **ベース 2,275** / 条件付き **300**（設計 152 + 構築 98 + テスト 50）→ **上限 2,575**。

> ⚠ **2026-08-26 追記: §9 の②成果物定義（7 行・積上 372）は非アトミックのため stale**。正は **[wbs-detailed-design-refinement-2026-08-26.md](wbs-detailed-design-refinement-2026-08-26.md)（64 項目・**441 人日**）**。按分 418 では **23 人日不足**（DD-G IaC 詳細設計 26 と DDJ-06 進捗管理 8 が DU 按分に馴染まないため）。
