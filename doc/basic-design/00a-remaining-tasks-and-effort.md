...
| **ベース合計(期待経路)** | | **51** | **699** | | **総計 730 人日**（C 完了 -22 / PoC 絞り込み -17） |
...
| A-7 | 切替先の機材確保 | 在庫・上限・対応可否（**PoC ではなく AWS / ベンダーへの照会**で足りるため 3 → 1 に縮小） | 1 | — |
...
| | **A 小計** | **21 行（旧 26 行）** | **40**（災害対策のスコープ外により -1） | |
...
| ~~B-4.2~~ | ~~災害時の入口切替に関する合意資料~~ ❌ **Phase 1 スコープ外** | DR 方式再判断(D-18.2)結果を反映した合意資料 | 0 | **D-18.2** |
| B-5 | 死活監視を攻撃と誤検知させないための合意 | Canary 送信元の WAF 誤検知除外を REQ 追補へ | 1 | B-1.1 |
| | **B 小計** | | **21**（災害対策のスコープ外により -2） | |
...
| ~~**D-7a**~~ | ~~検知データを切替先にも置くか~~ ❌ **Phase 1 スコープ外** | 検知の判定材料（端末・場所などの履歴）を保持する DB を大阪側にも持つか。持たないと切替直後に**全員が「初めての端末」扱い**になり追加確認が多発する（ITDR 用 DynamoDB、U7 O-U7-7）| 0 | D-18.2 |
| **D-7b** | **緊急時の入り口をどう用意するか**（同一地域内の非常口へ縮小） | 通常経路が使えないときに管理者が入るための仕組みを、大阪側にどう実体として持つか（Break-Glass、U8 O-U8-9）。**平時にクラスタが無い方式のため、入り口だけ先に用意できるかが論点** | **1** | D-18.2 |
...
| **D-18** | **災害対策をスコープ外とする判断の記録と、顧客への説明資料** | 「別地域への切替は行わない」判断の根拠・リスク・契約文言案・**将来復活する場合の追加見積**をまとめる | 2 | — |
...
| | **D 小計** | **31 行** | **45.5**（災害対策のスコープ外 -11、番号予約 +0.5） | |

**D 分岐表**:

| 分岐 ID | トリガー | 追加作業 | 増分 |
|---|---|---|---:|
| ~~C-D18~~ | ~~一部常時稼働の方式を決定した場合~~ ❌ **2026-09-02 削除**（災害対策をスコープ外としたため） | — | 0 |
...
| ~~G-1.8~~ | ~~手順書: 災害時の切替判断~~ ❌ **Phase 1 スコープ外（2026-09-02 D-21）** | 判断基準・ITDR フラグ(K-4) | 0 |
| ~~G-1.9~~ | ~~手順書: 切替先での再構築~~ ❌ **Phase 1 スコープ外** | ROSA 再構築+IaC 適用(D-18 決定方式で書く) | 0 |
| G-1.10 | 手順書: データの復元 | PITR/イミュータブル SS リストア(論理破壊系) | 2 |
| ~~G-1.11~~ | ~~手順書: 元の地域への復帰~~ ❌ **Phase 1 スコープ外** | 東京復帰・整合確認 | 0 |
...
- 運用: 分岐が顕在化したら該当 C-* をベースに繰入れ、§1 サマリを更新する。**予算計画上は「730 + リスク引当 149 = 879 人日を上限」**と読む。
