...
| A-11 | 小粒実機確認バンドル | O-U8-6(multi-cluster v2 版数)、O-U7-8(ROSA SA issuer JWKS)、E-11(Red Hat docs 403 分: EUS/Operator チャネル/監査ログ制約の本文確認)、**RFC 8414 形式 `/.well-known/oauth-authorization-server/realms/{realm}` の提供有無**(RFC 8414 準拠 RP・MCP 系クライアントの疎通可否。[kc#45271](https://github.com/keycloak/keycloak/issues/45271)、U5 §5.6.3b)、**O-U7-10(`kc_idp_hint` のテナント存在オラクル。存在する別名 / 存在しない別名 / Org 非紐付け別名の 3 パターンで応答・応答時間を比較 + SAML `/broker/<alias>/endpoint/descriptor` の到達性。成立すれば SPI で AuthNote 除去〔案 A〕。U7 §7.8.1a D-U7-19)** | **5 点セット**で 1 バッチ化 | 2 | — |
...
| # | タスク | 包含 ID | 人日 |
|---|---|---|---:|
| G-1 | **Runbook Phase 1 必須 13 冊の執筆**(TEN-01/03、**SEC-01/02/04/05/06**、DR-00〜05。仕様は各単元確定済み) | D-U9-07 | 16 |
| | ⚠ **2026-08-18 是正**: 旧記述は「SEC-01〜06」で合計 **14 冊**となり「13 冊」と矛盾していた。**[U9 D-U9-07](09-operations-observability-design.md) の定義（SEC-03 を含まない 5 冊）が正**。RB-SEC-03（緊急 IP 制限）はエッジ WAF 部分が他組織への依頼手順（REQ-IN-01）に依存するため、他組織回答が出るまで必須 13 冊に含めない | — | — |
