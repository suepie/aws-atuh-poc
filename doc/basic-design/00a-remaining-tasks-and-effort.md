...
作成: 2026-07-30 / ステータス: **v2.1(2026-08-26)** — v2(原子タスク分解 + 場合分け + 8 月決定同期) に **クラスタ H「設計成果物の執筆」280 人日を追加**(設計項目の網羅洗い出し。従来ゼロ計上だった最大の漏れ)
...
| **H. 設計成果物の執筆** 🆕 | **設計項目（機能/画面/データ/IF/バッチ/権限）の網羅と資料作成**。**v2 まで工数ゼロ計上だった最大の漏れ** | — | **280** | +87 | 一部 顧客レビュー |
| **ベース合計(期待経路)** | | **108** | **441** | | **総計 549 人日** |
...
**体制目安**: **クラスタ H(280 人日) 追加により大幅増** — 2 名専任で約 13 ヶ月 / 3 名で約 9 ヶ月 / 4 名で約 7 ヶ月。設計成果物(H)は調査系(A)と並走可能なため、**A/H の 2 系統並走**が最も効く。壁時計の律速は外部リードタイム(他組織回答・顧客ヒアリング・Red Hat DPA/照会)— W0 で外部依頼を先に投げるのが最重要(§4)。
...
| 群 | 主な成果物（❌ = 既存カバー皆無） | 人日 |
|---|---|---:|
| H-A 全体・方式 | 総則/全体構成図・**方式設計書(エラー/ログ/Tx/文字コード)**・設計標準規約・NFR 対応表・**❌トレーサビリティマトリクス** | 35 |
| H-B 機能設計 | **❌機能一覧・機能 ID 体系**（約 70 機能の洗い出し）・**❌機能仕様書 70 本**・業務フロー 15・状態遷移設計 | 68 |
| H-C 画面設計 | 画面一覧/遷移図（約 32 画面）・**❌画面仕様書 32 本**・UI 標準(A11y)・**❌メッセージ/エラーコード一覧** | 38 |
| H-D データ設計 | **❌ER 図**・**❌テーブル定義書(自前 20)**・データディクショナリ・**❌コード体系**・**❌CRUD 図** | 36 |
| H-E IF 設計 | **❌IF 一覧**・API 仕様書(idm-api 約 30EP)・SCIM IF・イベント IF(EventBridge/outbox/Webhook 7 種)・認証プロトコル IF・外部システム IF | 48 |
| H-F バッチ設計 | **❌バッチ一覧**(8 本)・バッチ仕様書・ジョブスケジュール | 13 |
| H-G 権限・監査 | **❌権限マトリクス**(ロール×機能×データ×テナント)・監査ログ取得項目一覧 | 14 |
| H-H 運用補完 | 監視項目一覧・閾値表 | 6 |
| H-I レビュー・合意 | 全体レビュー/整合性確認・ステークホルダー説明資料とレビュー対応 | 22 |
| | **H 小計** | **280** |

**規模前提**（変動時は CH-* で吸収）: 機能 約 70 / 画面 約 32 / 自前テーブル 約 20 / API 約 45EP / バッチ 8。**単価 = 棚卸し + 執筆 + 図版 + レビュー 2 巡 + 改訂**（±40% 込み）。
**精度向上の要点**: **H-06 機能一覧の完了時点で規模前提を実測値に更新し、H-07/H-11 を再見積り**するのが最も効く。
...
- 運用: 分岐が顕在化したら該当 C-* をベースに繰入れ、§1 サマリを更新する。**予算計画上は「549 + リスク引当 152 = 701 人日を上限」**と読む。
