...
> **★2026-09-25 追記 — REQ-IN-13 の選択は DR 切替の主導権を決める（[U8 §8.4.6](08-availability-dr-design.md) / O-U8-15）**:
> - **(a) パブリック custom origin** を採ると、オリジンを**弊社管理の FQDN（`origin-auth.basis.example.com` 等）で指定**できるため、**DR 切替が「弊社 Public Zone の `origin-*` レコード書換」だけで完結**する（U8 **D-U8-16 案 C** / **REQ-DR-06**）。他組織の作業は初期設定 1 回のみ。
> - **(b) VPC origins** を採ると、AWS 公式のとおり**オリジンはリソース ARN で直接指定され DNS による間接化が存在しない**（[公式](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-vpc-origins.html): 「For **Origin ARN**, select the ARN of your Application Load Balancer…」）。DR 切替は**大阪用 VPC origin の新規作成 → distribution から旧 origin を切離 → 付け替え（Deployed 待ち最大 15 分）**となり、**他組織作業（U8 案 A）が必須**になる。大阪用 VPC origin は**対象 ALB が Active になるまで作成できない**ため作り置きも不可。
> - **RTO 3 日なら (a)(b) いずれでも成立する**（15 分の待ちは誤差）ため、**DR 側から REQ-IN-13 への制約は課さない**。ただし「(a) なら DR 切替が自社完結する」ことを選択の判断材料として記録する。大阪（ap-northeast-3）は VPC origins 対応リージョン、東京は `apne1-az3` のみ除外。
> - **(a) 採用時の付帯条件**: ①大阪側エッジ LB にも `origin-*.basis.example.com` の証明書が必要（ALB 用 ACM はリージョン内資源）②**CloudFront は custom origin へ Host をオリジンドメイン名に書き換えて送る**ため、エッジ LB のホストベースルーティングが `origin-*` を受けるか、Origin Request Policy で viewer の Host を転送すること（KC 側は `hostname` 固定のため `iss` に影響なし。`proxy-headers` / XFF 信頼チェーンと整合させる）。
...
- 2026-09-25: **REQ-IN-13 と DR 切替の連動を明記**（§6.7.1 表下に追記、[U8 §8.4.6](08-availability-dr-design.md) / O-U8-15）。**(a) パブリック custom origin なら DR 切替が弊社の `origin-*` レコード書換だけで完結**（U8 D-U8-16 案 C / REQ-DR-06）/ **(b) VPC origins は AWS 公式どおりリソース ARN 指定で DNS 間接化が存在せず、DR 切替は他組織の VPC origin 付け替え（Deployed 待ち最大 15 分）**になる。RTO 3 日なら双方成立のため DR 側から制約は課さないが選択の判断材料として記録。(a) 採用時の付帯条件 2 点（大阪側 `origin-*` 証明書 / CloudFront が custom origin へ Host をオリジンドメイン名に書き換える点）も明記。あわせて §6.3.1 に **PHZ 構成規約（同名 2 面持ち / ゾーン名は狭く / Resolver ルールが PHZ より優先）** を追記。
