...
| REQ-IN-12 | **api.basis（idm-api Lambda）inbound の NFW 通過要否 + 到達方式**（2026-08-06、Q1(a) 具体化。予約採番を実体化。O-APP-1 と同一クラス、REQ-IN-13 連動） | 現状 `CloudFront → API GW → Lambda ネイティブ invoke` は**マネージドサービス経由で他組織 NFW を通らない**（auth./idp. は NFW 通過）。**全 inbound を NFW 通過とする組織方針なら api. も通す経路が必要**。3 案: **(1) NFW 経路外を exception 許容**（CloudFront+WAF+API GW authorizer+アプリ層 authz で防御、waiver 取得）/ **(2) ALB→Lambda ターゲットで NFW 経路に載せる**（CloudFront+WAF→NFW→TGW→自 Internal ALB〔host: api.→Lambda ターゲット群〕→Lambda。**auth.basis と同一エッジチェーン再利用**、REQ-IN-13。API GW を使わず JWT 検証は Lambda 内・throttle は WAF）/ **(3) Private API GW を NFW 経路に**（ALB IP ターゲット=execute-api ENI、API GW 機能維持だが ENI IP 動的追従、O-APP-1 と同じ）。**弊社推奨 = NFW 必須なら (2)**（第 2 のインバウンドパターンを作らず既存チェーンに host rule 相乗り、idm-api は機微な管理 API ゆえ NFW 検査は妥当）。exception 可なら (1)。**先方の NFW 方針回答が要**（REQ-IN-13 と同時） | 他組織回答、[U10 §10.2](10-integration-migration-design.md) |
