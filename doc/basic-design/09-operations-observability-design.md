...
| **12** | **証明書の残存有効期間**（内部 NLB server-TLS / ACM PCA 発行分 / ALB 公開証明書） | ACM・PCA メトリクス | **30 日前に警告 / 7 日前に重大**。**切れると内部通信が全断**する。2026-08-18 追加（D-U9-21） |
| **13** | **バックアップジョブの成否**（AWS Backup / Aurora スナップショット / クロスリージョンコピー） | AWS Backup イベント | **失敗 1 件で即アラート**。**黙って失敗していると DR 実施時に初めて気づく**。2026-08-18 追加（D-U9-21） |
...
| **18** | **ノード時刻の基準時刻からのずれ**（chrony `offset` / `stratum` / 最終同期からの経過時間） | Node Exporter（`node_timex_offset_seconds` 等） | **ずれ ±1 秒で警告 / ±5 秒で重大**。加えて**「最終同期からの経過時間」を存在確認**する（同期が止まってもずれが顕在化するまで無症状のため、offset だけでは検知できない）。**観点「沈黙」（D-U9-22）**。切れると `auth_time`/`max_age`・TOTP・ID Token 検証が同時に壊れ、症状は「原因不明のトークン検証エラー」としか出ない。[U6 D-U6-17](06-infra-network-design.md) / 2026-08-24 追加 |
| **19** | **HRD 解決の失敗率と探索パターン**（単一送信元からの distinct テナント別名 / distinct メールドメインの試行数） | **HRD SPI ② の手動計装 + SPI ③ Event Listener → ITDR** | **顧客企業名の列挙探索を検知する唯一の手段**。テナント存在の秘匿は Identifier-First に原理的に不可能（[U4 §4.1.3](04-auth-ux-design.md)）なため、**防御は「大量列挙の検知と抑止」に置く**。⚠ **Keycloak Brute Force Protection はパスワード試行の失敗を数えるものであり、HRD 探索（パスワード試行を伴わない）はカウントしない** — 既存の対策では素通りする。閾値は [U7 §7.8](07-security-compliance-design.md) と合同確定。2026-08-24 追加 |
...
> ⚠ **番号重複（2026-08-24 検出、未是正）**: 上表は `12` `13` `14` が**二重に採番**されている（D-U9-21 / D-U9-23 で追加した太字行と、初版からの 3 行）。既存文書からの参照があるため機械的な振り直しは行わず、**参照時は番号ではなく項目名で特定すること**。恒久是正は次回の U9 改訂時に行う。
...
**背景**: 要件側は `NFR-OPS-001`「監視・メトリクス」`NFR-OPS-002`「アラート通知」の**各一行**しかなく、「何を監視するか」の網羅を担保する仕組みが無い。実際、サービス全量との突合で**5 項目の監視漏れ**が出た（D-U9-21）。個別に足しても同じ漏れが再発するため、**障害の型で分類し、各型に最低 1 つのメトリクスを持つことを規約化**する。

| # | 観点 | 何を捉えるか | 本基盤での代表例 |
|---|---|---|---|
| 1 | **飽和** | 資源を使い切る | CPU / メモリ / **ディスク使用率**（#14）/ **DB 接続プール**（D-U6-08 の接続予算） |
| 2 | **枯渇** | サービス側の上限に当たる | **AWS サービスクォータ**（Lambda 同時実行 / API GW スロットル / KMS リクエストレート）。**1000 IdP 規模でまさに効く観点** |
| 3 | **期限** | 時間切れで止まる | **証明書の残存有効期間**（#12）/ 署名鍵の並走期間 / シークレットのローテ期日 |
| 4 | **停止** | 動くはずのものが動かない | **バックアップジョブの成否**（#13）/ 退職者遮断バッチ / リコンサイル |
| 5 | **滞留** | 溜まって流れない | **DLQ**（idm-api / 糊 Lambda）/ **削除伝播 outbox の未送出**（ADR-064）/ レプリケーション遅延 |
| 6 | **逸脱** | 宣言と実態がずれる | **IaC ドリフト検知**（§9.5.3、D-U8-07） |
| 7 | **品質** | 遅い・失敗する | エラー率 / レイテンシ p99 / **SLO Burn Rate**（D-U9-04） |
| 8 | **沈黙** | **来るはずのものが来ない** | **SCIM 24h 無受信**（#11）/ **マスク処理件数の突然のゼロ**（#10）/ ログ途絶 / **時刻同期の停止**（#18 — ずれが顕在化するまで完全に無症状、2026-08-24 追加） |

**採用**: **メトリクスを追加・見直しする際は、上表 8 観点それぞれに最低 1 つの項目があることを確認する**。`tools/check_design_consistency.py` の検査 D がこれを機械照合する。

**8 番目「沈黙」の重要性**: 本基盤で既に 2 件採用している（SCIM 無受信・マスク件数ゼロ）が、**この型は「異常値が出る」のではなく「何も出なくなる」ため、閾値監視では原理的に捉えられない**。存在確認（ハートビート）の形で設計する必要がある。**D-U9-21 で追加した「バックアップジョブの成否」も本質的にはこの型**（失敗通知が来ないのか、そもそもジョブが起動していないのかを区別する必要がある）。

**未充足（2026-08-18 時点）**: **観点 2「枯渇」に対応するメトリクスが存在しない**。AWS サービスクォータの逼迫は 1000 IdP / 10M MAU 規模で最初に当たる壁であり、**Service Quotas の使用率監視を追加する**（残タスク）。

---

#### 9.1.2a 決定 D-U9-21: 監視対象の網羅性是正（2026-08-18 新設）

**背景**: コスト見積り 35 項目（= 使用サービスの全量）と本節の監視項目を突合したところ、**5 サービスが監視対象に入っていなかった**。いずれも「壊れても即座には気づかず、必要になった瞬間に initially 判明する」類の障害である点が共通する。

| 追加項目 | 気づかないとどうなるか |
|---|---|
| 証明書の残存有効期間 | **内部通信が全断**。ACM PCA 発行の内部 NLB 証明書が切れると idm-api → Keycloak Admin API が不通になり、管理操作が一切できなくなる |
| バックアップジョブの成否 | **DR の前提が黙って壊れる**。実施時に初めて「控えが無い」と判明する最悪のパターン |
| ノードのディスク使用率 | ログ滞留・イメージ蓄積で枯渇し、Pod が起動しなくなる |
| SES のバウンス率・苦情率 | 閾値超過が続くと **AWS 側で送信を停止**され、招待メールが全断する |
| Route 53 ヘルスチェックの状態 | DR の自動切替（[U8 D-U8-15](08-availability-dr-design.md)）がこれに依存。誤発火や戻し忘れに気づけない |

**採用**: 上表 5 項目を §9.1.2 のメトリクス表に #12〜#16 として追加し、アラート閾値を定める。**証明書とバックアップの 2 件は Runbook の定期点検項目にも入れる**（監視の二重化。監視自体が壊れていた場合の受け皿）。
...
| 認可コンテキスト（`/api/me/context`、**NFR-PERF-003**） | 99.9% | **< 300ms（暫定 — G-SCIM 実測で確定）** | 43.8 分 |
...
**性能目標の明示（NFR-PERF-003 / 004、2026-08-10 追加）**: ①**権限チェック応答時間（NFR-PERF-003）= `/api/me/context` p99 < 300ms（暫定）**。ブランドローカル read（Keycloak 非経由・越境ゼロ、U3 D3-16）を前提とし、確定値は **G-SCIM（10M 規模の読取 p99）で実測**して凍結する。②**トークン検証スループット（NFR-PERF-004）**: 通行証（JWT）検証は **RP 側オフライン（JWKS 署名検証・基盤往復なし）** ゆえ基盤スループットの律速ではない（JWKS 配信 99.99% / < 100ms が RP 検証を支える、上表）。基盤側の**発行**スループット目標は **U6 サイジング予算（15 req/s/vCPU × Pod 数 + 150% ヘッドルーム、§6.5）** に従い、朝ピークの `/token` 発行レートを **PoC（A-8 Argon2id / C-1 サイジング）で実測して確定**する。
...
| **RB-DR-00〜05** | DR 系 6 冊（本書は U8 §8.7.2 仕様の**文書実体化のみ**を担う。**2026-07-30 D-18 コールド DR 反映**）: 00 判定チェックリスト（**リージョン災害/論理破壊の切り分け + ITDR 抑制/強化フラグ切替**: G-2/G-3 通知のみ降格 + Brute Force 感度引上げ、U7 §7.2.3）/ 01 データ復元（(A) Aurora Global Promote × 2 系統 / (B) 不変スナップショット + PITR 復元）/ 02 DNS・エッジ**手動**切替（他組織連絡、自動不要）/ 03 **大阪オンデマンド再構築**（IaC: VPC/ROSA×2/Machine Pool/Operator/KC CR/SPI/Facade + 構成整合 + 検証）/ 04 **論理破壊リストア**（L2 PITR 細粒度 / L3 不変スナップショット復元）/ 05 フェイルバック（**冒頭に禁止 3 操作**） | U8 §8.7.2 / §8.9.2 / D-U8-14 |
...
**採用**: **idm-api（ブランド管理 API）+ 非同期の糊（射影フィード / Webhook Dispatcher / shadow 制御）は Lambda**（[ADR-062](../adr/062-idm-api-execution-form-lambda.md)、O-9。トポロジは [ADR-063](../adr/063-brand-unit-architecture.md) = ブランド主役 + 中央 shadow 制御）。**Keycloak（ROSA/GitOps、§9.6.1〜9.6.3）とは別パイプライン**とし、これが **auth-critical な Keycloak（P0）と管理ツール（P1）の障害ドメイン分離の実体**（ADR-062）。

| 項目 | 決定 |
|---|---|
| CI | **GitHub Actions**（Keycloak と同じ VCS だが別ワークフロー） |
| ビルド/配布 | **コンテナイメージ Lambda** を **ECR** へ push（RHBK と同じ ECR、digest 指定）→ **SAM / CDK** で **2 アカウント**（Broker Acct = shadow 制御 / ブランド = IdP-KC Acct = idm-api）へデプロイ（[U10 D-U10-07](10-integration-migration-design.md)） |
...
- **根拠**: [ADR-062](../adr/062-idm-api-execution-form-lambda.md)（Lambda・別障害ドメイン）/ [ADR-063](../adr/063-brand-unit-architecture.md)（ブランド主役 + 中央 shadow 制御）/ [U10 §10.2](10-integration-migration-design.md)（2 アカウントデプロイ）/ [idm-api-ingress 比較ノート](research/idm-api-ingress-execution-comparison-notes.md)（dev/release）。
...
- **補足 3（★2026-09-25 新設 — DR 対応、[U8 REQ-DR-08](08-availability-dr-design.md)）**: **顧客 IdP が送信元 IP 制限を行う場合、ステップ 1（申請）の時点で「東京・大阪両リージョンの送信元 IP（NAT EIP 等）」を顧客へ通知し、L1 顧客側作業として両方を Inbound 許可リストへ登録してもらう。** 大阪分を登録しないと、DR フェイルオーバー後に Broker KC → 顧客 IdP の token / JWKS / userinfo 呼び出しが**ドメインが同一でも失敗する**（U8 §8.4.7 (b)）。**DR 発動後に 1,000+ IdP へ個別依頼する運用は成立しないため、必ず平時作業とする。** ステップ 5 の疎通確認では東京分しか検証できない点に留意し、大阪分は Game Day（U8 §8.7.2）で抽出検証する。
  - ⚠ 前提未確定: 大阪は平時プロビジョニングなし（U8 D-U8-14）のため、**通知する大阪側 IP を平時に確定させるには NAT EIP の先行確保が必要**（U8 O-U8-13）。未確定のうちは本補足を「要確認」としてテンプレートに含めない。
...
| D-U9-18 | **idm-api + 非同期の糊（Lambda）の dev/release = Keycloak と別パイプライン**（GitHub Actions → ECR コンテナイメージ → SAM/CDK で 2 アカウント / Secrets Manager+rotation / IRSA・IAM Role / X-Ray / Lambda バッチ = EventBridge Scheduler）。障害ドメイン分離の実体（ADR-062）、ブランド主役（ADR-063） | §9.6.4 |
...
- 2026-08-06: **[ADR-062](../adr/062-idm-api-execution-form-lambda.md)（Lambda）/ [ADR-063 ブランドユニット](../adr/063-brand-unit-architecture.md)を反映** — §9.6.4 D-U9-18 新設（idm-api + 非同期の糊の Lambda dev/release パイプライン: GitHub Actions→ECR コンテナイメージ Lambda→SAM/CDK で 2 アカウント、Secrets Manager+rotation、X-Ray、Lambda バッチ = EventBridge Scheduler。Keycloak の ROSA/GitOps と別系統 = クラスタ分離の実体）。決定一覧に D-U9-18。
...
- 2026-09-25 (v1.5): **DR 対応の運用手順 2 件を追加**（[U8](08-availability-dr-design.md) D-U8-16/17 の引き渡し） — ① §9.7.1 **補足 3 新設**: 顧客 IdP が送信元 IP 制限を行う場合、オンボーディング時に**東京・大阪両リージョンの送信元 IP を通知し両方を Inbound 許可リストへ登録**させる（REQ-DR-08。DR 発動後の個別依頼は 1,000+ IdP 規模で不成立。大阪側 IP の平時確定は U8 O-U8-13 待ち）。② **RB-DR-02 の改訂**: 「Route 53 Failover の Health Check Override」→「**オリジン FQDN レコードの書換 + 伝播確認 + P4 検証**」へ差替、公開ドメインのレコード・ACM 証明書は触らない旨を手順冒頭に明記（誤操作すると `iss` 不一致で全 RP 停止）。設計決定の変更なし（U8 が SSOT）。
