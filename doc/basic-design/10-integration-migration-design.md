...
- **実行形態 = Lambda で確定（[ADR-062](../adr/062-idm-api-execution-form-lambda.md)、O-9。旧「U3-OP-3 / U6 §6.8 の裁定に従う」を解決）**。Ingress（**2026-08-06 [U6 REQ-IN-12](06-infra-network-design.md) = Option 2 で確定 — 全 inbound NFW 通過が組織方針**）= `他組織 CloudFront+WAF+ALB/NLB+NFW（api. 専用エッジ一式）→ TGW → 自 Internal ALB（host rule: api.basis → Lambda ターゲットグループ）→ idm-api #2 Lambda`（auth.basis と同一エッジチェーン再利用）。**JWT L1 検証は Lambda 内**（API GW authorizer は使わない）、**throttle は他組織 WAF rate-limit**。Lambda の VPC アタッチ（層③・専用サブネット）は egress 用で不変。**旧「API GW ネイティブ invoke」は撤回**（ADR-062 追記）。**非同期の糊（射影フィード EventBridge ハンドラ / Webhook Dispatcher / idmap 更新ハンドラ）も Lambda（層③）**。Ingress は **エッジがブランド→#2 にルーティング**（`CloudFront(api.basis, WAF) → API GW(JWT L1) → ブランドの #2 を invoke`）。Keycloak Admin API へは内部 NLB 経由（U6 D-U6-11 改訂 — **`scheme=internal` = インターネット非露出**。in-cluster〔ClusterIP〕との差は VPC 内到達経路が 1 本増える分で、SG を Lambda SG 限定 + server-TLS + アプリ層認証で緩和。#2〔IdP-KC = credential アカウント〕はこの内部到達を特に厳格に）。
...
- 2026-08-06: §10.2.1 Ingress を **REQ-IN-12 = Option 2 で確定**（全 inbound NFW 必須 → 他組織 CloudFront+WAF+NFW+ALB → TGW → 自 Internal ALB〔host: api.→Lambda ターゲット群〕→ Lambda。JWT は Lambda 内・throttle は WAF。旧 API GW ネイティブ invoke 撤回）。
