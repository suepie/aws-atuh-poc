...
| デプロイ | 配置 | 利用者・経路 | 認可 |
|---|---|---|---|
| **idm-api（ブランド管理 API）= 主役** | ブランドユニット（IdP-KC Acct、[ADR-063](../adr/063-brand-unit-architecture.md)）・Lambda | 管理 SPA + アプリ（M2M）+ 移行バッチ。**入口は idm-api、ルーティングはエッジ**（CloudFront/API GW がブランド→idm-api、JWT L1 も）。**CRUD + 権限 + authz + projection + idmap の実体** | **ユーザ AT**（`tenant-admin` + `tenant_id`）/ アプリは **CC**（`idm:*`、U5 §5.8） |
| **shadow 制御 Lambda（中央・薄い）** | Broker Acct・Lambda | **Broker shadow の enable/disable のみ**（IdP-KC 削除イベント `user.deprovisioned` で発火、EventBridge）。重い中央 front door（旧トポロジ）は置かない | 内部（EventBridge ハンドラ + S2S 認可、U5 §5.8） |
...
- **ブランド主役トポロジ（2026-08-06 [ADR-063 ブランドユニット](../adr/063-brand-unit-architecture.md)。旧 front door ①一本化を改訂）**: **idm-api（ブランド管理 API）が CRUD + 権限 + authz + projection の実体**。**入口は idm-api**、ルーティングはエッジ（CloudFront/API GW がブランド → そのブランドの idm-api へ）。**重い中央 front door（旧トポロジ）は置かない**。**中央（Broker）に残るのは shadow 制御 Lambda のみ**（IdP-KC 削除をトリガーに Broker shadow を無効化、[control-plane ノート フロー④](research/control-plane-crud-authz-flows-notes.md)）。**authz/idmap/projection はブランド内ローカル**（`sub` + `brand_id` キー、Broker は authz 非保持）。**経路⑤（同居アプリ）は App Acct に置き、idm-api を Client Credentials で（エッジ経由）呼ぶ**（IdP-KC 非同居 = PW ハッシュのブラスト半径隔離）。
- 対象 Keycloak: **idm-api（ブランド）は IdP-KC Admin API を叩く（CRUD、内部 NLB）**。**Broker Admin API を叩くのは中央 shadow 制御 Lambda のみ**（shadow 無効化、IdP-KC 削除イベント発火）。クロスアカウント越境は EventBridge の 2 本（idm-api→Broker 削除 / Broker→ブランド 初回 sub 通知、U6 §6.3.2）。
- **実行形態 = Lambda（[ADR-062](../adr/062-idm-api-execution-form-lambda.md)、O-9 解決）**。決め手 = auth-critical な Keycloak〔P0〕と管理ツール〔P1〕を別障害ドメインに分離。**Ingress = `CloudFront(api.basis, WAF) → API GW（JWT L1 + throttle）→ Lambda 直 invoke`**（VPC Link/NLB を挟まない。**api. は API GW = NFW 例外**ゆえ NFW 経路外で準拠、[U6 REQ-IN-12](06-infra-network-design.md)）。**非同期の糊（shadow 制御 / outbox リレー / Webhook Dispatcher / idmap・projection ハンドラ）も Lambda（層③）**。Keycloak Admin API へは**内部 NLB（`scheme=internal` = インターネット非露出 + SG を Lambda SG 限定 + server-TLS + アプリ層認証）**で到達（[U6 D-U6-11](06-infra-network-design.md)。idm-api = credential アカウントは特に厳格に）。
...
- **Erasure と Phase 1 物理削除禁止の整合**: ADR-048 の「論理削除 30 日 → 物理削除」は、U3 D3-09（Phase 1 物理削除禁止 + 4 層ガードレール + `deprovisioned_at` 起算の Phase 2 バッチ）が**後発の確定決定として優先**する。Phase 1 の Erasure 請求は「Soft Delete + 表示属性の仮名化（`[Anonymized-xxxxxx]`、`sub`/監査 FK は保持）」で応答し、物理削除は Phase 2 バッチ（`retention_years` 起算）に統合する。この解釈は GDPR 上は Art.17(3) の除外事由で成立する（B-SN-21 の推奨 B と同型）。**ただし APPI では成立経路が異なる（下記 D-U10-13c）**。**ADR-048 §C への注記追記をメインセッションに依頼**（§10.6.3）。

- **決定 D-U10-13c: APPI 消去請求への応答手続（2026-08-13 新設、PPC 一次資料調査による是正）**
  - **⚠ 是正の要点**: **APPI には GDPR Art.17(3)(b) のような「法的義務の遵守に必要なら消去権を適用しない」という明示の適用除外条項が存在しない**。したがって「法的義務による保持だから応答不要」という GDPR 型の運用設計は **APPI では成立しない**。PPC が保存義務のケースに用意している経路は **法 35 条 6 項ただし書 +[ガイドライン通則編 3-8-5-3 事例 3](https://www.ppc.go.jp/files/pdf/260614_guidelines01.pdf)**（「他の法令の規定により保存が義務付けられている保有個人データを遅滞なく消去する代わりに、**当該法令の規定による保存期間の終了後に消去することを約束する**場合」）である。
  - **決定的な差分**: GDPR の除外は「**何もしなくてよい**」が、**APPI 35 条 6 項ただし書は「本人の権利利益を保護するため必要なこれに代わるべき措置をとるとき」という条件付き**であり、**代替措置を取らなければただし書は発動せず本則の義務が残る**。
  - **よって Phase 1 の DSAR 消去請求への応答は次の 3 点セットを必須とする**（Runbook 化 = U9 ④ DSAR 対応）:
    1. **代替措置**: 「**保存期間の終了後に消去する**」ことを本人に約束する（= GL 3-8-5-3 事例 3。本基盤では `deprovisioned_at` + `retention_years` 起算の Phase 2 物理削除バッチ〔U3 D3-09〕がその実行手段）
    2. **通知（法 35 条 7 項・法的義務）**: 「利用停止等を**行わない旨の決定**」を**遅滞なく本人へ通知**する
    3. **理由の説明（法 36 条・努力義務）**: 当該措置をとらない理由を説明するよう努める
  - **⚠ 保持根拠の限界**: [Q&A Q9-24](https://www.ppc.go.jp/files/pdf/250701_APPI_QA.pdf) は「法令上**保管が義務付けられている**場合等」は正当な利益として考慮される一方、「**将来受ける可能性のある行政調査等**のために保管すること」は**通常考慮されない**と明示している。すなわち**「念のため保持」は正当化根拠にならない**。本基盤の監査ログ 7 年保持は **PCI DSS を Phase 1 対象外化（B-PCI-1）した後の法令根拠が未確定**であり、**U10-OP-9 として起票**する（下記）。
  - **注記**: 「法令上の保存義務があるから法 18 条 3 項 1 号『法令に基づく場合』に該当し 35 条 1 項請求を拒否できる」という論法は、GL 3-1-5(1) の事例（捜査関係事項照会・行政調査・情報提供）に**保存義務型が含まれない**ため採らない。**経路は上記の 35 条 6 項ただし書に一本化**する。
  - **✅ 認証基盤に有利な一次資料（2026-08-13 追加）**: GL 通則編 3-8-5-1(3)③ の**【利用停止等が認められないと考えられる事例】が、本基盤の用途を名指しで正当化**している —— **事例 2）「匿名の投稿を行った者が、発信者情報開示請求による特定や損害賠償請求を免れるため、プロバイダに対してその保有する『接続認証ログ』等の利用停止等を請求する場合」**／**事例 3）「利用規約違反で強制退会処分を受けた者が、再度サービスを利用するため、強制退会処分を受けたことを含むユーザー情報の利用停止等を請求する場合」**。→ **認証ログの保持と、退会・BAN 記録の保持は、削除請求が通らない類型として PPC が明示**している。§10.5 の「Soft Delete + `deprovisioned_at` 保持」「[U3 D3-09](03-identity-provisioning-design.md) の Re-Activation 判定用フラグ保持」の正当性根拠として用いる。
  - **監査ログ 7 年保持の正当化ロジック（U10-OP-9 の回答方向、4 層）**: ① **他法令の保存期間**がある範囲は GL 3-4-1 ただし書「なお、法令の定めにより保存期間等が定められている場合は、この限りではない」で消去努力義務（法 22 条）の対象外 ② [Q&A Q13-32](https://www.ppc.go.jp/files/pdf/250701_APPI_QA.pdf)「確認・記録義務の履行のために個人データを保存する場合は、この限りでない」の類推 ③ **GL 通則編 別添「講ずべき安全管理措置」がシステムログ・アクセスログの整備と定期的分析を求めている**（＝保持の**積極的**根拠、法 23 条側） ④ [Q&A Q6-2](https://www.ppc.go.jp/files/pdf/250701_APPI_QA.pdf)「**アクセスログ等が確認できない場合には、漏えい（又は漏えいのおそれ）に該当し得る**」（＝保持しないと法 26 条上不利）。**APPI 自体は 7 年を要求も禁止もしていない**ため、年数の根拠は**他法令・業界基準・時効（民法 166 条の 5 年/10 年）**に置き、**利用目的として明示＋保存期間を設定**する（**無期限保持は法 22 条と整合しにくい**）。
  - **実装方針: 「消去」＝識別不能化を含む（二段構え）**: GL 3-4-1（※）および [Q&A Q9-18](https://www.ppc.go.jp/files/pdf/250701_APPI_QA.pdf) は、法 22 条・法 35 条の「消去」に「**当該データから特定の個人を識別できないようにすること**」を含めている。→ **「ログ本体は N 年保持／識別子は M 年（M<N）で不可逆化し、ソルト・鍵・対応表も破棄する」二段構え**が、保持要求と消去義務を両立させる設計解となる。M/N の具体値は U10-OP-9 と合わせ確定。**ソルト・HMAC 鍵の分離保管と権限分離**（[U7 §7.1](07-security-compliance-design.md) / D-U7-19）は本方針の前提。
...
- **忘れられる権利 vs 7 年不変監査の境界（2026-08-12 追加、B-RTBF-1）**: 消去要求と改ざん不能監査（D-U7-13）は衝突するため、**「消去する対象」と「不変監査で tombstone/仮名化して残す対象」の境界を明文化**する（都度判断は不可）。既定 = 個人データ本体は Soft Delete→物理削除（U3 D3-09）、監査ログ内の主体識別子は**仮名化して保持**（コンプラ証跡の法的保持根拠）。境界の最終確定は hearing **B-RTBF-1**（決定ログ内 `sub` の仮名化は ADR-067 と整合）。
  - **⚠ 用語の前提（2026-08-13、[U7 §7.7.7 D-U7-22](07-security-compliance-design.md)）**: ここでいう「仮名化」は**法 23 条の安全管理措置**であり、**APPI の「仮名加工情報」（法 2 条 5 項・法 41 条）ではない**。したがって**仮名化済みの監査ログも引き続き「個人データ」**であり、**漏えい時は法 26 条の報告・本人通知の対象**（「仮名化したから報告不要」は成立しない）。仮名加工情報制度を採用しない理由は D-U7-22 参照。
...
| D-U10-07 | ユーザ管理 API = 単一 OpenAPI `idm-api` v1。**実行形態 = Lambda（ADR-062）、ブランド主役（ADR-063: idm-api が CRUD+authz+projection の実体、中央は shadow 制御 Lambda のみ、入口=idm-api/エッジルーティング、越境=EventBridge 2 本）** | §10.2.1 | U3 D3-05、U5 §5.8、U7 §7.5.2、**ADR-062/063**、U6 D-U6-11 |
...
| D-U10-13c | **APPI 消去請求への応答手続**（APPI に GDPR Art.17(3) 相当の適用除外は無い → 法 35 条 6 項ただし書 + GL 3-8-5-3 事例 3 の経路。**代替措置〔保存期間終了後に消去〕+ 35 条 7 項の通知〔法的義務〕+ 36 条の理由説明〔努力義務〕の 3 点セット必須**） | §10.5 | PPC GL 通則編 3-8-5-3 / Q&A Q9-24、U10-OP-9 |
...
- 2026-08-06: §10.2.1 Ingress を **REQ-IN-12 訂正 = API GW 例外で元設計維持**（全 inbound NFW 必須だが静的 SPA・API GW は例外 → CloudFront→API GW→Lambda のまま準拠、Option 2 不要）。
- 2026-08-06: §10.2.1 の Admin API 内部 NLB 表現を正確化（「最厳格」→ `scheme=internal` でインターネット非露出、in-cluster との差は VPC 内到達経路 1 本分・SG 限定+TLS+認証で緩和。過大表現是正）。
- 2026-08-06: **[ADR-062](../adr/062-idm-api-execution-form-lambda.md)（Lambda）+ [ADR-063 ブランドユニット](../adr/063-brand-unit-architecture.md)を反映** — §10.2.1 を **ブランド主役トポロジ**に改訂（idm-api が CRUD+権限+authz+projection の実体 / 入口=idm-api・ルーティングはエッジ / 中央は shadow 制御 Lambda のみ / 越境=EventBridge 2 本 / 経路⑤=App Acct が idm-api を CC 呼び）。**旧 front door ①一本化・「中央→ブランド idm-api」PrivateLink 委譲は撤回**。D-U10-07 更新。
