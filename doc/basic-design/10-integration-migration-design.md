...
| デプロイ | 配置 | 利用者・経路 | 認可 |
|---|---|---|---|
| **idm-api #2（ブランド管理 API）= 主役** | ブランドユニット（IdP-KC Acct、[ADR-063](../adr/063-brand-unit-architecture.md)）・Lambda | 管理 SPA + アプリ（M2M）+ 移行バッチ。**入口は #2、ルーティングはエッジ**（CloudFront/API GW がブランド→#2、JWT L1 も）。**CRUD + 権限 + authz + projection + idmap の実体** | **ユーザ AT**（`tenant-admin` + `tenant_id`）/ アプリは **CC**（`idm:*`、U5 §5.8） |
| **shadow 制御 Lambda（中央・薄い）** | Broker Acct・Lambda | **Broker shadow の enable/disable のみ**（IdP-KC 削除イベント `user.deprovisioned` で発火、EventBridge）。重い中央 front door（旧 #1）は置かない | 内部（EventBridge ハンドラ + S2S 認可、U5 §5.8） |
...
- **ブランド主役トポロジ（2026-08-06 [ADR-063 ブランドユニット](../adr/063-brand-unit-architecture.md)。旧 front door ①一本化を改訂）**: **#2（ブランド管理 API）が CRUD + 権限 + authz + projection の実体**。**入口は #2**、ルーティングはエッジ（CloudFront/API GW がブランド → そのブランドの #2 へ）。**重い中央 front door（旧 #1）は置かない**。**中央（Broker）に残るのは shadow 制御 Lambda のみ**（IdP-KC 削除をトリガーに Broker shadow を無効化、[control-plane ノート フロー④](research/control-plane-crud-authz-flows-notes.md)）。**authz/idmap/projection はブランド内ローカル**（`sub` + `brand_id` キー、Broker は authz 非保持）。**経路⑤（同居アプリ）は App Acct に置き、#2 を Client Credentials で（エッジ経由）呼ぶ**（IdP-KC 非同居 = PW ハッシュのブラスト半径隔離）。
- 対象 Keycloak: **#2（ブランド）は IdP-KC Admin API を叩く（CRUD、内部 NLB）**。**Broker Admin API を叩くのは中央 shadow 制御 Lambda のみ**（shadow 無効化、IdP-KC 削除イベント発火）。クロスアカウント越境は EventBridge の 2 本（#2→Broker 削除 / Broker→ブランド 初回 sub 通知、U6 §6.3.2）。
- **実行形態 = Lambda（[ADR-062](../adr/062-idm-api-execution-form-lambda.md)、O-9 解決）**。決め手 = auth-critical な Keycloak〔P0〕と管理ツール〔P1〕を別障害ドメインに分離。**Ingress = `CloudFront(api.basis, WAF) → API GW（JWT L1 + throttle）→ Lambda 直 invoke`**（VPC Link/NLB を挟まない。**api. は API GW = NFW 例外**ゆえ NFW 経路外で準拠、[U6 REQ-IN-12](06-infra-network-design.md)）。**非同期の糊（shadow 制御 / outbox リレー / Webhook Dispatcher / idmap・projection ハンドラ）も Lambda（層③）**。Keycloak Admin API へは**内部 NLB（`scheme=internal` = インターネット非露出 + SG を Lambda SG 限定 + server-TLS + アプリ層認証）**で到達（[U6 D-U6-11](06-infra-network-design.md)。#2 = credential アカウントは特に厳格に）。
...
| D-U10-07 | ユーザ管理 API = 単一 OpenAPI `idm-api` v1。**実行形態 = Lambda（ADR-062）、ブランド主役（ADR-063: #2 が CRUD+authz+projection の実体、中央は shadow 制御 Lambda のみ、入口=#2/エッジルーティング、越境=EventBridge 2 本）** | §10.2.1 | U3 D3-05、U5 §5.8、U7 §7.5.2、**ADR-062/063**、U6 D-U6-11 |
...
- 2026-08-06: **[ADR-062](../adr/062-idm-api-execution-form-lambda.md)（Lambda）+ [ADR-063 ブランドユニット](../adr/063-brand-unit-architecture.md)を反映** — §10.2.1 を **ブランド主役トポロジ**に改訂（#2 が CRUD+権限+authz+projection の実体 / 入口=#2・ルーティングはエッジ / 中央は shadow 制御 Lambda のみ / 越境=EventBridge 2 本 / 経路⑤=App Acct が #2 を CC 呼び）。**旧 front door ①一本化・#1→#2 PrivateLink 委譲は撤回**。D-U10-07 更新。
