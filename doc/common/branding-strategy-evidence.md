...
## 1.A 2 軸 × Yes/No によるパターン自動判定（理解の前提）

ブランディングカスタマイズの 4 パターン（A / A' / B / C）は、実は **2 つの独立した軸** の Yes/No 組合せで自動的に決まる構造です。顧客との対話では「4 つから 1 つ選んでください」ではなく、**「2 つの軸でそれぞれ Yes/No を選んでください」** とした方が混乱が少なく、整理しやすくなります。

### 2 つの独立軸

| 軸 | 質問 | カスタマイズの実装単位 | ヒアリング ID |
|---|---|---|:---:|
| **軸 1: アプリ別カスタマイズ** | アプリごとに認証基盤側のログイン画面を変えるか?（経費精算 vs 決済管理 等） | 認証基盤側で **`client_id` ベース**の Branding Style / Login Theme | [A-11](../requirements/hearing-checklist.md) |
| **軸 2: 顧客別カスタマイズ** | 顧客企業ごとに認証基盤側のログイン画面を変えるか?（Acme vs Globex 等） | 認証基盤側で **`tenant_id` / Pool / Realm ベース**の Branding | [A-11-α](../requirements/hearing-checklist.md) |

### 2 軸 × Yes/No マトリクスからのパターン自動判定

```mermaid
flowchart TB
    Q1{軸 1<br/>アプリ別カスタマイズ?}
    Q2A{軸 2<br/>顧客別カスタマイズ?}
    Q2B{軸 2<br/>顧客別カスタマイズ?}

    Q1 -->|No| Q2A
    Q1 -->|Yes| Q2B

    Q2A -->|No| A["パターン A<br/>共通基盤 1 枚 +<br/>アプリ側 tenant_id 差替"]
    Q2A -->|Yes 部分| B["パターン B<br/>顧客別 Branding<br/>Cognito 20 顧客上限"]
    Q2A -->|Yes 完全分離| C1["パターン C<br/>Pool/Realm 分離<br/>SSO 喪失リスク"]
    Q2B -->|No| Ap["パターン A'<br/>アプリ別 Branding +<br/>アプリ側 tenant_id 差替"]
    Q2B -->|Yes 部分| Mix["A' × B 複合<br/>(Cognito ほぼ不可、<br/>Keycloak 必須)"]
    Q2B -->|Yes 完全分離| C2["パターン C<br/>同上"]

    style A fill:#e8f5e9
    style Ap fill:#fff3e0
    style B fill:#e3f2fd
    style Mix fill:#ffe0e0
...
|:---:|:---:|:---:|---|
| ❌ No | ❌ No | **A** | アプリ画面のみカスタマイズ（Slack / Notion 型、最シンプル）|
| ✅ Yes | ❌ No | **A'** | アプリ間差別化（Auth0 / Entra / Okta 型、業界主流）|
| ❌ No | ✅ Yes 部分 | **B** | 顧客別ブランディング（Cognito 20 上限、規制業種）|
| ✅ Yes | ✅ Yes 部分 | A' × B 複合 | アプリ × 顧客の組合せ（Cognito ほぼ不可、Keycloak 必須）|
| - | ✅ Yes 完全分離 | **C** | Pool / Realm 分離、SSO 喪失、Enterprise プラン |

### この整理のメリット

| 観点 | 4 パターン直接選択 | 2 軸 × Yes/No |
|---|---|---|
| **顧客の認知負荷** | 4 つから 1 つ選ぶ（A/A'/B/C の差を把握する必要あり）| 2 つの独立 Yes/No だけ |
| **判断軸の明確化** | 軸が混在（アプリ別 vs 顧客別が一緒に並ぶ）| 軸が分離（独立に判断可能）|
| **複合パターンの理解** | A' × B 複合の存在が見えにくい | マトリクスで自然に表現 |
| **ヒアリング工数** | 1 質問だが解説が長い | 2 質問だが各々シンプル |
| **新規要望への拡張性** | パターン追加で 5 個 / 6 個 / ... と増える | 軸を追加・細分化で対応可 |

### ヒアリング推奨順序

1. **A-11**（軸 1: アプリ別）を Yes/No で確認
2. **A-11-α**（軸 2: 顧客別）を No / Yes 部分 / Yes 完全分離 で確認
3. 上記マトリクスで判定されたパターンを顧客に提示・合意取得
4. **A-11-2**（アプリ側実装責務）: 軸 1 = No または 軸 2 = No 時に確認
5. **A-11-3**（カスタマイズレベル L1-L8）: 軸 1 = Yes または 軸 2 = Yes 時に確認
...
「本基盤のログイン画面のカスタマイズについて、2 つの観点で確認させてください。

1 つ目: アプリごとに違うログイン画面を出したいですか?
        (経費精算と決済管理で違うロゴ・配色等)

2 つ目: 顧客企業ごとに違うログイン画面を出したいですか?
        (Acme 社と Globex 社で違うロゴ・配色等)

それぞれの組合せで、業界標準のパターンが自動的に決まります。」
```

→ この切り出し方で、顧客は **「4 つから 1 つ選ぶ」のではなく「2 つの独立判断」** に集中できる。
...
## 6.A フェデユーザー / ローカルユーザーの画面遷移と責務分担

A-11 / A-11-α のカスタマイズを設計する際、**フェデユーザー（P-3）とローカルユーザー（P-2 / P-4 等）で経由する画面が異なる**ことを理解しておく必要があります。本セクションは画面遷移の責務分担と、フェデユーザーのログイン操作 UX を整理します。

### フェデユーザー（P-3）が経由する 3 画面

| 画面 | 物理的所在 | 管理責務 | A-11 / A-11-α | カスタマイズ手段 |
|---|---|---|:---:|---|
| **❶ 本基盤の IdP セレクター画面** | 本基盤（`auth.example.com`）| 本基盤チーム | ✅ 対象 | Cognito Managed Login Branding / Keycloak Theme |
| **❷ 顧客 IdP のログイン画面** | **顧客 IdP**（`login.microsoftonline.com` 等）| **顧客 IT 部門** | ❌ **対象外** | 顧客 IdP の設定（Entra ID なら "Company Branding"）|
| **❸ 本基盤の補完画面**（同意 / プロファイル補完 / アカウントリンク確認）| 本基盤 | 本基盤チーム | ✅ 対象 | 本基盤の Theme |

### ローカルユーザー（P-2 / P-4 等）が経由する画面

| 画面 | 物理的所在 | 管理責務 | A-11 / A-11-α |
|---|---|---|:---:|
| **❹ 本基盤の ID/PW 入力フォーム**（外部 IdP ボタンと統合 UI）| 本基盤 | 本基盤チーム | ✅ 対象 |

### Managed Login / Theme の重要な特性：1 つの統合画面に両方が乗る

Cognito Managed Login や Keycloak Theme の **ログイン画面は「フェデ専用」「ローカル専用」と分かれていない**。同じ画面上にローカル ID/PW フィールドと外部 IdP ボタンが並ぶ統合 UI。

→ A-11 / A-11-α のカスタマイズは「統合 UI 全体」に適用される。フェデ専用 / ローカル専用に分けて別 Branding を当てたい場合は、**App Client 単位で「ローカル無効 / フェデ無効」を設定**することで実現:

| App Client | Identity Providers 設定 | 表示される UI | 採用シーン |
|---|---|---|---|
| 経費精算アプリ（P-3 フェデ強制）| 外部 IdP のみ許可 | 外部 IdP ボタンのみ | γ シナリオ |
| 管理画面アプリ（P-1 / P-2 / P-5 用）| ローカル Pool のみ許可 | ID/PW フォームのみ | 管理者専用 |
| 汎用アプリ（P-3 + P-4 両方）| 両方許可 | 統合 UI | β シナリオ |

### フェデユーザーのログイン操作 UX（HRD / セレクター / 組織固有 URL）

「**❶ の画面で IdP をどう選ばせるか**」には 3 つの UX パターン:

| パターン | フロー | 業界実例 | 顧客追加コスト |
|---|---|---|---|
| **A. Home Realm Discovery（HRD）**| メール入力 → ドメインから IdP 自動判定 → 顧客 IdP へ | Microsoft 365 / Slack | ドメイン → IdP マッピング設定要 |
| **B. IdP セレクター**| 各 IdP のボタン選択 → 顧客 IdP へ | GitHub / GitLab | IdP ボタン追加のみ |
| **C. 組織固有 URL**| `acme.app.example.com` から組織確定 → 顧客 IdP へ | Slack Workspace / Notion | DNS / 証明書設定要 |
...
| 顧客追加リードタイム | △ マッピング設定要 | ◎ ボタン追加のみ | ⚠ DNS / 証明書設定要 |
| 顧客間の混同リスク | ❌ 他社 IdP ボタンも表示される可能性 | ❌ 他社 IdP ボタン表示 | ✅ URL で完全分離 |
| ブランディング（顧客別画面）| △ 本基盤共通 | △ 本基盤共通 | ✅ 組織別カスタムページ可（パターン B / C 連動）|
| マルチテナント所属時の UX | ✅ メール自動判定 | ⚠ 手動選択 | ⚠ 別 URL 訪問必要 |
| 業界主流度 | **★★★** | ★★ | ★★ |

→ **業界推奨は A. HRD**（Microsoft 365 / Slack 採用）。本基盤の[§FR-2.3.3 ベースライン](../requirements/proposal/fr/02-federation.md#fr-233-ログイン画面で-idp-選択-ux--home-realm-discoveryfr-fed-013) でも HRD を推奨。

### 採用シナリオ（A-5-3）と UX パターンの組合せ推奨

| シナリオ | フェデユーザー UX 推奨 | ローカル UI 表示 | App Client 設定 |
|---|---|---|---|
| **α 全カテゴリ受け入れ** | A. HRD + 統合 UI | あり（ローカル ID/PW フォーム） | 両方許可 |
| **β 管理者 + IdP なし顧客** | A. HRD + 統合 UI | あり（IdP なし顧客向け） | 両方許可 |
| **γ 管理者層のみ（推奨）** | **A. HRD（フェデ専用）** | なし（管理者用は別 App Client） | フェデ用 App Client = 外部 IdP のみ / 管理用 = ローカルのみ |
| **δ Break Glass のみ** | A. HRD or B. セレクター | なし（緊急用は別 App Client）| 同上 |

### 顧客要望の責務分担マトリクス（重要）

顧客から「フェデユーザーのログイン画面をカスタマイズしたい」と要望があった場合の責務分離:
...
| 「顧客 IdP に飛ばす前に本基盤側で MFA 要求」 | ✅ | [§FR-3.3 ステップアップ MFA](../requirements/proposal/fr/03-mfa.md) + ACR 制御で実装可 |

### 契約・SOW 段階での明示推奨

「フェデユーザー画面カスタマイズ」要望は、**❶❸（本基盤側）と ❷（顧客 IdP 側）の責務を契約段階で分離**することを強く推奨:

- **本基盤の SOW 範囲**: ❶ IdP セレクター画面、❸ 補完画面、❹ ローカル ID/PW フォーム
- **顧客責任の範囲**: ❷ 顧客 IdP のログイン画面（Entra / Okta / HENNGE 等のサービス内設定）
- **「本基盤を入れたら全画面カスタマイズできる」という誤解を初期段階で解消**

→ この明示がないと、後から「Entra の画面も変えてほしい」要望が出て揉める原因になります。

---

## 6.B フェデ UX の誤解と対策（「2 回ログイン」問題の整理）

A-11 / A-11-α / A-5-3 / B-601 のヒアリングで頻出する「**フェデなのに 2 回ログインさせるのか?**」という顧客誤解への対策。

### 「2 回ログイン」と見える 3 種類の現象

| 見え方 | 本当のところ | 評価 |
|---|---|:---:|
| ❶ メール入力 + ❷ IdP ログイン（HRD パターン）| ❶ は IdP 振り分け識別子入力で認証ではない、認証は ❷ 1 回のみ | ✅ 業界標準 |
| 本基盤の IdP セレクター + IdP ログイン | ❶ はボタンクリックで認証ではない | ✅ 業界標準 |
| **顧客 IdP で MFA + 本基盤で MFA**（MFA 重複） | **本当に 2 回認証**、MFA 重複回避ができていない | ❌ アンチパターン |
...
→ **顧客が「2 回ログイン」と言う場合、まず「狭義」か「広義」かを確認**することが対話の出発点。

### SSO の本質：2 回目以降の効果で測る
...
[2 回目以降（同じブラウザ）]
ユーザー → アプリ B → ❶❷ 完全スキップ → 即時利用可  ← SSO の効果
```

→ **SSO は「2 回目以降の体験」で評価**。初回の段階数で判断しない。

### 「本当に 2 回認証させる」アンチパターン 4 種

| パターン | 原因 | 対策 | 関連章 |
|---|---|---|---|
| パターン 1: MFA 重複（最多）| 顧客 IdP MFA 済なのに本基盤側でも MFA 要求 | `amr` 信頼設計 | [§FR-2.2.3](../requirements/proposal/fr/02-federation.md#fr-223-mfa-重複回避--fr-fed-012) |
| パターン 2: 信頼レベル L4 不信任 | 本基盤側で `prompt=login` 強制 | L1〜L3 採用、L4 は規制業種のみ | [§FR-4.2](../requirements/proposal/fr/04-sso.md) |
| パターン 3: 古い auth_time の max_age 制約 | 高セキュ操作時に再認証要求 | 重要操作のみ、max_age 値を慎重に | [§FR-4.2](../requirements/proposal/fr/04-sso.md) |
| パターン 4: SCIM / JIT 競合の補完画面 | 初回ログイン時のアカウントリンク確認 | 認証ではないが操作が増える | [§FR-2.2.1.A](../requirements/proposal/fr/02-federation.md#fr-221a-同一テナント内ユーザー重複の扱い) |

### 「2 回ログイン」整理表

| 状況 | ❶ 操作 | ❷ 操作 | 認証回数 | 問題? |
|---|---|---|:---:|:---:|
| HRD 標準 | メール入力 | ID/PW + MFA | 1 回 | ❌ なし |
| セレクター標準 | IdP ボタンクリック | ID/PW + MFA | 1 回 | ❌ なし |
| 組織固有 URL | (URL アクセス) | ID/PW + MFA | 1 回 | ❌ なし |
| MFA 重複 | ID/PW + MFA | ID/PW + MFA | 2 回 | ✅ 問題 |
| L4 不信任 | ID/PW + MFA | ID/PW + MFA | 2 回 | ⚠ 意図的 |
| max_age 強制 | 過去ログイン | ID/PW + MFA | 2 回（時間差）| ⚠ 意図的 |
| **同一基盤 SSO（2 回目）** | (スキップ) | (スキップ) | **0 回** | ✅ SSO 効果 |

### 業界実例（フェデ 2 段階は標準）

| サービス | 初回ログイン操作 | 認証回数 |
|---|---|:---:|
| Microsoft 365 | メール入力 → Entra ID で ID/PW + MFA | 1 回 |
| Slack | Workspace 名 → SSO ボタン → IdP | 1 回 |
| Notion | メール入力 → SSO 自動振り分け → IdP | 1 回 |
| Salesforce | My Domain → SSO ボタン → IdP | 1 回 |
| GitHub Enterprise SSO | IdP ボタン → IdP | 1 回 |

→ **業界全体で「初回は 2 段階操作、認証は 1 回」が標準**。

### 顧客対話用の説明テンプレート（営業 / SE 向け）

詳細テンプレートは [§FR-2.3.3.A 顧客対話用の説明テンプレート](../requirements/proposal/fr/02-federation.md#fr-233a-画面所在マトリクスとカスタマイズ-3-パターン) 参照。

**短縮版（営業 1 ライナー）**:
```
「2 段階に見えますが、認証は 1 回だけです。
 メール入力は『どの会社の IdP に振り分けるか』を決める識別子で、
 パスワードは御社の Entra ID で 1 回だけ入力します。
 Microsoft 365 / Slack / Salesforce と同じ業界標準の挙動です。」
```
...
| チェック項目 | 確認 |
|---|---|
| ☐ B-506 で「外部 IdP MFA を全面信頼」を採用したか | MFA 重複回避（パターン 1）|
| ☐ B-801-1 で L1 完全信頼を採用したか（規制業種は L3 まで）| L4 不信任（パターン 2）回避 |
| ☐ B-802-2 で max_age 制約は高セキュ操作のみに限定したか | パターン 3 回避 |
| ☐ B-406 / B-407 で重複検出時の挙動を「自動リンク」もしくは「初回のみ確認」に設定したか | パターン 4 対応 |
| ☐ 営業・SE 向けに「2 段階 ≠ 2 回認証」の説明資料を共有したか | 顧客対話品質 |

→ これらが揃って初めて、業界標準の「**フェデ初回は 2 段階操作、認証は 1 回、2 回目以降は完全 SSO**」体験が実現される。
