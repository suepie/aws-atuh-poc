...
| [session-lifecycle-and-flows.md](session-lifecycle-and-flows.md) | **セッション/トークンの全ライフサイクル SSOT**：全 11 層の一覧（AT/RT/Broker SSO/認証セッション/IdP-KC/顧客 IdP 等）+ 作成・削除フロー + 削除マトリクス + **退職者の実効遮断ラグ（AT 30 分は誤解、実効 = SCIM 数分 / JIT アイドル 1h・絶対 24h）** + DR 時の扱い。要件定義 §FR-5 の説明資料 + 基本設計 U5/U2/U8 の参照元 |
