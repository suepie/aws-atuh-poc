...
| ⑫ | **SAML SP セッション（ServiceNow 等）** | **SP 側（ServiceNow）** | SP が SAML アサーションを受領した時 | **SP 側設定**（SN のアイドル / 絶対タイムアウト） | SP 側タイムアウトのみ。**SAML SLO 不採用（Phase 1）のため Broker のログアウト・退職では破棄されない** | SAML アプリ（SN）に「ログイン中」。§3.0 の SP 注記参照 |

**ご質問の 3 つ（AT / RT・Broker SSO / 顧客 IdP）は ②④⑤⑪ に対応するが、全体では 12 層ある**。特に見落としやすいのが ⑦認証セッション（ログイン途中の状態）、⑩ IdP-KC セッション（非 IdP テナント専用）、⑥オフライン（Phase 1 無効）、**⑫ SAML SP セッション（ログアウト・退職でも SP 側タイムアウトまで残る、§3.0）**。
...
## 1.6 上記 12 層とは別扱いのトークン・状態（混同注意）

セッション/トークン文脈で語られるが、①〜⑫ とは性質が違うため別掲する。混同すると失効設計を誤る:

| 項目 | 何か | 寿命 | ①〜⑫ との違い | 出典 |
|---|---|---|---|---|
| **Token Exchange 後 AT** | マイクロサービス間で aud 付替え・scope 縮小した交換トークン（②の変種） | **15 分・RT 発行なし** | 元⑤失効で連動失効。短命で吸収 | U5 §5.3.3 |
| **M2M 資格情報**（SCIM Facade Bearer / `idm:*` Client Credentials AT） | 機械間の資格情報（ユーザー不在） | SCIM Bearer=90 日ローテ / CC AT=15 分 | **ユーザーセッションではない**。ユーザー失効・ログアウトと無関係 | U3 D3-11 / U5 §5.8 |
| **Step-up の max_age 鮮度窓** | ⑤ は生きていても `auth_time > max_age`（AAL2=900s / AAL3=300s）だと高価値操作は再認証 | 操作ごとの鮮度判定 | セッションの「生存」ではなく「鮮度ゲート」。⑤ が生きていても操作を止める | U5 §5.1.2 / U4 §4.3.4 |
| **Break Glass セッション** | SSO をバイパスするローカル管理者ログイン（SN / 基盤） | 通常ログイン | ①/⑤ の特例。別統制・別監査。SSO 障害時の非常口 | U10 §10.1.5 |
| **not-before 監視窓**（ITDR L4） | not-before push 後、ローカル検証 RP に伝播しない AT の残余 | ≤30 分 | ②のゾンビ窓の一種。§5 Z-4 と同根。Runbook で 30 分監視 | U5 §5.4.3 |
| **Phase 2 未実装**（DPoP バウンドトークン / Trust Device 30 日 MFA スキップ / HRD remember cookie / Mobile SDK セッション / ⑥ Offline Token） | Phase 1 には存在しない | — | 追加時に①〜⑫ 本表へ昇格 | U5 §5.9 / U4 §4.1.1・§4.3.1 / ADR-050 |
...
- **⑫ SAML SP（ServiceNow）セッションは上表の列に含めない**（Broker から破棄できないため別掲、§3.0）。

### 3.0 SAML SP（ServiceNow）セッションの扱い（SLO 不採用の帰結）

⑫ SP セッションは **Broker から破棄できない**（SAML SLO 不採用、Phase 1）。ログアウト・退職いずれの契機でも **SP 側タイムアウトまで残る**:

| 契機 | ⑫ SP(SN) セッション | ユーザーの見え方 |
|---|---|---|
| **明示ログアウト**（Broker L2） | ❌ **SN 側アイドルまで残る**（SLO なし） | 他 OIDC アプリは L4 でログアウト済みだが、**SN は開いたままになり得る** |
| **退職・削除**（SCIM / JIT / mode A） | ❌ **SN タイムアウトまで残る** | 認証チェーンは塞がる（再アクセス時 SAML 拒否）が、**既存 SN セッションは残余** |
| 実効遮断の上限 | SN セッションタイムアウト設定値 | 即時性要件が強いテナントは **SN 側タイムアウト短縮**を案内 |

- **緩和策**: SN 側アイドルタイムアウト短縮、または SP-initiated SLO の個別採用（テナント明示要求時、ゲート **B-APP-OIDC-3**）。
- **② AT のゾンビ窓（≤30 分）とは別物**: SP セッションは SN 側 Cookie で、SN タイムアウト（分〜時間単位）まで残る。退職者遮断の「最後の残余」はここになり得る点を顧客説明に含める。
- 出典: [U10 §10.1.6](../basic-design/10-integration-migration-design.md)（削除連鎖 / SN 残余窓）、[saml-vs-oidc §14](saml-vs-oidc-comparison.md)（SLO vs Back-Channel）。
...
- 2026-07-27 (v1.4): **⑫ SAML SP（ServiceNow）セッションを §1 一覧に追加**（11→12 層）。SLO 不採用の帰結として「ログアウト・退職いずれでも SP 側タイムアウトまで残る」を **§3.0 新設**（削除マトリクスとは別掲、U10 §10.1.6 参照）。**§1.6 新設**：①〜⑫ と別扱いのトークン・状態（Token Exchange 後 AT / M2M 資格情報 SCIM Bearer・idm CC / Step-up max_age 鮮度窓 / Break Glass / not-before 監視窓 / Phase 2 未実装）をセッション文脈で一元化。
