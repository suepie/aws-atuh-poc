...
| `deprovisioned_at` | 90 日定期バッチで無効化された日時 | バッチ管理（[jit-scim §10.4](jit-scim-coexistence-keycloak.md)）|
...
**ポイント**:
- broker は「現在ログイン中のアプリ群」を可視化できる → [self-service-responsibility.md §4.2](self-service-responsibility.md) で Account Console の **Devices タブ**として利用
...
| **退職フラグ・組織変更** | 顧客 IdP の HR 連携で管理 | broker は SCIM or 定期バッチで反映する（[jit-scim §10.4](jit-scim-coexistence-keycloak.md)）|
...
| **PCI DSS Req 8.2.6「90 日未使用無効化」** | `user_session` の last_session_refresh と `event_entity` で判定 → `enabled=false`（[jit-scim §10.4](jit-scim-coexistence-keycloak.md) のバッチスクリプト）|
