...
> ℹ **呼称・モード改定（2026-08-20）**: 実行モードを **2 つに統合** — **M1 → 自動差分検査（モード1）**、**M3 → 全量検査（モード2、日次定期＋手動。旧「手動全量検査（モード3）」）**、**M2（heartbeat 型）→ 廃止**（シグナルなし変化は全量検査の日次定期実行が受け持つ、18 §18.1.1）。本 ADR の本文・追記中の M1/M2/M3 は経緯記録としてそのまま維持し、上記に読み替える。

> ℹ **呼称改定（2026-09-11）**: **監視資材 → 認証構成情報** / **資材バケット（App アカウント側）→ 認証構成情報連携バケット** / **Monitoring Registry（共通基盤側）→ 認証構成情報配置バケット**（用語表は 10 §10.0.4）。本 ADR の本文・追記中の旧称は経緯記録としてそのまま維持し、上記に読み替える（「資材オンリー原則」＝「認証構成情報オンリー原則」）。

> ⚠ **追記（2026-09-16）: 連携バケットと IAM ロールを「案件側作成」に変更し、対象アカウントを SSM 登録リスト（申請ベース）に変更**（ユーザー決定）。
> - **作成主体の変更**: 認証構成情報連携バケット・`DiscoveryReadRole`・`ArtifactUploadRole-{appId}` は、中央が StackSets で全アカウントへ自動配布するのをやめ、**中央が提供するテンプレートと手順にもとづいて案件側（アプリ／ベンダー）が自アカウントに作成**する。
> - **対象アカウントの決定方法**: **SSM Parameter Store の登録リスト**（`/auth-monitoring/target-accounts`、中央管理）から取得する。**Organizations の `ListAccounts` は使わない**（2026-08 に推奨としていた委任ポリシー方式は不採用）。理由: バケットとロールを案件側が作る方式では組織全体を列挙しても「どのアカウントが監視対象か」を判別できず、判別のために全アカウントへ AssumeRole を試すと対象外アカウントで必ず失敗して `DiscoveryAccountErrors`（MM-3）が常時発報し、本物の障害が埋もれるため。副次効果として**管理アカウントへの委任ポリシー依頼（外部調整）が不要**になった。
> - **「登録漏れが構造的にゼロ」の範囲が変わる（重要）**: **アプリ単位**は従来どおり自動発見で登録漏れゼロを維持するが、**アカウント単位は申請ベースとなり登録漏れが起きうる**。登録されていないアカウントのアプリは一切監視されないため、月次棚卸し（17 §17.7 M-Q-17-3）がアカウント単位の唯一の検出手段になる。本 ADR 冒頭の Decision にある「登録漏れが構造的に起きない」は**この範囲に読み替える**。
> - 基本設計 16/17 章・WBS v8・ベンダー配布資料は改訂反映済み。

> ⚠ **追記（2026-08-21）: 資材取得元を CodeCommit → App アカウントの S3 監視資材バケットに変更 + deploymentId 併読を廃止（資材オンリー原則）**。
> - 前提変化: コードリポジトリは**開発ベンダーごとに外部（GitHub 等）**にあり、中央から読めないことが判明。「git を読む」前提（2026-08-07 改訂）が不成立に。
> - **案 A 採用**（検討: [research/external-git-artifact-store-study.md](../api-platform/basic-design/research/external-git-artifact-store-study.md)）: 各 App アカウントに**資材バケットを StackSets 配布**し、デプロイパイプライン最終段で monitoring.yaml / openapi.yaml（+ 任意 deploy-info.json）を Put。中央の pull 巡回・read-only 原則は維持し、検知シグナルは**資材オブジェクトの VersionId 比較**に変更。案 B（中央集約バケットへの push）は「App → 中央の書き込み開放」の復活になるため不採用。
> - **deploymentId 併読（2026-08-19 追記の ③）は廃止**（ユーザー決定 2026-08-21）: 「中央が App アカウントで読むのは資材だけ」という**資材オンリー原則**で統一し、クロスアカウント権限を s3 read 1 種に縮小（apigateway:GET 削除、ネットワークの線も削減）。コンソール直変更の挙動検知は日次の全量検査（最大 24h）+ Config Rules に緩和して受容。**アップロード忘れ・資材誤りは原則アプリ責任**（顧客合意事項、17 §17.7 M-Q-17-7）とし、中央は staleness 検知 + 棚卸しで補助。
> - 副次効果: アップロード＝デプロイ後のため穴②「コミット ≠ デプロイ」が構造的に縮小 / モノレポのパス突合（GetDifferences）が不要化 / CodeCommit アクティブユーザー課金が消滅。
> - 基本設計 10/12/13/16/17/18 章・WBS・費用見積は改訂反映済み。
