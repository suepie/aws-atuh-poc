...
>
> 凍結は維持。**移行を再開する場合に必要な前提が 2 つ増えた**ので記録する:
>
> 1. **一括投入時の DB 接続枠は確保済み**（[接続予算表](../basic-design/06-infra-network-design.md)に「移行バッチ 10 本」として計上）。**表に無い主体は接続してはならない**規約になったため、**移行ツールを増やす場合は接続予算表への行追加が必須**。
> 2. **投入先の DB が 3 系統に分かれた**（認証製品用 × 2 + 権限用 × 1）。利用者本体は認証製品用、権限・対応づけは権限用に入るため、**移行ツールは 2 系統への書き込みを 1 つの処理として整合させる**必要がある（片方だけ入った状態を作らない）。
- **関連**:
...
### 方式② の前提条件と 方式① フォールバック（2026-08-03 追記、是正 1 / FC-2）

方式②（User Storage SPI キャッシュ移行）は「**初回ログイン時にレガシー DB を参照できる**」ことが暗黙の前提であり、次の 2 条件が成立しない顧客では採れない:

| 前提条件 | 内容 | 不成立時の影響 |
|---|---|---|
| **到達性** | レガシー DB / ディレクトリが**並走期間中オンラインで、IdP-KC（別 AWS Acct）からネットワーク到達可能**（オンプレなら PrivateLink/VPN、アプリ内蔵 DB なら経路開通）| SPI が legacy lookup できず認証不能 |
| **読取資格** | SPI がユーザー + PW hash を取得できる**読取専用資格情報 / API** が提供される | 同上 |

- **フォールバック = 方式①（ハッシュ直接インポート）**: レガシー DB が並走期にオフライン化・到達不能・エクスポートのみ提供の場合は、**エクスポート済みハッシュを `credentialData`(hashIterations/algorithm) + `secretData`(salt/value) 形式で投入**する（PBKDF2/Argon2 はネイティブ、bcrypt 等は方式③ Custom PasswordHashProvider SPI 併用）。一括投入時は §NFR-9.1 のバッチ規約（Partial Import 1000/コール上限）に従う。
- **工場系の含意**: レガシーがオンプレの生産管理システム DB のことが多く、**到達経路の有無が方式②/①の分岐を決める**。B-MIG-1（現行ハッシュ）に加え「**レガシー DB の並走期到達可否**」をヒアリングで確定する。
