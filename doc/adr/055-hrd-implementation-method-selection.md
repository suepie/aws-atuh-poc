...
- **2026-08-24 追記**: **§F.1 新設** — 方式 C（Phase 2 顧客別 URL）採用時は**ワイルドカード証明書 1 枚に限定**する（顧客ごとの個別証明書は CT ログで顧客一覧が全件公開され、[U7 D-U7-19](../basic-design/07-security-compliance-design.md) のテナント列挙耐性を方式 A より悪化させるため）。Phase 2 着手時の必須ゲート。
...
| **大口顧客 / カスタムブランディング要望**（顧客別 URL `acme.basis.example.com`）| **方式 C: URL + CloudFront Function** | 顧客ブランディング統一、ユーザ入力不要。**⚠ 証明書の発行方式を必ずセットで決めること（下記 F.1）** |

### F.1 方式 C 採用時の必須検討: 証明書発行方式とテナント列挙（2026-08-24 追加）

**方式 C は、実装を誤ると顧客企業名の秘匿性を方式 A より悪化させる。**

顧客ごとにサブドメイン（`acme.basis.example.com`）を切り、**顧客ごとに個別の証明書を発行すると、証明書の透明性ログ（Certificate Transparency）に全ドメインが公開される**。CT ログは [crt.sh](https://crt.sh/) 等で誰でも横断検索でき、**`*.basis.example.com` で検索するだけで顧客企業の一覧が取得できる**。

これは [U7 §7.8.1a D-U7-19](../basic-design/07-security-compliance-design.md) の脅威（テナント列挙）に対し、**「1 件ずつ試す」から「検索するだけで全件」へ、攻撃コストを桁違いに下げる**。方式 A（共通 URL）にはこの問題が無いため、**方式 C の導入が列挙耐性を後退させる**構図になる。

| 証明書方式 | CT ログからの列挙 | DNS 総当たり | 評価 |
|---|---|---|---|
| **顧客ごとに個別発行** | 🔴 **全件が公開・検索可能** | — | **採用禁止** |
| **ワイルドカード証明書 `*.basis.example.com` 単一** | ✅ 漏れない（個別名が証明書に載らない） | ⚠ 残る（推測可能な別名なら当たる） | **方式 C を採るならこれ一択** |
| 個別発行 + CT 非掲載 | — | — | ❌ **不可能**（主要ブラウザが CT 掲載を必須要件としている） |

- **決め（Phase 2 着手時の前提）**: 方式 C を採用する場合は **ワイルドカード証明書 1 枚**とし、**顧客別サブドメインの個別証明書は発行しない**。あわせて**サブドメイン名を顧客名から推測できない値にするか**を [U7 O-U7-11](../basic-design/07-security-compliance-design.md) と合同で判断する（ただし方式 A のメールドメイン経路が残る限り、推測困難化の効果は限定的）。
- **DNS 側の注意**: ワイルドカード証明書でも、**公開 DNS にサブドメインを個別登録すればゾーン転送・パッシブ DNS から漏れうる**。ワイルドカード DNS レコード 1 本で受ける構成を前提とする。
- **`kc_idp_hint` 無効化との干渉（2026-08-27 追記）**: 方式 C は CloudFront Function が **`kc_idp_hint` を付与する**仕組みだが、テナント列挙対策 [U7 D-U7-19 ②](../basic-design/07-security-compliance-design.md) は**公開経路の素の `kc_idp_hint` を SPI で無効化する案 A を第一候補**としている。両立させるには**方式 C を署名付きヒント（同 表の案 C）とセットで実装する**こと。素の `kc_idp_hint` を方式 C のために残すと存在オラクル（1 件ずつのテナント存在確認）が復活する。
- **本項の位置づけ**: 方式 C は Phase 2 候補のまま変更しないが、**この検討（証明書 + `kc_idp_hint` 干渉）を経ずに採用してはならない**（Phase 2 着手時の必須ゲート）。
