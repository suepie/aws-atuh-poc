...
### 例外 Realm を切る場合の代償 — アプリ側への波及（2026-08-27 追記）

B 案の例外適用を判断する際、**基盤側の運用コスト（上記 5 観点）に加えてアプリ側への波及**を必ず評価すること。Keycloak の URL は `/realms/<realm>/...` を含むため、**Realm 分離 = ログイン入口・issuer・JWKS の URL がすべて変わる**。

| 波及 | 内容 |
|---|---|
| **issuer** | [ADR-063 不変条件⑤](063-brand-unit-architecture.md)「各アプリは 1 issuer のみ照合」の前提が崩れる。例外 Realm のテナントを共有アプリが受けるなら**複数 issuer・複数 JWKS の検証**が必要 |
| **Client 登録** | Client 定義は Realm 単位。例外 Realm にも**全アプリの Client（aud scope・Mapper 一式）を複製登録**し二重保守（B-1 伝播コストの実体） |
| **authorize URL** | アプリのリダイレクト先が Realm 依存になり、**アプリがログイン前にテナント判別する必要**が生じる = **HRD が基盤で吸収していた仕事がアプリへ漏れる**（最も筋の悪い波及） |
| その他 | Back-Channel Logout / 鍵ローテ / 外形監視が Realm ごとに増える |

**帰結**: 例外 Realm を切る判断は実質「その顧客専用ブランドを 1 つ立てる」判断と同じ重さになる。**「PW 規則や画面を顧客別にしたい」という希望だけでは上表の例外条件に該当せず、B 案を適用しない**（希望ベースで配ると際限がなく 5 観点に戻る）。提供する場合は個別費用のオプションとし、規制・契約要件の確認を先行させる（→ [U7 §7.8.1b](../basic-design/07-security-compliance-design.md)）。
