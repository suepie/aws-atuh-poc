...
> 🔴 **2026-08-27: 本節全体を Phase 1 対象外とする（C-8）**。P-12 の決定により **LDAP 経路を利用する顧客が Phase 1 に存在しない**ため、本節の設計は実装対象外。関連ゲート **G-LDAP / B-SCIM-13 は廃止**（[U1 §1.5](../basic-design/01-architecture-baseline.md)）。
>
> **削除せず残す理由**: LDAP は「引きに行く（Pull）」方式で他と根本的に異なり、**通常のログイン処理を通らないため、そこに置いた拡張が発火しない**という固有の落とし穴がある。将来 LDAP 顧客が入る際、この調査をやり直す損失が大きい。**再開時は本節を起点とし、G-LDAP を最優先ゲートとして復活させること**。
...
> **✅ 2026-07-23 基本設計 Wave 1 確定 — SCIM 受信 = 自作 SCIM Facade（[U3 D3-11](../basic-design/03-identity-provisioning-design.md)）**
>
> SCIM 受信は **自作 SCIM Facade**（外部コンポーネント、SCIM 2.0（RFC 7643/7644）を受けて Keycloak Admin API に写像）を **Phase 1 確定**（代替 A の延長。属性書込は Admin API + User Profile `ADMIN_EDIT` で実測 PASS 済み）。**Metatavu keycloak-scim-server は工数超過時 / Phase 2 の代替候補に降格**。
>
> エンドポイントはテナント別 URL `/t/{tenant_id}/scim/v2`、認証はテナント別発行の**不透明 Bearer トークン**（Secrets Manager 管理）。
...
**実装**: 顧客 IdP の SAML/OIDC Mapper で Import 属性を `username` / `tenant_id` に絞る。IdP 既定 syncMode = **IMPORT**（[U2 §2.5.4](../basic-design/02-keycloak-logical-design.md) 確定、2026-07-23。旧記述の `sync-mode=FORCE` 前提を改訂）。ライフサイクル属性（`scim_active` / `provisioned_by` / `last_login`）は Mapper 非対象化し、`mfa_indicator` のみ Mapper 単位で FORCE とする
...
| 対策 | ゾンビ期間 | Phase |
|---|---|---|
| **① Access Token TTL = 30 分（Baseline P-09 凍結値）+ RT Rotation 併用** | 最大 30 分（実質遮断は次回 Refresh 時 ≤30 分）| Phase 1 必須 |
...
> **2026-07-23 改訂（基本設計 Wave 1）**: 旧記述「Access Token TTL 5 分 Phase 1 必須」は **Access Token TTL = 30 分（Baseline P-09 凍結値）** に改訂。RT Rotation 併用で実質遮断は次回 Refresh 時 ≤30 分。ゾンビウィンドウ最大 30 分への変更に伴い、PCI DSS §8.2.5 の説明は**「Soft Delete 即時 + RT Rotation + 管理者強制ログアウト」の複合**で担保する。規制業種向けにはテナント別 TTL 短縮オプション（5-15 分）を用意。最終確定は基本設計 U5。

### I.6 顧客 IdP 別の SCIM 対応状況（Auth0 例外）

| 顧客 IdP | Outbound SCIM Push | 本設計の対応 |
|---|:-:|---|
| Microsoft Entra ID / Okta / Google / Ping | ✅ | SCIM Facade に Push（2026-07-23 更新、旧: Metatavu SCIM Endpoint）|
...
- **§I ~~Metatavu SCIM Server 導入~~ 自作 SCIM Facade + Custom Event Listener SPI 実装工数（2026-07-23 改訂: Facade 自作は Metatavu 導入より工数増の可能性、U3 D3-11 で見積り）**（2026-07-08 / 2026-07-23）
- **§I Auth0 顧客は Event Streams + Custom Actions で workaround 必要（Auth0 個別対応が発生）**（2026-07-08）
- **§I Access Token TTL 30 分によるゾンビ期間の発生（2026-07-23 改訂: 旧 5 分 → P-09 凍結値 30 分。RT Rotation + Soft Delete 即時 + 管理者強制ログアウトの複合で緩和、規制業種はテナント別 5-15 分短縮。Phase 3 で API GW Introspection でリアルタイム化）**（2026-07-08 / 2026-07-23）
