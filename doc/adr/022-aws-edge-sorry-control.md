...
- **日付**: 2026-06-12、**2026-07-23 更新（v3、基本設計 U4: 主実装を基盤側 Sorry SPA へ、エッジ集約は要求仕様に降格）**
- **⚠ 2026-07-23 改訂（v3、P-18 + U4 D-U4-07）**: CloudFront + Lambda@Edge は P-18 により**他組織管理 Acct 内**となり弊社実装保証不能。**主実装 = 基盤側 Sorry SPA（launchpad 同居 `/sorry`）+ RP 側 403 redirect 規約（[U5 §5.6.6](../basic-design/05-token-session-authz-design.md)）**とし、エッジ 403→302 集約（パターン ii）は **REQ-IN-07b の要求仕様・追加防御層に降格**（技術選定自体は維持）。v2 注記の「Auth Acct S3」は **Broker Acct 配信**に読み替え、launchpad 用 CloudFront/WAF は **REQ-IN-11** で要求（[04-auth-ux-design.md](../basic-design/04-auth-ux-design.md) §4.5）。
...
ADR-021 で Sorry の業界標準実装パターンは「**C. 共通 エラー / 案内画面 SPA**」と確定した。これを AWS インフラ層でどう実装するかが本 ADR の論点。
...
- エラー / 案内画面 SPA は **1 つだけ S3 に置けば全アプリで共有**できる
...
flowchart LR
    ...
    CF -->|"forward"| SorrySPA["エラー / 案内画面 SPA<br/>(S3 origin)"]
    SorrySPA --> User
...
| **ブランディング細かな反映** | ⚠ 静的 HTML | ✅ エラー / 案内画面 SPA で自由 | ✅ | ⚠ ALB は HTML 表示弱い | ⚠ 同左 |
| **エラー / 案内画面 SPA との統合**（ADR-021 連動）| ⚠ 別 SPA で運用は弱い | ✅ クエリ受渡で完全統合 | ✅ | ⚠ 受渡が手間 | ⚠ 同左 |
...
| 顧客状況 | 推奨パターン | 理由 |
|---|---|---|
| **業務アプリ 3 個以上、CloudFront 前段あり**（本基盤の標準想定）| **ii Lambda@Edge** | 動的コンテキスト + エラー / 案内画面 SPA 統合 + アプリ無変更（403 ヘッダ返却のみ）|
...
- エラー / 案内画面 SPA は 1 つだけ S3 に置けば全アプリで共有
...
| **運用負荷・コスト最小** | パターン ii は Lambda@Edge 1 本 + エラー / 案内画面 SPA 1 つ + S3 で完結 |
