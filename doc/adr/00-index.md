...
| [062](062-idm-api-execution-form-lambda.md) | **ユーザー管理 API（idm-api）の実行形態 = Lambda 採用（O-9 決定）**（テナント管理 API #1 / ユーザー連携 API #2 を ROSA 常駐でなく Lambda で実装。決め手 = auth-critical な Keycloak クラスタ〔P0〕と管理ツール idm-api〔P1〕を別障害ドメインに分離。Ingress = API GW ネイティブ invoke、Keycloak Admin API は内部 NLB〔scheme=internal + SG 限定 + server-TLS + アプリ層認証〕で到達、#1→#2 PrivateLink 単方向、非同期の糊も Lambda。NLB 露出は外部に効かず SG/TLS で防御・cold start 許容と判断。代替 A/B/A'〔ROSA 常駐〕は同居問題で不採用） | Proposed | 2026-08-06 |
