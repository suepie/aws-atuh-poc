...
| [061](061-deploy-detection-pull-model.md) | **デプロイ検知の pull 型統一（中央巡回による発見・差分検知）**（各アプリの CodeCommit を中央から巡回し、先端コミット ID を台帳の `lastCheckedCommitId` と比較して変更を検知 → `GetDifferences` で対象を特定 → 認証実装を検査。メタデータは資源タグでなくリポジトリ内 `monitoring.yaml`（config-as-code）、OpenAPI もリポジトリ内 spec を正本とする。読み取りロールは codecommit read のみに縮小。**モノリス（ALB 直）も自動発見の対象になる**〔旧設計の「モノリスは手動登録」の穴が解消〕。**受容した穴**: ①コンソール直変更は git に現れず見逃す〔L2 Config Rules と M3 手動フルで補完〕②コミット直後は未デプロイで偽安心があり得る。Lambda 3 本 = 発見 / 認証実装チェック / Alert Router。**スコープは API プラットフォーム側**のため認証基盤の設計書 10 冊からは参照されない） | Accepted | 2026-08-06（08-07 改訂）|
| [062](062-idm-api-execution-form-lambda.md) | **idm-api（管理コントロールプレーン）の実行形態 = Lambda 採用（O-9）**（ブランド管理 API #2〔主役〕+ 中央 shadow 制御 + 非同期の糊を ROSA 常駐でなく Lambda で。決め手 = auth-critical な Keycloak〔P0〕と管理ツール〔P1〕を別障害ドメインに分離。api. は API GW〔NFW 例外、REQ-IN-12〕、Keycloak Admin API は内部 NLB〔scheme=internal・インターネット非露出・SG 限定・TLS・アプリ層認証〕。**トポロジ(#2 主役)は ADR-063、削除伝播は ADR-064**。代替 A/B/A'〔ROSA 常駐〕は同居問題で不採用） | Proposed | 2026-08-06（08-08 brand 反映） |
| [063](063-brand-unit-architecture.md) | **ブランドユニット・アーキテクチャ（共有 Broker + ブランド別 backend）**（Broker は共有 1 つ〔横断認証/SSO・ログイン画面ブランド別テーマ・sub 発番・ルーティング・shadow〕。ブランドを将来の隔離/複製単位とし、authz/idmap/projection/CRUD/アプリをブランドユニット〔IdP-KC 側〕に配置、Broker は authz 非保持。federated も sub+brand_id でブランド authz に保持、/api/me/context はブランドローカル read で越境ゼロ。Phase 1 = 1 ブランド、物理 per-brand 分割は将来。不変条件: sub グローバル安定 / brand_id 一級キー / cross-brand join なし / Broker 共有関心のみ。U1 §1.2 E 判断〔旧 authz=Broker〕を改訂） | Proposed | 2026-08-06 |
| [064](064-deprovisioning-propagation-outbox.md) | **削除／デプロビ伝播 = outbox + 中央 shadow 制御（A 案）**（#2 が soft-delete + `user.deprovisioned` outbox 行を authz DB 1Tx → outbox リレーが EventBridge 必達 → 中央 shadow 制御 Lambda が冪等に Broker shadow 無効化。遮断チェックのみ数分リコンサイル。窓 = 通常数秒/worst = リコンサイル間隔・発行済み AT ≤30 分。federated は SCIM deprovision or 90 日バッチ。旧 Broker-first 同期 2 コール却下 / S 案は即時ゼロ窓要件時に保留 / SCIM 内部伝播は却下〔KC outbound 未実装〕） | Proposed | 2026-08-08 |
| [065](065-continuous-access-caep-shared-signals.md) | **継続アクセス評価 = CAEP / Shared Signals（ゾンビ窓の標準解）**（3 フェーズ段階導入。AT 失効後 ≤30 分のゾンビ窓〔U5 §5.2.4〕の恒久解として OpenID SSF/CAEP を目標アーキに。Phase 1 は暫定ブリッジ〔短命 AT + not-before push + 高価値 Introspection + DPoP〕、CAEP は G-SSF PoC 後に段階導入。2-tier ゆえ双方向〔顧客 IdP 受信 / RP 送信〕。Keycloak SSF は experimental transmitter-only ＝成熟待ち、outbox からの自作 SET エミッタを早期 transmitter 候補に。Phase 1=暫定ブリッジ+outbox自作SETエミッタ、Phase 2=KC native SSF、Phase 3=receiver。最終確定=G-SSF+B-CAEP-1） | Proposed | 2026-08-12 |
| [066](066-non-human-identity-governance.md) | **非人間ID（NHI）ガバナンス**（軽量 NHI 台帳 + 命名規約 + 孤立検知 + 失効伝播。M2M クライアント資格情報・IRSA/Pod Identity・SCIM/JIT コネクタ = 人間IDの数十倍の NHI に台帳・所有者・目的・最小権限・ローテ・孤立検知の統制層を新設。ADR-037 軽量 IGA の傘下で軽量台帳 + 規約〔案 B〕。失効は ADR-064 と同機構で伝播検討。台帳=authz系Aurora同居、失効=ADR-064機構。最終確定=B-NHI-1） | Proposed | 2026-08-12 |
| [067](067-authz-decision-logging-and-access-recertification.md) | **認可判定ログ + アクセス再認証（IGA 強化）**（① Backend DB エンタイトルメント判定の「なぜ許可/拒否」決定ログ〔SOC2 CC6.1 / ISO A.8/A.9〕② 3 層管理スコープ・テナント管理者・NHI の定期 recertification〔ISO A.9.2.5、実施記録〕を軽量 IGA に組込。7 年不変監査〔認証中心〕を認可の説明可能性 + 棚卸し記録で補完。サンプリング=deny+高権限全件、recert=3層×リスクベース。最終確定=B-AUTHZLOG-1/B-IGA-REC-1） | Proposed | 2026-08-12 |
