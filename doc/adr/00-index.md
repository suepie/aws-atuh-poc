...
| [065](065-continuous-access-caep-shared-signals.md) | **継続アクセス評価 = CAEP / Shared Signals（ゾンビ窓の標準解）**〔スタブ〕（AT 失効後 ≤30 分のゾンビ窓〔U5 §5.2.4〕の恒久解として OpenID SSF/CAEP を目標アーキに。Phase 1 は暫定ブリッジ〔短命 AT + not-before push + 高価値 Introspection + DPoP〕、CAEP は G-SSF PoC 後に段階導入。2-tier ゆえ双方向〔顧客 IdP 受信 / RP 送信〕。Keycloak SSF は experimental transmitter-only ＝成熟待ち、outbox からの自作 SET エミッタを早期 transmitter 候補に。方式確定 = G-SSF + B-CAEP-1） | Proposed（スタブ） | 2026-08-12 |
| [066](066-non-human-identity-governance.md) | **非人間ID（NHI）ガバナンス**〔スタブ〕（M2M クライアント資格情報・IRSA/Pod Identity・SCIM/JIT コネクタ = 人間IDの数十倍の NHI に台帳・所有者・目的・最小権限・ローテ・孤立検知の統制層を新設。ADR-037 軽量 IGA の傘下で軽量台帳 + 規約〔案 B〕。失効は ADR-064 と同機構で伝播検討。方式確定 = B-NHI-1） | Proposed（スタブ） | 2026-08-12 |
| [067](067-authz-decision-logging-and-access-recertification.md) | **認可判定ログ + アクセス再認証（IGA 強化）**〔スタブ〕（① Backend DB エンタイトルメント判定の「なぜ許可/拒否」決定ログ〔SOC2 CC6.1 / ISO A.8/A.9〕② 3 層管理スコープ・テナント管理者・NHI の定期 recertification〔ISO A.9.2.5、実施記録〕を軽量 IGA に組込。7 年不変監査〔認証中心〕を認可の説明可能性 + 棚卸し記録で補完。方式確定 = B-AUTHZLOG-1 / B-IGA-REC-1） | Proposed（スタブ） | 2026-08-12 |
