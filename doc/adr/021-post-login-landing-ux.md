...
2. ログイン**後**に entitled apps の一覧画面（サービス選択画面）を出すか（Microsoft 365 portal 型）
...
| Post-login サービス選択画面 | **採用**（業界標準、複数アプリの効率な起動）|
...
| # | 観点 | Pre-login | Post-login サービス選択画面 |
|---|---|---|---|
...
| 13 | **アプリ追加時の更新場所** | Pre ページ（手動）| サービス選択画面（ロール定義 + 自動表示）|
| 14 | **アプリ削除時の更新場所** | Pre ページの手動更新 | サービス選択画面 は自動 |
...
| 19 | **Cookie / セッション削除後の挙動** | 何も困らない | 再ログイン後 サービス選択画面 へ |
...
flowchart TB
    ...
    Q3 -->|Yes 2 個以上| Post[Post-login サービス選択画面 必須]
...
| 完全社内、アプリ 1 つだけ | **④ いずれもなし**（Deep-link）| サービス選択画面 もなしで OK |
...
| 「サービス選択画面 = ホームページ」| ❌ サービス選択画面 は entitled apps の起動台、ホームページ（マーケサイト）と役割違い |
| 「Pre-login で entitled apps を見せる」| ❌ 未認証では権限不明、原理的に不可 |
| 「Pre と サービス選択画面 は同じ画面でいい」| ⚠ 認証前後で見せる情報が違うため、画面を分けるのが業界標準 |

---

## E. Post-login サービス選択画面 の設計
...
→ いずれも**独立した SPA** で構築。Keycloak の アカウント設定画面 とは別物。

### サービス選択画面 の責務と Keycloak の限界

| 機能 | Keycloak アカウント設定画面 "Applications" タブ | 独立 サービス選択画面 SPA |
|---|:---:|:---:|
...
flowchart LR
    ...
    Roles --> Match[サービス選択画面 SPA で<br/>roles → app 表示マッピング]
...
Common Auth Platform (Keycloak)
   ↓ OIDC
サービス選択画面 SPA (auth.example.com/launchpad)
   ↓ JWT 検証 + roles 解析
...
|---|---|
| Post-login サービス選択画面 あり | Sorry → サービス選択画面 にリダイレクト |
| Post なし、Pre のみ | Sorry → Pre ページに戻す（UX 弱い）|
| 両方なし（Deep-link 主体）| Sorry 専用ページが必須 |
| 両方あり | Sorry → Post サービス選択画面 へ |

### 3 つの実装パターン

| パターン | 責務分担 | UX | 採用例 |
|---|---|---|---|
| A. アプリ側 Sorry + サービス選択画面 リンク | アプリ X が 403 → 自前 Sorry ページ → サービス選択画面 リンク | アプリのブランディング維持 | Atlassian Cloud |
| B. 認証基盤の Sorry テーマ | Keycloak の `access_denied` ページを Theme カスタマイズ | 認証基盤側で統一 | 小規模システム |
| **C. 共通 エラー / 案内画面 SPA**（**推奨**）| アプリ X が 403 → `auth.example.com/sorry?app=x` にリダイレクト → 共通 SPA がガイダンス表示 + entitled apps リスト | 業界標準 | LinkedIn / Microsoft 365 |
...
| **entitled apps の一覧 or サービス選択画面 リンク** | 行き先を提供 |
...
| サービス選択画面 経由ログインでアプリに飛ばしたい | サービス選択画面 が App tile クリック時にアプリの URL に redirect（アプリ側 OIDC フローが起動、SSO セッションで即返る）|
| 認証後に サービス選択画面 に必ず着地させたい | `redirect_uri` を サービス選択画面 SPA に固定（業界標準は **アプリ側 redirect_uri 優先**、サービス選択画面 はデフォルト着地点のみ）|
...
flowchart TB
    subgraph User_Entry["エントリ点"]
        ...
        LPLink["サービス選択画面 URL<br/>(launchpad.example.com)"]
    end
...
- サービス選択画面 は別 SPA で構築 → ロールベース可視性 + 自由ブランディング
- Sorry は edge 集約（ADR-022）で実装、アプリは 403 を返すだけ
- 業界標準（Atlassian / Microsoft / Okta）と整合

### Negative

- サービス選択画面 SPA の別途構築工数（OIDC RP として 1 つの client 追加）
- アプリ追加時に サービス選択画面 の表示ロール / マッピング更新が必要
...
| アカウント設定画面（簡易 サービス選択画面）| ❌ なし | ✅ あり（補助用途）|
| Custom Sorry テーマ | ⚠ Hosted UI で限定的 | ✅ Theme で完全カスタマイズ |
| クライアント別 default redirect | ✅ App Client 単位 | ✅ Client 単位 |
| サービス選択画面 SPA との統合 | ✅ OIDC RP として実装可 | ✅ 同左 |

→ どちらでも サービス選択画面 SPA を別構築する設計は同等に可能。Keycloak は アカウント設定画面 を補助的に使える分やや優位。

---

## 参考資料

- [Keycloak アカウント設定画面 — Applications tab](https://access.redhat.com/documentation/en-us/red_hat_build_of_keycloak/22.0/html/server_administration_guide/account-service)
- [Keycloak issue #23885: Display only accessible clients in アカウント設定画面](https://github.com/keycloak/keycloak/issues/23885)
