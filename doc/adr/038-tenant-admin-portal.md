...
| | 招待リンク発行 | 新規ユーザーをセルフサインアップへ（**email 配信前提**）|
| | **資格情報配布モード**（2026-08-03 追加、是正 4 / FC-4）| **email 非保有顧客向け**。招待リンク/リセットメールの代替として、**初期 PW・ワンタイムコード・QR を画面表示 → 管理者が印刷・対面配布**する。メール送信を伴わない「発行して手渡す」フロー。email 非保有判定は B-IDM-1/10、周知は [§FR-2.3.2.B email 非保有版](../requirements/proposal/fr/02-federation.md) と対 |
| | パスワードリセット（強制発行）| 管理者主導の PW リセット（email 配信 or 上記 配布モード）|
