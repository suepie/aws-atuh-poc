...
### 推奨デフォルト：**パターン ②（L1 SCIM + L2 SAML JIT）**（2026-07-15 明示化）

**核心**：**プロビジョニングには L1 と L2 の 2 種類の JIT/SCIM が独立して存在**する。それぞれを別々に選択可能、両者は独立して動作する。

| 項目 | 採用方針 |
|---|---|
| **SSO プロトコル**（L2）| **SAML 2.0**（Multi-Provider SSO Plugin、業界標準）。OIDC は Tokyo+ で対応するが事例少、新規案件のみオプション |
| **L1 Provisioning**（顧客 IdP → 本基盤）| **SCIM 受信**（本基盤側で SCIM Endpoint 実装、[jit-scim §10.4](../common/jit-scim-coexistence-keycloak.md) デフォルト）、SCIM 未対応顧客は JIT |
| **L2 Provisioning**（本基盤 → ServiceNow）| **SAML JIT**（ServiceNow が初回 SSO 時に自動作成）。SCIM Push は**第二オプション**（[§L L2 SCIM Push 採用条件](#l-2026-07-15-新設-l1l2-独立性--4-パターン組合せ--l2-scim-代替根拠) 参照）|
| **ユーザーマスタ** | **ServiceNow に残す**（業務システム側がユーザーマスタを持つのが業界標準。本基盤は認証マスタ、顧客 IdP は SoT）|
| **既存 ServiceNow ユーザー** | `user_name` を Layer B `external_id` として本基盤にマッピング、ADR-018 と整合 |
| **属性連携**（L2）| SAML Assertion 経由（roles / department / manager 等）|
| **退職フロー** | **L1 側で `enabled=false` → SAML 認証拒否 → SN ログイン不可**（L2 の sys_user は残置、業務履歴保持）|

**推奨組み合わせマトリクス**：

| # | L1（顧客 IdP → 本基盤）| L2（本基盤 → SN）| 適用シーン | 推奨度 |
|---|---|---|---|:---:|
| ① | JIT | SAML JIT | 中小 SaaS 顧客、SCIM 未対応 IdP | ★★★ |
| **② ★** | **SCIM** | **SAML JIT** | **主要 IdP 対応顧客 = 大半の顧客** | **★★★★★** |
| ③ | JIT | SCIM Push | 珍しい | ★ |
| ④ | SCIM | SCIM Push | 超大企業 + 承認フロー事前設定必須 | ★★（3 条件揃う時のみ）|

→ 大半の顧客は **パターン ②**。L2 SCIM Push は 3 条件揃う場合のみ検討（[§L.5](#l5-l2-scim-push-パターン--採用条件3-条件全て揃う場合のみ) 参照）。
...
## L. 【2026-07-15 新設】L1/L2 独立性 + 4 パターン組合せ + L2 SCIM 代替根拠

> **背景・なぜここで新設するか**：§B の 4 パターン比較（A/B/C/D）は **L2 側の視点のみ**。しかし [jit-scim §10.4](../common/jit-scim-coexistence-keycloak.md) の議論で確定した通り、プロビジョニングには **L1（顧客 IdP → 本基盤）と L2（本基盤 → SN）の 2 種類の JIT/SCIM が独立して存在** する。両者を独立に選択可能で、実質推奨は **L1 SCIM + L2 JIT の非対称組合せ**（パターン ②）。実装者 / 契約担当者の理解不足で「本基盤 → SN も SCIM が必要では?」と混乱するのを防ぐため明示化。

### L.1 L1/L2 の 2 種類の JIT/SCIM

**プロビジョニングは 3 段構造の各層で独立して発生**:

```mermaid
flowchart LR
    subgraph L3 [Layer 3: SoT = 顧客 IdP]
        IdP_DB[Entra/Okta/AD の<br/>ユーザ DB]
    end
    subgraph L2mid [Layer 2: 本基盤 Keycloak = ブローカー ★]
        Broker_DB[user_entity]
    end
    subgraph L1sp [Layer 1: 業務 SP = ServiceNow]
        SN_DB[sys_user]
    end

    IdP_DB -->|① L1 JIT<br/>本人が本基盤にログイン時| Broker_DB
    Broker_DB -->|② L2 JIT<br/>本人が SN にログイン時| SN_DB
    IdP_DB -.->|① L1 SCIM<br/>IdP イベント起点| Broker_DB
    Broker_DB -.->|② L2 SCIM Push<br/>本基盤イベント起点<br/>パターン C の場合| SN_DB

    classDef sot fill:#e6ffe6,stroke:#4a4
    classDef broker fill:#e3f2fd,stroke:#248
    classDef sp fill:#fff8dc,stroke:#a80
    class L3 sot
    class L2mid broker
    class L1sp sp
```
...
| Layer | 向き | 作られる場所 | 決定 ADR |
|---|---|---|---|
| **L1** | 顧客 IdP → 本基盤 | `user_entity` テーブル | [ADR-025 SCIM 受信スタンス](025-scim-positioning-and-receive-stance.md) + [jit-scim §10.4](../common/jit-scim-coexistence-keycloak.md) |
| **L2** | 本基盤 → ServiceNow | `sys_user` テーブル | 本 ADR（ADR-023） |

**核心**：**L1 と L2 は独立**。L1 側の JIT/SCIM 選択と L2 側の JIT/SCIM 選択は別々に決められる。

### L.2 4 パターン組合せの完全比較

L1 × L2 = 4 パターン:

| # | L1（顧客 IdP → 本基盤）| L2（本基盤 → SN）| 特徴 | 適用シーン | 推奨度 |
|---|---|---|---|---|:---:|
| **①** | JIT（本人ログイン起点）| SAML JIT（本人 SN アクセス起点）| 最軽量、両側事前登録なし | 中小 SaaS 顧客、SCIM 未対応 IdP | ★★★ |
| **② ★** | **SCIM**（IdP Push）| **SAML JIT** | 本基盤事前登録 + SN 側 JIT | **主要 IdP 対応顧客 = 大半の顧客** | **★★★★★** |
| ③ | JIT | SCIM Push（本基盤→SN）| 珍しい、採用理由乏しい | ほぼ採用理由なし | ★ |
| ④ | SCIM | SCIM Push | 全層事前登録、大規模統合志向 | 超大企業 + 3 条件揃う時のみ | ★★ |

### L.3 パターン ② が推奨される理由

| 要素 | パターン ② の優位性 |
|---|---|
| **L1 SCIM の実装コスト** | 主要 IdP（Entra/Okta/Google）がネイティブ SCIM 送信対応 → **顧客側実装コスト = ゼロ** |
| **本基盤の SCIM 受信** | 1 度実装で全顧客対応可能 → **スケール可能** |
| **L2 SAML JIT の実装コスト** | ServiceNow 側は `User Provisioning Enabled=Yes` の **1 項目設定のみ** |
| **本基盤の Response** | SAML Response の Attribute Statement で user_name/email/roles 送信、既存実装で対応 |
| **削除の対応** | L1 SCIM で本基盤 `enabled=false` → **SAML 認証拒否で SN も塞がる**（L2 SCIM 不要、§L.4 参照）|
| **合計工数** | 最小、業界標準に沿う |

**具体的なフロー**（初回 SN アクセスの本人）:
...
sequenceDiagram
    ...
    Note over IdP: [Day 0] 新入社員登録
    IdP->>Broker: L1 SCIM POST /Users<br/>alice@example.com
    Broker->>Broker: user_entity 作成<br/>provisioned_by=scim<br/>scim_active=true
    Note over Broker: 本基盤に事前登録済み<br/>SN 側にはまだない

    Note over User,SN: [Day 3] 本人が初めて SN アクセス
    User->>SN: ServiceNow にアクセス
    SN->>Broker: SAML AuthnRequest
    Broker->>IdP: OIDC AuthnRequest (フェデ経由)
    IdP-->>Broker: ID Token
    Note over Broker: federated_identity 既存 hit<br/>(L1 SCIM で事前登録済)
    Broker-->>SN: SAML Response (user_name, email, roles)

    rect rgb(255, 250, 220)
        Note over SN: ★ L2 SAML JIT 発火
        SN->>SN: sys_user 検索 → なし
        SN->>SN: INSERT sys_user<br/>user_name=alice<br/>属性は SAML Attribute から
    end

    SN-->>User: ServiceNow ログイン完了
```

### L.4 L1 SCIM で L2 SCIM を代替できる根拠（削除フロー）

**核心**：**削除は L1 側だけで完結、L2 SCIM Push なしでも認証チェーンで塞がる**。
...
sequenceDiagram
    ...
    participant Broker as 本基盤
    participant SN as ServiceNow
    participant User as 元従業員

    IdP->>Broker: [退職時] SCIM DELETE /Users/{id}
    Broker->>Broker: enabled=false + Session Revoke ✅

    Note over SN: SN 側は何もしない<br/>sys_user は active=true のまま残置

    User->>SN: [Day X] 再ログイン試行
    SN->>Broker: SAML AuthnRequest
    Note over Broker: enabled=false で認証拒否
    Broker-->>SN: SAML Response = 認証失敗
    SN-->>User: ❌ ログイン不可（SN 側では sys_user active でも）

    Note over SN: 業務履歴 / チケット / 承認履歴は sys_user に紐付き保持 ✅
```
...
| # | 内容 | 意味 |
|---|---|---|
| **1** | **L1 SCIM で本基盤 `enabled=false` にすれば SAML 認証拒否 → SN も塞がる** | セキュリティ的には L1 SCIM のみで十分 |
| **2** | **SN 側の sys_user 残置は "業務履歴保持" の観点でメリット** | チケット / 承認 / インシデントが FK 切れずに残る |
| **3** | **本基盤 → SN の SCIM Push を追加実装しても、この認証チェーンでの防御に "追加のセキュリティ価値" は無い** | SCIM Push 追加コストが正当化されない |

### L.5 L2 SCIM Push（パターン ④）採用条件（3 条件全て揃う場合のみ）

以下 3 条件が **全て揃う** 場合のみ L2 SCIM Push を検討する価値あり:

| 条件 | 詳細 | 頻度 |
|---|---|:---:|
| **1. 超大企業**（10,000 人以上）| 人事プロセスと SN の統合が業務クリティカル | 稀 |
| **2. 承認フロー事前設定必須** | 本人が SN 初アクセスする前に Role / Group / Manager 情報が必要 | 稀 |
| **3. 顧客 IdP が Microsoft Entra ID 以外** | KB2599716 の影響を受けない | 業界の 60% は Entra なので該当稀 |

**該当例**：
- 金融大手（10 万人規模）+ Okta + SN の統合
- 新入社員が入社日から Approval Chain で承認可能状態必須

**該当しない例（大半）**：中小 SaaS 顧客 / Entra ID 使用顧客 / 承認フロー都度手動対応で問題ない顧客

### L.6 L1/L2 削除の非対称性（Deprovisioning 経路の設計）
...
flowchart TB
    ...
    Trigger --> L1[L1 削除: 本基盤側]
    L1 --> L1_A[SCIM DELETE 受信 or 90 日バッチ<br/>詳細: jit-scim §10.4]
    L1_A --> L1_B[user_entity.enabled=false<br/>+ not_before + Session Revoke]

    Trigger --> L2[L2 削除: ServiceNow 側]
    L2 --> L2_A{パターン選択}
    L2_A -->|基本 推奨| L2_B[SN 側は何もしない<br/>sys_user はそのまま残置<br/>=業務履歴保持<br/>次回 SN アクセス時に SAML 認証で拒否<br/>実質的に使用不可]
    L2_A -->|完全同期| L2_C[本基盤から SCIM DELETE を SN に Push<br/>sys_user.active=false<br/>パターン ④ 採用時のみ]
    L2_A -->|SN 手動| L2_D[SN 管理者が手動で削除<br/>パターン ① の A 亜種]

    classDef primary fill:#e6f2ff,stroke:#248
    classDef default fill:#fff8dc,stroke:#a80
    class L1_B,L2_B primary
    class L2_A default
```

### L.7 L2 は L1 混在を意識しない（透過性）

**核心**：本基盤がブローカーとして L1 の SCIM/JIT 混在を吸収 → **L2 側の実装は透過的**。

```mermaid
flowchart LR
    subgraph L3_side [L3: 顧客 IdP 側]
        IdP_SCIM[SCIM 送信]
        IdP_JIT[JIT ログイン起点]
    end

    subgraph L1_side [L1: 本基盤側 混在対応]
        Broker[本基盤 Keycloak<br/>混在管理<br/>provisioned_by / scim_active]
    end

    subgraph L2_side [L2: ServiceNow 側]
        SN[ServiceNow SP<br/>本基盤からの<br/>SAML Response のみ受信]
    end

    IdP_SCIM --> Broker
    IdP_JIT --> Broker
    Broker -->|SAML Response 統一形式<br/>user_name, email, roles| SN

    Note1[L1 SCIM/JIT の違いは<br/>本基盤内で吸収]
    Broker -.-> Note1

    Note2[SN は L1 の違いを意識しない<br/>SAML Response 受信するだけ]
    SN -.-> Note2

    classDef primary fill:#e3f2fd,stroke:#248
    class Broker primary
```

**含意**：
- L1 SCIM でも L1 JIT でも、**L2 に送る SAML Response は統一形式**（user_name, email, roles 等）
- **L2 側の実装は L1 混在を意識せず**、SAML Response のみ処理
- SN 側の SAML JIT は **L1 が JIT か SCIM かに関わらず動作**
- **L1 混在（[jit-scim §10.4.I.6 Case 1-5](../common/jit-scim-coexistence-keycloak.md)）は L2 に伝播しない**

**L2 側から見えるユーザ状態の唯一の変化**:

| L1 側イベント | L2 側で観測 |
|---|---|
| L1 SCIM POST（新規）| なし（次回 SN アクセスまで反映されない）|
| L1 JIT ログイン（新規）| なし |
| L1 SCIM PATCH（属性更新）| なし |
| L1 SCIM DELETE | 次回 SN アクセス時に SAML 認証拒否 → ログイン不可 |
| L1 90 日バッチ（Soft Delete）| 同上 |
| L1 Re-Activation SPI 発火 | 次回 SN アクセス時に SAML 認証成功 → ログイン再開 |

→ **L2 は L1 のイベントを直接受け取らない、次回 SN アクセス時の SAML AuthnRequest 経由で結果的に観測**。
...
- SAML Response の Attribute Mapper（user_name, email, roles）
- SP メタデータ交換（SN の SAML SP メタデータ登録）

**ServiceNow 側**（顧客側で対応）:
- Multi-Provider SSO Plugin 有効化
- 本基盤の SAML IdP メタデータインポート
- `System Properties > User Provisioning Enabled = Yes`
