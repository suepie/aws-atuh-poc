...
> ℹ **呼称・モード改定（2026-08-20）**: 実行モードを **2 つに統合** — **M1 → 自動差分検査（モード1）**、**M3 → 全量検査（モード2、日次定期＋手動。旧「手動全量検査（モード3）」）**、**M2（heartbeat 型）→ 廃止**（シグナルなし変化は全量検査の日次定期実行が受け持つ、18 §18.1.1）。本 ADR 中の M1/M2/M3 は経緯記録としてそのまま維持し、上記に読み替える。
