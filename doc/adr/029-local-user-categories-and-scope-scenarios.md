...
> 🚫 **Superseded（2026-08-18）— 本 ADR の第一推奨は現行前提と矛盾する**
>
> 本 ADR は「**シナリオ γ（管理者層のみローカル、顧客従業員は全員顧客 IdP 経由）**」を第一推奨としていたが、
> **[P-07](../basic-design/01-architecture-baseline.md)（2026-07-30 確定）で明示的に転換された**:
> **テナント管理者を含む全顧客ユーザーをフェデレーション**とし、顧客 IdP 非保有テナントは **IdP-KC 収容**（Broker から見て IdP の一つ）。
> **Broker のローカルは基盤運用者 / Break-Glass のみ**。P-07 の備考に「**従前 γ「管理者層のみローカル」から転換**」と明記されている。
>
> **本 ADR は利用者カテゴリ P-1〜P-4 の定義と、各シナリオの比較検討の記録として保持する**（削除しない）。
> **範囲の結論は P-07 が正**。ADR-033（2-tier）+ ADR-063（ブランドユニット）が現行の配置を定める。
---
