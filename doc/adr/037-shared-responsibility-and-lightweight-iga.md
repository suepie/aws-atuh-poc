...
| 責務 | 弊社（基盤）| 顧客（テナント管理者）| 備考 |
|---|:---:|:---:|---|
| **IdP-KC インフラ運用**（稼働 / パッチ / HA / DR）| ✅ | — | NFR-1〜5 |
| **IdP-KC ユーザーデータの暗号化保管** | ✅ | — | NFR-4.1（Aurora + KMS CMK）|
| **インフラ層セキュリティ**（VPC / WAF / KMS 等）| ✅ | — | NFR-4 |
| **Compromised Credentials 検知** | ✅ | — | ADR-035 ITDR |
| **インシデント通知** | ✅ | — | NFR-6 |
| **コンプライアンス基盤**（SOC 2 / ISO 27001 / PCI DSS）| ✅ | — | NFR-7 + ADR-036 |
| **ユーザーマスタの作成・更新・削除** | — | ✅ | 顧客テナント管理者が CRUD |
| **パスワードポリシー適用**（弊社推奨 / 顧客固有）| ⚠ 推奨提示 | ✅ 最終判断 | 共同責任 |
| **MFA 強制**（弊社デフォルト / 顧客固有）| ⚠ 推奨提示 | ✅ 最終判断 | 共同責任 |
| **ロール / 権限の付与** | — | ✅ | 顧客の業務判断 |
| **退職時 Deprovision** | ⚠ ツール提供 | ✅ 実施 | 顧客責務、弊社は SCIM/Webhook を提供 |
| **アクセスレビュー**（Access Certification）| ⚠ ツール提供 | ✅ 実施 | 顧客責務、弊社は軽量 IGA を提供 |
| **監査ログ提供** | ✅ | — | 顧客向け Trust Center |
| **監査ログの分析・対応** | — | ✅ | 顧客の責務 |
| **データ削除請求（GDPR Art.17 等）対応** | ⚠ ツール提供 | ✅ 申請受付 | 顧客が窓口 |
...
| **ADR-029 利用者カテゴリ**| P-1〜P-6 のカテゴリ別に、IdP-KC 該当者は P-2 + P-4 が中心 |
