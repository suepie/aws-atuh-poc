...
### 【公開境界昇格の承認権限者】 (API-D-101, 🔥)

公開境界の昇格（Private → Internal、Internal → Public、Partner 追加 等）の承認権限者をご教示ください。
- SecOps
- アーキテクチャ委員会
- プロジェクトオーナー
- 上記の段階承認
...
緊急昇格（インシデント対応等で公開境界を一時的に変更）のエスケープハッチを許容しますか。許容する場合の事後監査手順をご教示ください。
...
[§NFR-API-4 §4.5 死守事項マトリクス](../proposal/nfr/04-security.md) の公開境界別の粒度妥当性をご評価ください。
