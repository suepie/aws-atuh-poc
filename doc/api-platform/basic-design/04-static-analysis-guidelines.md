...
### §4.4.6 認証構成情報アップロード（デプロイ成功後の標準ステップ）

全静的解析 pass → デプロイ成功の**後**に、パイプライン最終段で**認証構成情報**（`monitoring.yaml` / `openapi.yaml`（デプロイ版の写し）+ 任意 `deploy-info.json`）を App アカウントの認証構成情報連携バケットへアップロードする（規約は [17 章 §17.3](17-deployment-integration-and-registration.md)、権限は [16 章 §16.2.2](16-cross-account-iam-design.md)）。中央の外形監視はこの認証構成情報の VersionId 変化で変更を検知するため、**アップロードしないと新しい版が監視対象に反映されない**（アップロード漏れ・誤りは原則アプリ責任、17 §17.2.2）。
...
- **順序を固定**: 「デプロイ成功 → 認証構成情報アップロード」。逆順・デプロイ失敗時のアップロードは禁止（「認証構成情報あり = その版がデプロイ済み」を成立させるため）
