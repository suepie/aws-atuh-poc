...
| ~~M-Q-18-4~~ | ~~モノリスの変更を全量検査で補う~~ → **解消**: 認証構成情報の巡回でモノリスも自動発見・自動検知（17 章 §17.4）。全量検査（モード2）の役割は「認証構成情報に現れない変更（コンソール直変更等）の網羅確認」に純化（2026-08-20 から日次定期で自動化）|
...
- [13-openapi-registry-design.md](13-openapi-registry-design.md) — OpenAPI の spec コピー置き場（正本はベンダー git、中央が読むのは連携バケットのデプロイ版の写し。差分判定は**認証構成情報の VersionId**）
