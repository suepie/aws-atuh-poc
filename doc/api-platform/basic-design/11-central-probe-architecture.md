...
> 本節は認証実装チェックの **1 回の実行内容（処理フロー）** を示す。**実行基盤（Lambda）と頻度（自動差分検査（モード1、旧称 M1・自動）・全量検査（モード2、旧称 M3「手動全量検査」・日次定期+手動））は [18 章](18-scan-modes-and-scheduling.md) が SSOT**。

1 回の実行で（自動差分検査（モード1）は対象アプリ、全量検査（モード2）は全アプリを）以下のように横断 probe する。
...
| WAF が probe をブロックすると | 4×4 での素朴な解釈 | 実際 | 結果 |
|---|---|---|---|
| Negative probe が WAF 403 | 「401/403 = 認証は正常」 | 認証まで到達していない | **偽陰性**（認証漏れがあっても OK 判定）|
| Positive probe が WAF 403 | 「Neg 2xx + Pos 403 = 認証逆転 CRITICAL」等 | 認証は正常、WAF が弾いただけ | **誤 CRITICAL（誤報）** |

**対策（2 段）**:

1. **probe の識別と WAF 許可（主対策）**: probe は固定の識別ヘッダ（例 `X-Auth-Probe: <secret>`）を付与し、**各アプリの WAF に「この識別を持つリクエストは Bot Control / レートルールの対象外」とする許可ルール**を入れる（Origin Protection の `X-Origin-Verify` と同じ発想。WAF は境界管理側のため設定依頼が必要 → M-Q-11-5）。probe は変更時のみ・低頻度なのでレートルールには通常かからないが、Bot Control / ジオ制限は明示許可がないと誤爆し得る
2. **classify 側の防御（副対策）**: WAF ブロック応答は API GW / アプリの 401/403 と**応答の形が異なる**（CloudFront/WAF 由来のエラーボディ・ヘッダ）。classify は WAF ブロックと判定できた場合、OK/CRITICAL ではなく **WARN「probe が境界でブロックされた（テスト構成問題）」** に分類し、Smoke test（§11.2.3）にも境界通過確認を含める

> **原則**: 「403 だから認証は効いている」とは**限らない**。probe が認証レイヤーに到達したことを確かめて初めて Negative の 401/403 を「正常」と判定できる。
...
|---|---|---|
| 正しく実装されている（想定どおり）| 401/403 で弾かれる | **なし** |
| **実装漏れ（＝本機構が検知したい状態）** | **リクエストが通り、本番データが更新される** | **あり** |

つまり「**検知したい異常が起きているときに限って副作用が出る**」という構造で、通常時に無害であることは安全の根拠にならない。

| 案 | 内容 | 評価 |
|---|---|---|
| ① | 本番の Negative も **GET 限定** | 副作用ゼロだが、**更新系 endpoint の認証漏れを検知できない穴**が残る（`POST /admin/*` の無認証は最も危険なパターン）|
| **②（推奨）** | 更新系は**空ボディ・不正 Content-Type** で送り、**401/403 以外はすべて認証漏れ疑い**として扱う | 認証を通過すればバリデーションで 400/415/422 になり実行前に落ちる。**副作用を最小化しつつ更新系の検知を維持**。判定は「401/403 なら正常、それ以外は疑い」に単純化できる |
| ③ | 更新系は **stg のみ**で検査 | 本番と stg の構成差の分だけ保証が落ちる |

→ **未決 M-Q-11-6**（§11.9）。決定するまでは、処理設計（確認-04）も現行どおり「全メソッド対象」として記述してある。
...
| ID | 内容 |
|---|---|
| ~~M-Q-11-1~~ | ~~probe 頻度（5min / 15min）~~ → **解決**: 自動差分検査（モード1、1h）/ 全量検査（モード2、日次定期+手動）の 2 モードに再設計済みで 5min/15min 級の heartbeat 型定期 probe は無い（18 章 / ADR-061）|
...
| M-Q-11-5 | **probe 識別ヘッダの WAF 許可ルール**（§11.2.4 主対策）を境界管理側（ネットワーク監査アカウント）へ依頼・合意。M-Q-10-3（NW-2 例外承認）と同じ相手のため同梱で調整可 |
| **M-Q-11-6** | **Negative probe における更新系メソッド（POST / PUT / PATCH / DELETE）の扱い**（2026-09-11 起票、§11.5.1）。現行 §11.5 は **Positive のみ**本番 POST をスキップと定めており、**Negative は全メソッドが対象**のまま。認証が正しく効いていれば 401/403 で副作用はないが、**まさに検知したい「認証漏れ」のときだけ本番データを更新してしまう**。案 ① 本番 Negative も GET 限定（更新系の認証漏れを検知できない穴が残る）/ **② 更新系は空ボディ・不正 Content-Type で送り「401/403 以外はすべて認証漏れ疑い」とする（副作用を最小化しつつ検知は維持。推奨）** / ③ 更新系は stg のみ（本番との構成差だけ保証が落ちる）。判断者: 共通基盤チーム + アプリ（顧客合意）|
