...
flowchart TB
    ...
    subgraph Gov["アプリが世に出る道（ガバナンスパス）"]
        ...
        SC --> REG["中央巡回が自動発見・登録<br/>（App Registry）"]
...
**ガバナンスフロー（アプリが世に出る）**: 実装 → **静的解析（04, deploy 前に止める）** → **Service Catalog デプロイ（05/17, 境界・認証・タグを自動付与）** → **中央巡回が自動発見・登録（17, アプリ側の登録作業なし）** → **認証 外形監視（10-18, 稼働中の漏れを検知）**。
...
認証外形監視（認証実装確認処理）は **別領域**（章 10-18）として扱う。本ガイドラインはアプリチームが日常的に守る「ルール」、外形監視は共通基盤チームが運用する「中央機構」。
...
| TP-3 | Runtime で **外形監視の対象になっている**（中央巡回が自動登録。リポジトリに `monitoring.yaml` を置くのが前提、17 章）| §5.3 |
...
## §1.2 責務分担（アプリチーム vs 中央〔Platform / 共通基盤 / ネットワーク監査〕）
...
| 領域 | アプリチームの責務 | 中央（Platform / 共通基盤 / ネットワーク監査）の責務 |
|---|---|---|
...
| **テストプロセス** | Pre-Deploy テスト実行 / OpenAPI 維持 | Service Catalog 製品 / 認証実装確認処理 / 検知基盤 |
...
flowchart TD
    ...
    Deploy --> Monitor[認証実装チェック<br/>監視対象に自動登録<br/>章 10-18]
...
| D-G-02 | 認証外形監視（認証実装確認処理）はガイドラインと分離し章 10-18 で扱う | アプリチームが日常守る「ルール」と中央運用の「機構」は読者・責務が異なる |
