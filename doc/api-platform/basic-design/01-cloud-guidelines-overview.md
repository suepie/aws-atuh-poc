...
認証外形監視（Central Canary）は **別領域**（章 10-18）として扱う。本ガイドラインはアプリチームが日常的に守る「ルール」、外形監視は Network 監査チームが運用する「中央機構」。
...
flowchart TD
    ...
    Deploy --> Monitor[Central Canary<br/>監視対象に自動登録<br/>章 10-18]
...
| D-G-02 | 認証外形監視（Central Canary）はガイドラインと分離し章 10-18 で扱う | アプリチームが日常守る「ルール」と中央運用の「機構」は読者・責務が異なる |
