...
ネットワーク監査 Acct に 1 テーブル。PK=`appId` / SK=`env`。
...
| 配置 | Cross-Acct | 説明 |
|---|:---:|---|
| **ネットワーク監査 Acct に配置** | 不要 | App Acct から Lambda を Invoke。DDB は同 Acct |
| App Acct に配置 | 要 | STS AssumeRole でネットワーク監査 Acct のロールを引き受けて Put（`CROSS_ACCT_ROLE_ARN`）|

→ **推奨は前者**（Lambda を中央に置き、App Acct からは Invoke するだけ）。Cross-Acct の複雑性を Registry 書込みだけに閉じ込める。
...
| D-M-12-4 | app-registry Lambda はネットワーク監査 Acct 配置を推奨 | Cross-Acct 複雑性を最小化 |
