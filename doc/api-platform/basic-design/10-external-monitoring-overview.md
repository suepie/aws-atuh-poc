...
| **認証構成情報** | アプリが提供する監視メタデータ `{appId}/monitoring.yaml` と API 仕様 `openapi.yaml`（+ 任意 `deploy-info.json`）。デプロイパイプライン最終段で認証構成情報連携バケットへアップロードされる（**旧称: 監視資材**、〜2026-09-10）|
| **認証構成情報連携バケット** | **App アカウント側**。各 App アカウントに StackSets 配布する S3 バケット `auth-monitoring-artifacts-{accountId}`（Versioning 有効）。アプリが認証構成情報を置き、中央はそれだけを読む。中央はこの**認証構成情報の VersionId 比較**で変更を検知する（コードリポジトリは外部ベンダー git のため中央から読めない。17 §17.3 / [ADR-061 追記 2026-08-21](../../adr/061-deploy-detection-pull-model.md)）。**旧称: 資材バケット** |
| **認証構成情報配置バケット** | **共通基盤アカウント側**。台帳 `registry/{appId}/{env}.json` と API 仕様の複写 `openapi/…` を保管する S3 バケット ×1（§10.1.5、12 / 13 章）。**旧称: Monitoring Registry** |
...
flowchart TB
    subgraph Central["共通基盤アカウント（中央運用・自社管理）"]
        ...
        Reg[認証構成情報配置バケット S3<br/>台帳 registry/ + spec openapi/]
    ...
    subgraph AppA["App アカウント A"]
        ARTA[("認証構成情報連携バケット S3<br/>monitoring.yaml + openapi.yaml")]
        APIA[API GW / ALB<br/>認証実装]
    end

    subgraph AppB["App アカウント B"]
        ARTB[("認証構成情報連携バケット S3")]
        APIB[API GW / ALB<br/>認証実装]
    end

    DISC -.->|"読み取り AssumeRole：<br/>認証構成情報 VersionId 比較・<br/>monitoring.yaml・spec 取得"| ARTA
    DISC -.->|"同"| ARTB
...
- **巡回の読み取り（対象検索 Lambda → 認証構成情報連携バケット）は境界を通らない**（AWS API〔S3〕を読み取りロールで直接呼ぶ、16 章）。変更検知は**認証構成情報の VersionId 比較**（[ADR-061 追記 2026-08-21](../../adr/061-deploy-detection-pull-model.md)）。認証構成情報に現れないコンソール直変更は L2 Config Rules + 全量検査（モード2、日次定期・最大 24h）が補完（17 章 §17.2.2）
...
flowchart TB
    ...
    subgraph Central["共通基盤アカウント（中央運用・自社管理）"]
        ...
        REG["認証構成情報配置バケット S3<br/>registry/（台帳+スナップショット）<br/>openapi/（spec コピー）"]
    ...
    subgraph AppAcct["各 App アカウント（監視対象）"]
        ...
        ART[("認証構成情報連携バケット S3<br/>monitoring.yaml + openapi.yaml")]
        APIGW["API GW / ALB<br/>認証実装"]
        ROLE["DiscoveryReadRole<br/>認証構成情報 s3 読み取り専用（StackSets 配布）"]
        SCP --> APIGW
    end

    SCH --> DISC
    DISC -->|"① AssumeRole（読み取り）"| ROLE
    ROLE -->|"② 認証構成情報 List・VersionId 比較 / GetObject"| ART
    DISC -->|"③ 自動登録・lastArtifactVersions 更新<br/>+ spec Put（認証構成情報の openapi.yaml）"| REG
...
> §10.1.1 との違い: こちらは **巡回の番号付きステップ（①〜⑥、17 章）と classify・Alarm（11 章）・通知（15 章）を明示**した完全版。登録・OpenAPI 取得は**すべて中央の pull（認証構成情報（S3）の VersionId 比較・[ADR-061 追記 2026-08-21](../../adr/061-deploy-detection-pull-model.md)）**で、アプリ側に登録処理はない（認証構成情報アップロードはデプロイパイプラインの標準ステップ、17 §17.3）。**2 本の経路が別物**であることに注意：巡回の読み取り（①②＝認証構成情報連携バケット）は AWS API を直接呼び境界を通らない。probe（⑤）は実ユーザーと同じく境界（ネットワーク監査アカウントの CloudFront）を通る。
...
```mermaid
flowchart LR
    D["① デプロイ & 認証構成情報アップロード<br/>04 静的解析 pass"] --> R["② 巡回発見<br/>（認証構成情報 VersionId 比較）<br/>最大 1h 後に検知・登録・spec 取得<br/>（17 章）"]
...
| **対象検索 Lambda** | Lambda | **変更検知と登録の実行体（pull）**。各 App アカウントの **認証構成情報連携バケットを読み取り巡回**し、認証構成情報（`{appId}/monitoring.yaml`）の発見・**認証構成情報 VersionId 比較（lastArtifactVersions）**・台帳登録・spec 取得を行い、変更のあったアプリの検査を起動する | 17 |
| **認証構成情報配置バケット** | **S3（Versioning）×1 バケット** | **台帳 + spec コピーの一体ストア**（DynamoDB は不使用、ADR-061 追記）。`registry/{appId}/{env}.json` = 監視対象の台帳＋巡回スナップショット（baseUrl / authPattern / 通知先 / **前回確認認証構成情報 VersionId（lastArtifactVersions）**。設定値は monitoring.yaml 由来、書き手は対象検索 Lambda）。`openapi/…` = 認証構成情報連携バケットのデプロイ版 openapi.yaml（正本はベンダー git）のさらにコピーで、endpoint 一覧と公開印（MON-1）の情報源 | 12 / 13 |
...
| **認証構成情報連携バケット** | S3（`auth-monitoring-artifacts-{accountId}`、StackSets 配布・Versioning）| **変更検知とメタデータの源**。デプロイパイプライン最終段で monitoring.yaml（監視宣言）+ openapi.yaml（デプロイ版 spec の写し）+ 任意 deploy-info.json をアップロード。認証構成情報の VersionId 変化が自動差分検査（モード1）のトリガー | 17 §17.2-3 |
| **DiscoveryReadRole** | IAM ロール（StackSets 配布）| **中央からの読み取り窓口**。対象検索 Lambda だけが AssumeRole でき、認証構成情報連携バケットの s3 read のみ | 16 §16.2 |
| **ArtifactUploadRole-{appId}** | IAM ロール（StackSets 配布）| **ベンダー CI からの認証構成情報アップロード窓口**。`{appId}/*` 限定の s3:PutObject のみ（デプロイロールと分離）| 16 §16.2.2 / 17 §17.3 |
...
> **1 行まとめ**: アプリが**デプロイ時に認証構成情報（monitoring.yaml + openapi.yaml）を認証構成情報連携バケットへアップロード**すると、**中央の対象検索 Lambda が 1 時間毎の巡回（認証構成情報 VersionId 比較）で見つけて台帳・仕様を自動登録**し、変更のあったアプリ（自動差分検査（モード1））と日次・監査時の全量（全量検査（モード2））に**認証実装チェック Lambda が実際にリクエストを投げて認証漏れを検査**、問題があれば **4×4 分類で適切なチームに通知**される。アプリ側に登録処理はない（モノリスも同じ）。
...
flowchart TB
    subgraph Central["共通基盤アカウント（自社管理）— VPC なし"]
        ...
        S3R[("認証構成情報配置バケット S3<br/>registry/ 台帳 + openapi/ spec")]
    ...
    subgraph AppAcct["各 App アカウント"]
        ROLE["DiscoveryReadRole<br/>（認証構成情報 s3 read-only）"]
        ART[("認証構成情報連携バケット S3<br/>monitoring.yaml + openapi.yaml")]
        APIGW["API GW / ALB<br/>認証実装"]
    end
...
#### F1. 認証構成情報連携フロー（CI 認証構成情報アップロード → 巡回検知 → 自動差分検査（モード1）起動）

```mermaid
sequenceDiagram
    participant DEV as ベンダー CI（デプロイパイプライン）
    participant ART as 認証構成情報連携バケット S3（App アカウント）
    participant SCH as Scheduler（共通基盤）
    participant DISC as 対象検索 Lambda（共通基盤・VPC 外）
    participant STS as STS
    participant S3 as 認証構成情報配置バケット S3（共通基盤）
    participant PROBE as 認証実装チェック Lambda（共通基盤）

    DEV->>ART: W1 認証構成情報アップロード（デプロイ成功後・ArtifactUploadRole-{appId}）
...
|---|---|---|---|---|---|
| P1 | 台帳・spec 取得 | 共通基盤 / 認証実装チェック Lambda | 共通基盤 / 認証構成情報配置バケット S3 | `s3.ap-northeast-1.amazonaws.com`（自動差分検査(モード1)=対象 1 件 Get / 全量検査(モード2)=`registry/` List→Get）| 443、IAM |
...
flowchart LR
    ...
    ALR -->|"N2 alertRouting 解決<br/>（S3 registry/ Get）"| S3[("認証構成情報配置バケット S3")]
