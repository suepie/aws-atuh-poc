...
| W1-6 | 外部調整 | 「コンソール直変更」を検知する別の仕組みの実在確認 | 【これは何か】本監視はコードの変更履歴（git）を見て検査する方式のため、**AWS コンソールで直接 API の認証設定を外した場合は検知できない**という既知の穴がある。この穴は AWS Config Rules（リソースの設定変更を常時チェックする別の仕組み）が塞ぐ前提で設計している。【なぜ調整が必要か】その Config Rules が組織に本当に整備されているかを確認していない。無ければ前提が崩れ、穴が塞がらないまま運用に入ってしまう。【やること】該当ルール（API GW の認証必須チェック）の有無を確認し、無ければ追加実装（+8 人時）を別タスクとして起票する | — | 4 | 0.5 |
| **小計** | | | | | **40** | **5.0** |
