...
| W1-6 | 外部調整 | 「コンソール直変更」を検知する別の仕組みの実在確認と整備 | 【これは何か】本監視はコードの変更履歴（git）に加え、デプロイ番号（deploymentId、C12）も見て手動変更を検知するが、**デプロイされていないコンソール編集や、API GW を使わない構成（ALB 直）の手動変更**は残る死角。この死角は AWS Config Rules（リソースの設定変更を常時チェックする別の仕組み）が塞ぐ設計とした（2026-08-19 方針確定 ②）。【なぜ調整が必要か】その Config Rules が組織に本当に整備されているかを確認していない。無ければ死角が塞がらないまま運用に入ってしまう。【やること】該当ルール（API GW の認証必須チェック）の有無を確認し、**無ければ追加実装（+8 人時）を必須タスクとして起票する**（前提頼みではなく本設計の必須対策。管理番号: 17 §17.2.2 ②）| — | 4 | 0.5 |
| **小計** | | | | | **40** | **5.0** |
...
| S0-d | 配布物 | DiscoveryReadRole 設計 | ロール設計書: 権限 6 アクション（codecommit 読み取り 5 + apigateway:GET＝deploymentId 併読用）・信頼ポリシー（信頼先 1 本 + ExternalId）・全アカウント同一名（16 §16.2 準拠のパラメータシート）。【補足】DiscoveryReadRole＝各 App アカウントに置く「中央からの読み取り窓口」となる IAM ロール（リポジトリの読み取りのみ可）。StackSets＝CloudFormation を複数アカウントへ一括配布する仕組みで、新規アカウントにも自動で配られる | — | 2 | 0.25 |
...
**W2 合計: 214 人時 = 26.8 人日**（A 31 + B 13 + 発見 60 + 検査 24.5 + 周辺 85.5）
...
| T-D | テスト | 結合テスト D: 障害・運用系（6 ケース）| D-1 1 アカウントの AssumeRole 拒否→他アカウント完走+AccountErrors→MM-3 / D-2 検査 invoke 失敗→リトライ→DLQ→MM-4 / D-3 Scheduler 停止→2h 後 MM-1 発火 / D-4 自動差分検査(モード1) 途中失敗→lastCheckedCommitId 据え置き→次巡回で再検知（at-least-once）/ D-5 全量検査(モード2) の手動起動と日次定期起動→fan-out で全アプリ検査 / D-6 台帳の手動更新と巡回の競合（ETag 412→リトライ）| W3-2 | 8 | 1.0 |
...
NW-2 例外否認 → VPC 戻し +40〜64h / L2 Config Rules 不在 → **+8h の実装が必須発生**（v4 で「前提頼み」から「無ければ必ず作る」に格上げ、W1-6）/ WAF 調整難航 → 副対策のみで暫定可 / SC 製品の組織承認が重い → 監視は先行リリース可 / Keycloak 遅延 → Negative のみ先行可
