...
| 2 | `18-scan-modes-and-scheduling.md` | **実行モデルの SSOT**（M1 巡回差分 1h / M3 手動フル / M2 将来。頻度・基盤の記述が他章と食い違ったら 18 章が勝つ）|
...
- **受容した穴 2 つ**（コンソール直変更の見逃し→L2 Config Rules+M3+SCP / コミット≠デプロイ→次回巡回+M3）は必ず載せる（17 §17.2.2）
...
| 6 | 実行モード | M1/M3/M2 の表 + 差分粒度（アプリ単位）+ 穴 2 つと補完 | 18 章, 17 §17.2.2 |
...
- [ ] M1 = 中央巡回・コミット差分・1 時間毎、probe 範囲 = アプリ単位全 endpoint
- [ ] 穴 2 つ + 補完レイヤーが実行モードまたはセキュリティのシートに記載されている
