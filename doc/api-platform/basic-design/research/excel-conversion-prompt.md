...
| 2 | `18-scan-modes-and-scheduling.md` | **実行モデルの SSOT**（自動差分検査（モード1、旧称 M1）＝1h 巡回差分 / 手動全量検査（モード3、旧称 M3） / 常時定期検査（モード2、旧称 M2）＝将来。頻度・基盤の記述が他章と食い違ったら 18 章が勝つ）|
...
- **受容した穴 2 つ**（コンソール直変更の見逃し→L2 Config Rules+手動全量検査（モード3）+SCP / コミット≠デプロイ→次回巡回+手動全量検査（モード3））は必ず載せる（17 §17.2.2）
...
10. **呼称改定**: M1/M2/M3 → 自動差分検査（モード1）/常時定期検査（モード2）/手動全量検査（モード3）（2026-08-20）。md 側に旧称 M1/M2/M3 が残っていたら新呼称に読み替えて転記する
...
| 6 | 実行トリガー（旧称: 実行モード）| **検査がいつ・何をきっかけに走るかの設計**。先頭に導入行「自動差分検査（モード1）＝変更を検知したら自動（1 時間毎巡回）/ 手動全量検査（モード3）＝運用者が手動で全量 / 常時定期検査（モード2）＝将来拡張」を置き、以降に自動差分検査（モード1）/手動全量検査（モード3）/常時定期検査（モード2）の表 + 差分粒度（アプリ単位）+ 穴 2 つと補完。処理フローシート（5）とは「動き方はシート 5、動くタイミングは本シート」の相互参照を明記 | 18 章, 17 §17.2.2 |
...
- [ ] 自動差分検査（モード1）= 中央巡回・コミット差分・1 時間毎、probe 範囲 = アプリ単位全 endpoint
