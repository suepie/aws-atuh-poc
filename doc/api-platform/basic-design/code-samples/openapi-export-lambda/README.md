...
> したがって S3 Put は通常 `CROSS_ACCT_ROLE_ARN` 経由の クロスアカウント書込になる。
> `GetExportCommand` は同一 アカウント内の API GW に対して既定クレデンシャルで実行する。
...
- **AWS SDK v3**（`aws-sdk` v2 禁止）:
  ...
  - S3 は `@aws-sdk/client-s3` の `PutObjectCommand`/`DeleteObjectCommand`。クロスアカウントは `@aws-sdk/client-sts` の `AssumeRoleCommand`。
