...
[ネットワーク監査 Acct]                          [各 App Acct]
  ...
  Central Canary (Synthetics Puppeteer 16.1)
    │ 5min 周期
    ├─ App Registry を Scan（全アプリ取得）
...
| ディレクトリ | 役割 | Runtime |
|---|---|---|
| [central-canary-puppeteer/](central-canary-puppeteer/) | Central Canary 本体（OpenAPI 動的発見、Hybrid 検証、4×4 分類）| `syn-nodejs-puppeteer-16.1` |
| [multi-checks-blueprint/](multi-checks-blueprint/) | 小規模用 Multi Checks Blueprint JSON（≤10 checks、OAuth ネイティブ）| `syn-nodejs-5.1` |
...
各アプリが deploy 時に 1 レコード登録。Central Canary がこれを Scan する。
...
**`authPattern` enum**（Central Canary が assertion 方式を切替）:
...
### 2.3 OpenAPI アノテーション（アプリチームが付与、Central Canary が解釈）
...
### 2.4 CloudWatch Metrics（Central Canary が emit）
...
### 2.5 4×4 真偽値表（分類ロジック、canary と alert-router で共有）
...
### 2.6 Alert イベント形式（canary → alert-router）
...
| 対象 | バージョン | 備考 |
|---|---|---|
| Synthetics Puppeteer runtime | **`syn-nodejs-puppeteer-16.1`** | Lambda Node.js 22.x、旧 `-7.0` は Deprecated |
| Synthetics Node.js-only runtime | **`syn-nodejs-5.1`** | Multi Checks Blueprint 用 |
| Synthetics namespace | **`@aws/synthetics-puppeteer`** / `@aws/synthetics-logger` | v13.1+ で旧 `Synthetics` から変更 |
| AWS SDK | **v3**（`@aws-sdk/client-*`）| Lambda Node.js 18+ は v2 非バンドル |
...
3. `central-canary-puppeteer` を Synthetics canary としてデプロイ（5min schedule）
