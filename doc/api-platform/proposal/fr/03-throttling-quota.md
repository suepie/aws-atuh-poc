...
| 基本方針 | 本章での具体化 |
|---|---|
| 絶対安全 | **全公開 API に rate-based WAF rule を必須化**（IP 単位、既定 5 分窓 / 60・120・300・600 秒から選択可） |
...
| 評価ウィンドウ | 60 / 120 / 300 / 600 秒（既定 300 = 5 分）|
| 1 Rule の閾値範囲（`Limit`）| **10 〜 2,000,000,000** req / 評価ウィンドウ（AWS 公式 API リファレンス確認 2026-07）|
