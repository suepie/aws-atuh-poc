...
| §6.2 | 公開範囲とロードバランサ | ALB / NLB / API Gateway VPC Link / 共有 ALB |
| **§6.2.A** | **ALB only vs API Gateway + ALB の選定基準** ★ | **Pattern X (ALB) vs Pattern Y (API GW + ALB)、選定マトリクス + デフォルト推奨** |
...
| 章 | モノリスでの注意 |
|---|---|
| §FR-API-1 公開範囲 | 区分 (Public/Internal/Partner) は **path-based**（`/admin/*` は Internal 扱い等） |
...
### §6.1.A.5 モノリスでの API Gateway 利用に関する留意

**結論**：**モノリス前段に API Gateway は技術的に可能だが、推奨しない**。SSR モノリスは HTML / 静的アセット / API を同一プロセスで配信する性質上、API Gateway を被せると不整合が発生する。

#### 推奨しない理由

| 観点 | 問題 |
|---|---|
| **HTML 配信のコスト** | API GW はリクエスト課金。HTML / page アクセスはトラフィック多 → コスト爆発（例：100M req/月で API GW $350 vs ALB $25 + LCU）|
| **静的アセット** | `/assets/*` `/_next/static/*` は API GW で配信不適、CloudFront 直接配信が正解 |
| **ペイロード上限** | API GW は **レスポンス 10 MB 上限**、大型 HTML やダウンロードで詰まる |
| **タイムアウト** | API GW は **29 秒上限**、Streaming SSR（Next.js 14+ Streaming HTML / Suspense）で問題化 |
| **WebSocket** | ライブ機能用に WebSocket 使う場合、REST API は別 API（WebSocket API）必要 |
| **ヘッダ・Cookie 制約** | API GW はヘッダサイズ・数に制限、Cookie 操作が SSR フレームワークで複雑化 |

#### 例外パターン：モノリス内に Partner B2B API を含む場合

```
[Public Path]
CloudFront → ALB → ECS Monolith                       ← /pages/*, /api/v1/*

[Partner B2B Path]（例外的に切り出し）
CloudFront → API GW REST + Usage Plan → VPC Link → ALB → ECS  ← /api/v1/partner/*
```

→ Partner B2B path のみ API GW を切り出す。ただし **モノリスの本質（同一プロセス配信）と矛盾するため、長期的には §C-API-2 §C-2.3 段階移行パスでマイクロサービス化を推奨**。
...
**主な判断軸**：公開範囲（§1）と整合、共有 ALB でコスト密度を上げる。
**§6 全体との関係**：§FR-API-1 公開範囲の Container 実装版。
...
## §6.2.A ALB only vs API Gateway + ALB の選定基準

**このサブセクションで定めること**：ECS 前段に **ALB を直接置く** か **API Gateway を被せる** かの選定基準。
**主な判断軸**：Usage Plan / JWT Authorizer / コスト / 制約（タイムアウト、ペイロード、WebSocket、Streaming）。
**§6 全体との関係**：§6.1.A モノリス vs マイクロサービス + §C-API-2 アーキパターン選定の延長線上での詳細選択。
...
**Pattern Y：API Gateway + ALB（VPC Link 経由）**

```
CloudFront → WAF → API Gateway → VPC Link → ALB → ECS Fargate
                    ↑
            (REST API or HTTP API)
            (Usage Plan / JWT Authorizer)
```

### §6.2.A.2 観点別比較表

| 観点 | Pattern X：ALB only | Pattern Y：API GW + ALB |
|---|:---:|:---:|
| **Usage Plan / API Key** | ❌ | ✅（REST API 限定） |
| **JWT Authorizer（マネージド）**| △（ALB Cognito 認証は session 系） | ✅（HTTP API JWT Authorizer） |
| **Stage / バージョン管理** | ❌ | ✅ |
| **Canary deployments** | △（target group weight） | ✅ stage canary |
| **Throttling** | △（WAF rate-based のみ） | ✅ stage/method 単位 |
| **コスト（1M req/月）**| ALB 固定費 ~$25 + LCU | API GW ~$3.5 + ALB |
| **コスト（100M req/月）**| ALB 固定費 ~$25 + LCU（~$50） | API GW ~$350 + ALB |
| **コスト（1B req/月）**| ALB 固定費 ~$25 + LCU（~$500） | API GW ~$3500 + ALB |
| **レイテンシ** | 低（直接） | +10-20ms（API GW 経由） |
| **WebSocket** | ✅ | △ HTTP API のみ対応、REST は別 API |
| **mTLS** | ✅ ALB Listener | ✅ Custom Domain |
| **Streaming レスポンス**（SSE 等）| ✅ | ❌ **29 秒タイムアウト** |
| **大容量レスポンス** | ✅ | ❌ **10 MB 上限** |
| **HTML / 静的配信** | ✅ 自然 | ❌ 不適 |
| **Partner B2B Usage Plan** | ❌ | ✅ |
| **AWS Marketplace 統合** | ❌ | ✅ |
| **HTTP/2 Server Push** | ✅ | △ |

### §6.2.A.3 選定マトリクス（実用判断）

| ケース | 推奨 | 理由 |
|---|---|---|
| **SSR モノリス**（Next.js full-stack / Rails 等）| **Pattern X：ALB only** | API GW は HTML 配信に不適、タイムアウト・ペイロード制約問題（§6.1.A.5） |
| **ECS バックエンド + SPA**（[§C-API-2 §C-2.1 パターン A](../common/02-runtime-selection-criteria.md)）かつ Partner B2B あり | **Pattern Y：API GW + ALB**（REST） | Usage Plan 必須 |
| **ECS バックエンド + SPA** かつ純粋 B2C / 内部 | **どちらでも可** | ALB 低コスト、API GW 機能豊富。トラフィック規模と要件で判断 |
| **トラフィック大（>100M req/月）+ 機能シンプル** | **Pattern X：ALB only** | コスト優位（API GW per-request 課金が痛い） |
| **マルチテナント SaaS（per-tenant throttle 必要）**| **Pattern Y：API GW（REST + Usage Plan）** | API Key per-tenant で Usage Plan |
| **マイクロサービス間（内部）**| **VPC Lattice or ALB（Service Connect）**、API GW は不要 | 内部通信に API GW のオーバーヘッドは無駄 |
| **WebSocket / Streaming / 大容量レスポンス**| **Pattern X：ALB only** | API GW の制約（29s / 10MB） |
| **AWS Marketplace SaaS 公開**| **Pattern Y：API GW** | Marketplace 課金統合は API GW + Usage Plan が必須 |

### §6.2.A.4 デフォルト推奨

| アーキパターン（[§C-API-2 §C-2.1](../common/02-runtime-selection-criteria.md)）| デフォルト前段 |
|---|---|
| **A. SPA + 別 API**（バックエンド ECS） | **Pattern X（ALB）が標準、Partner B2B あれば Pattern Y（API GW）に escalation** |
| **B. SSR + 別 API**（バックエンド ECS）| **Pattern X（ALB）が標準、Partner B2B あれば Pattern Y に escalation** |
| **C. SSR モノリス**（フルスタック ECS）| **Pattern X（ALB）一択**、API GW は非推奨（§6.1.A.5） |
| マイクロサービス間（内部）| ALB or Service Connect or VPC Lattice、API GW は非推奨 |

### §6.2.A.5 ハイブリッド構成

実務では **同一アプリで Pattern X + Pattern Y を path 別に併用**するパターンも可：

```
CloudFront → WAF
  ├─ /api/v1/public/* → ALB → ECS              ← B2C 用、ALB only
  ├─ /api/v1/partner/* → API GW REST + Usage Plan → VPC Link → ALB → ECS  ← Partner B2B
  └─ /assets/* → S3 (静的配信)                   ← CloudFront キャッシュ
```

→ **Path 別の前段選定**は柔軟性高いが、設計・運用複雑化。**新規アプリは原則どちらかに寄せる**ことを推奨。

### §6.2.A.6 TBD / 要確認

- Q: **ECS バックエンドの前段に ALB only / API GW + ALB のどちらをデフォルトとするか** → `API-B-623` ⭐
