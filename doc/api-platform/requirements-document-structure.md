...
API プラットフォーム要件定義書および対関係者説明資料は、以下の 7 ステップで論理を組み立てる。**「アーキパターン選定 → 公開範囲（信頼プロファイル）→ 認証認可 → 流量制御 → 実装ランタイム → ガードレール → 観測性」** が本フェーズの中核ストーリー。
...
| ① | **どんな公開範囲（信頼プロファイル）の API を扱うか？**（パブリック認証有 / オープン / 社内 / パートナー / 社内限定、判定フロー） | `functional-requirements.md §FR-API-1 公開範囲（信頼プロファイル）` | API Gateway endpoint type / CloudFront / PrivateLink パターン |
...
| 公開範囲 | API Gateway（HTTP/REST）/ Function URL | ALB / NLB / API Gateway VPC Link |
...
flowchart TB
    subgraph App[各アプリ AWS アカウント]
        L1[公開範囲層<br/>API Gateway / ALB / CloudFront]
...
│   │   │   ├── 00-index.md
│   │   │   ├── 01-exposure-boundary.md   ← §FR-API-1 公開範囲（信頼プロファイル）— 5 Profile を統合概念で
...
| § | カテゴリ | 接頭辞 | 想定サブセクション |
|---|---|---|---|
| 1 | 公開範囲（信頼プロファイル）| `FR-API-EXP-*` | §1.1 Profile 統合定義 / §1.2 Profile 別ネットワーク構成 / §1.3 Profile 変更プロセス |
...
### A.0 章立て構造（4 層 + 横串）の根拠

「公開範囲 → 認証認可 → 流量制御 → 実装ランタイム + 横串（観測性・ガードレール・コスト）」の **4 層 + 横串モデル**は、**AWS が公式に命名したフレームワークではない**。以下の公式・業界ソースに共通する論述順を抽出した **本標準の合成・命名** である。

#### A.0.1 抽出元と抽出された要素

| 抽出元 | 種別 | 抽出された要素 |
|---|---|---|
| **Amazon API Gateway Developer Guide** TOC | AWS 公式 | Endpoint Types / Authorization / Throttling / Integrations |
| **AWS Well-Architected Framework** + **Serverless Applications Lens** | AWS 公式 | API design / Security（認証認可）/ Throttling / Backend integration |
| **AWS Prescriptive Guidance「Multi-tenant SaaS API access control: Implementation options and best practices」** | AWS 公式 | Tenant isolation（境界）/ AuthZ / Throttling per tenant / Implementation patterns |
| **AWS re:Invent / Builders Library** API best practices 系 | AWS 公式 | 同上の論述順 |
| **"Microservices Patterns" (Chris Richardson)** API Gateway パターン | 業界標準 | Routing / Security / Rate limiting / Request transformation |
| **OWASP API Security Top 10** | 業界標準 | 認可 / 流量制御 / データ露出（部分的に対応） |
| **Kong / Apigee / MuleSoft 公式 doc** API gateway 概念章 | 業界標準 | Publish / Secure / Control / Route の章構成 |

#### A.0.2 「4 層」という分解粒度の選定根拠

公開範囲・認証認可・流量制御・実装ランタイムは、**上記の各ソースで独立した章として扱われる単位**。これらを：
- **3 層以下に統合** → 同一文脈で扱われる単位が崩れ、章の独立性が失われる
- **5 層以上に細分化** → 同一文脈の議論が散逸し、横串の整理が難しい

したがって **4 層は複数ソースの章境界に最も整合する粒度** として選定した。「観測性・コスト・ガードレール」は **どの層にも横断する関心事**であるため、4 層に並列せず **横串** として独立扱いとした。

#### A.0.3 「AWS 流」という表現の射程

本書および関連ドキュメントで「**AWS 流**」「**AWS の API 設計議論で自然に出てくる**」と表現している箇所は、**AWS が公式に名付けたフレームではなく、AWS 公式ドキュメント群に共通する論述順** の意味である。直接引用ではなく **構造的合致** を示す表現として使用している。

レビュアー対応の標準回答：
> 「4 層モデル」は AWS が公式に命名したフレームワークではありません。API Gateway Developer Guide / Well-Architected Serverless Lens / Prescriptive Guidance「Multi-tenant SaaS API access」/ 業界標準の API Gateway パターンに **共通する論述順を抽出した本標準の合成** です。抽出元はすべて公開された公式・業界標準のため、**第三者が同じソースを当たれば同じ構造に辿り着く再現性**は担保されています。

---

### A.1 API Gateway / 公開範囲
...
**「標準化対象の意思決定を、AWS 公式・業界標準ソース群から抽出した 4 層 + 横串 × IPA 非機能グレードで分解し、各単位で『AWS 公式ベースライン』と『組織判断・実測データ・トレードオフの選好』を対で抽出した」**

属人判断ではなく、**3 つの公的・準公的フレーム**（AWS 公式ガイダンス / IPA 非機能要求グレード / 認証側で合意済みの要件定義流儀）に乗せて機械的に切り出した、と説明可能な構造。

ただし「**4 層 + 横串**」自体は AWS が公式に命名したフレームワークではなく、複数の AWS 公式 doc + 業界標準に共通する論述順を抽出した本標準の合成である（根拠は [付録 A.0](#a0-章立て構造4-層--横串の根拠)）。

### B.2 抽出を支える 4 つの軸

#### 軸 1: 章立て構造 — 4 層 + 横串（複数ソースからの合成）

公開範囲 → 認証認可 → 流量制御 → 実装ランタイム + 横串（観測性・ガードレール・コスト）

**根拠ソース**（複数ソースに共通する論述順を抽出。詳細は [付録 A.0](#a0-章立て構造4-層--横串の根拠) ）:
- Amazon API Gateway Developer Guide TOC（Endpoint Types / Authorization / Throttling / Integrations）
- AWS Well-Architected Framework + Serverless Applications Lens
- AWS Prescriptive Guidance「Multi-tenant SaaS API access」「Multi-account strategy」
- AWS Security Reference Architecture (SRA)
- 業界標準：API Gateway パターン（Microservices Patterns）/ OWASP API Security Top 10 / Kong・Apigee・MuleSoft 公式 doc

**位置づけ**: **AWS が公式に名付けたフレームワークではなく、上記ソースに共通する論述順を抽出した本標準の合成・命名**。第三者が同じソースを当たれば同じ構造に到達する **再現性は担保** している。

**効果**: ユーザー要望の 6 テーマがこの 4 層 + 横串に機械的に **8 章 FR + 9 章 NFR + 5 章 common** へマップされる。
...
| 章 | 引用根拠 |
|---|---|
| §FR-API-1 公開範囲（信頼プロファイル）| API Gateway endpoint type 3 種、HTTP API vs REST API、CloudFront + WAF Perimeter Protection、ネットワーク × 認証 × WAF を 1 つの Profile に統合 |
...
| カテゴリ | 例 | 主な優先度 |
|---|---|:---:|
| **組織判断**（承認権限・予算・体制） | 公開範囲昇格の承認者、SecOps と Platform の境界、必須タグ CostCenter 粒度 | 🔥 |
...
| 優先度 | 基準 | 件数 |
|:---:|---|---:|
| 🔥 最優先 | 他の章の前提になる項目（公開範囲デフォルト、ランタイム選定方針、共有認証基盤との境界、必須タグ、Service Catalog 初期ラインナップ 等）+ 中核ストーリーに直結 | 25 |
...
>
> 1. **章立て構造**：複数の AWS 公式ドキュメント（API Gateway Developer Guide / Well-Architected Serverless Lens / Prescriptive Guidance「Multi-tenant SaaS API access」）および業界標準（API Gateway パターン / OWASP API Security Top 10）に共通する **論述順を抽出した 4 層モデル（公開範囲 → 認証認可 → 流量制御 → 実装ランタイム）+ 横串（観測性・ガードレール・コスト）** で機能要件を切り出しました。AWS が公式に命名したフレームではありませんが、複数ソースを当たれば同じ構造に到達する再現性が担保されています。
>
...
**Q0: 「4 層 + 横串」は AWS 公式のフレームワークか？**

A: **AWS が公式に命名したフレームワークではありません**。Amazon API Gateway Developer Guide TOC / Well-Architected Serverless Lens / Prescriptive Guidance「Multi-tenant SaaS API access」/ 業界標準の API Gateway パターン（Microservices Patterns）/ OWASP API Security Top 10 / Kong・Apigee・MuleSoft 公式 doc に **共通する論述順を抽出した本標準の合成・命名** です。詳細な抽出元と粒度選定根拠は [付録 A.0](#a0-章立て構造4-層--横串の根拠) を参照。
抽出元はすべて公開された公式・業界標準のため、**第三者が同じソースを当たれば同じ構造に到達する再現性**は担保されています。本書で「AWS 流」「AWS の API 設計議論で自然に出てくる」と表現している箇所は、**直接引用ではなく構造的合致**の意味です。
...
A: 全件を一度に揃える必要はなく、🔥 **25 項目（Stage 1）** で本標準の中核判断（公開範囲デフォルト、Serverless/Container 選定方針、監査アカウント役割、Service Catalog 初期ラインナップ）が確定する設計です。残り 110 項目は標準の細部チューニングで、Phase B/C で段階的に確認。**段階的収束を前提とした構造**であり、項目数の多さは網羅性の表れです。
