# Google Cloud 共通基盤 GitOps / ChatOps 構成設計書 (v2.2 統合確定版)
...
v2.1での改訂: ①アクター記号（A1〜A4）を廃止し役割名のみに ②GitHub PR画面の参考イメージを追加 ③Discussions画面イメージをFirewall追加の例に変更 ④付録CにVertex AI連携のフィジビリティ表を追加。以後の更新はmd（本書）を正とし、drawioは必要時に再生成する。
v2.2での追加: **提案用エグゼクティブサマリを冒頭に新設（§0）**。
...
> ### 「GitHubに集まる。AIが下ごしらえする。人が決める。」

Google Cloud共通基盤の運用を、**GitHubを唯一の窓口・唯一の記録**として再構築する。Firewall・VPC-SCの変更、案件からの申請・質問、日々の運用タスクをすべてGitに集約し（GitOps）、Teams通知とAI（Gemini）による分析・起案を組み合わせる（ChatOps）。AIは差分の検知・エラーの原因推定・回答や修正PRの下書きまでを担い、**実環境に触れるのは人が承認したパイプラインだけ**。担当者の仕事は「調べて書き起こす」から「レビューして決める」に変わる。

### メッセージ候補（用途別）

| 相手 | メッセージ |
|---|---|
| 全体（メイン） | GitHubに集まる。AIが下ごしらえする。人が決める。 |
| 統制・セキュリティ重視層 | 変更はPRだけ。実行は承認後だけ。調べ物はAIが先回り。 |
| 現場担当者 | 「探す・書き起こす・転記する」を、共通基盤の運用から消す。 |
| 中長期の価値訴求 | 運用すればするほど賢くなる基盤へ。 |

### 差別化ポイント（なぜこの設計か）

| ポイント | 内容 |
|---|---|
| **AIに実行させない設計** | AIの役割は提案・起案まで。プレイブック適用条件 → policy check → CODEOWNERS → Environments承認の**4層ガードレール**で統制し、承認なしの完全自動化（Lv5）は仕様として採用しない（§11.5）。AI導入への不安に対する回答が設計に組み込まれている |
| **作らない設計** | GitHub標準機能（Issue Form / Discussions / Actions / Environments）とGCPマネージド（Gemini / Vertex AI Search）の組み合わせが主体。自前開発は担当者コパイロットのみで、受付（Issue Form）は初日から動く。低コスト・低保守 |
| **運用するほど賢くなる設計** | 解決した問い合わせはFAQへ、対応した障害はRunbookへ、頻出パターンは自動対応プレイブックへ自動昇格（図18）。効果が逓増する構造 |

### Before → After

| 現状 | 導入後 |
|---|---|
| FW/VPC-SCは手動変更、変更履歴は人の記憶 | 変更は100% PR経由・二段承認。履歴と根拠が全件Gitに残る |
| ルール名でしか案件を区別できない | 全ルールが「どの案件・何のため・いつまで」を持つ台帳に（§7）。棚卸しは期限到来で自動起票 |
| ドリフト・野良変更は事故や監査で発覚 | 24時間以内に分類済みタスク化。手動変更は即日検知（§9） |
| 問い合わせはゼロから調査、知見は担当者の頭の中 | AIが解釈・類似事例・回答ドラフト付きで届き、担当者は確認して返すだけ（図20）。知見はFAQ/Runbookへ自動蓄積 |
| 定型作業（申請反映・期限切れ削除・メンテ準備）も人が起票からPRまで作成 | 検知・予定から修正PRドラフトまで自動生成（§11.5）。人はレビューと承認のみ |

### 効果仮説（導入後に計測して差し替える前提の仮置き）

- 問い合わせの一次回答リードタイム: AIドラフトの自動添付により**即日化**
- 定型申請（FW/VPC-SC）の反映リードタイム: 申請→PRドラフト自動生成により**短縮**（Phase 4）
- 棚卸し工数: `review_by`期限管理の自動化により**大幅削減**
- 監査対応: 全変更がPR・申請Issueにトレースされ**即答可能**に
...
| アクター | 誰か | 主な関心事 |
|---|---|---|
| 基盤エンジニア | 共通基盤チーム担当者 | 変更作業、エラー対応、問い合わせ回答 |
| 承認者（基盤リーダー） | チーム内承認権限者 | 変更妥当性の確認、apply承認 |
| 案件利用者（各案件の代表者） | 各案件のインフラ担当 | 申請・困りごと解決・仕様確認 |
| 自動システム | Actions / Cloud Run / Gemini | 検知・分類・起票・ドラフト・通知 |
...
**参考: PR画面のイメージ（上部のみ・モック）** — plan影響サマリの自動コメント、CI結果、CODEOWNERSレビュー必須、マージ≠即反映の注記。

![GitHub PR 画面イメージ](images/mockup-github-pr.svg)
...
| IF | 対象 | 役割 | 実装 |
|---|---|---|---|
| **GitHub Discussions（Q&A受付・supportリポジトリ）** | 案件代表者（招待制） | 自由な質問の投稿。AIボットが**出典つき一次回答を自動コメント**（公開用RAGのみ参照・根拠なしは回答しない・応答数十秒〜1分）。未解決時は要約プレフィル済み起票URLを案内 | Actions（discussionトリガ）→ Cloud Run → Gemini（図22と同じRAG構成） |
| **GitHub Issue Form（申請受付・supportリポジトリ）** | 案件代表者（招待制） | 正式な問い合わせ・**申請**の受付と対話スレッド。申請の構造化はPR自動生成（§11.5）の前提のため維持 | GitHub Issue Form（付録A）。メンバー管理は払い出しIaCで自動化 |
| **担当者コパイロット（v1.8: ポータルはこれ専用に縮小）** | 基盤エンジニア | 問い合わせIssue/Discussionごとのワークスペース。AIの解釈・類似事例・ベストプラクティス・回答ドラフトを提示し、チャットで練り直し → 確定回答をIssue/Discussionへ投稿 | Cloud Run + IAP。利用者向けチャットUIは不要化（全利用者が代表者=GitHub上で完結するため）。将来の利用者拡大時に復活可 |
| **iac: Discussions** | 基盤エンジニア | 基盤内のカジュアル相談・設計議論（変更なし）。関連Runbook・過去Issue・IaCコードの自動参照コメント | GitHub Discussions |

### 図19: 利用者IF（基盤ポータル）のフロー

```mermaid
flowchart LR
    U1["利用者: 質問を投稿"] --> U2["RAG検索<br/>(FAQ/ガイド/公開ナレッジのみ)"]
...
**参考: Q&A画面のイメージ（モック・Firewall追加の例）** — 質問→AIボットの出典つき一次回答→「回答としてマーク」→そのまま申請へ進める導線、の流れ。

![GitHub Discussions Q&A 画面イメージ](images/mockup-github-discussions.svg)

### Vertex AI連携のフィジビリティ（v2.1追記）

Discussionsのやり取りの頭脳は**GCP側のVertex AI（Gemini＋Vertex AI Search）**である。GitHubは入出力の場に徹し、AIはGitHub内に組み込まない。実現性の根拠:

| 観点 | 内容 | 評価 |
|---|---|---|
| トリガ | GitHub Actionsの `discussion` / `discussion_comment` イベント（標準機能） | ○ |
| GitHub→GCP認証 | CI/CDで採用済みのWorkload Identity連携をそのまま流用（キーレス・新規認証基盤不要） | ○ |
| 回答の書き戻し | GitHub GraphQL API（addDiscussionComment） | ○ |
| 会話の文脈 | スレッドの過去コメントをGraphQLで取得しGeminiに渡す → 追加質問にも文脈つきで応答 | ○ |
| ループ防止 | 「Q&Aカテゴリ限定」＋「ボット自身のコメントに反応しない」の2条件 | ○ |
| レイテンシ | Actions経由で**30秒〜1分程度**（掲示板的な体感）。高速化が必要ならGitHub App Webhook→Cloud Run直結で数秒（公開エンドポイント＋署名検証の整備が必要） | △→○ |
| 推奨 | **まずActions方式（実装最小）で開始し、体感が悪ければWebhook方式へ移行** | ― |
